...
function offsetNaturalDate(day: string, amountValue: string, unitValue: string): string | null {
  ...
  if (/^d/.test(unit)) date.setDate(date.getDate() + amount);
    ...
    date.setDate(
      Math.min(targetDay, new Date(date.getFullYear(), date.getMonth() + 1, 0).getDate()),
    );
  } else {
...
export function parseNaturalDate(
  ...
  const value = String(phrase || "")
    ...
    .replace(/,/g, "")
    .replace(/\s+/g, " ");
  const relativeFrom = value.match(
    /^(\d+)\s*(d|days?|w|weeks?|mo|months?|y|years?)\s+from\s+(.+)$/,
  );
  if (relativeFrom) {
  ...
  if (["today", "tod", "tonight", "evening", "eve", "this evening", "this eve"].includes(value))
    return today;
  ...
  const ordinalWeekday = value.match(
    /^(?:(\d+)(?:st|nd|rd|th)|(last))\s+(sun(?:day)?|mon(?:day)?|tue(?:s(?:day)?)?|wed(?:nesday)?|thu(?:rs(?:day)?)?|fri(?:day)?|sat(?:urday)?)\s+(jan(?:uary)?|feb(?:ruary)?|mar(?:ch)?|apr(?:il)?|may|jun(?:e)?|jul(?:y)?|aug(?:ust)?|sep(?:tember)?|oct(?:ober)?|nov(?:ember)?|dec(?:ember)?)(?:\s+(\d{4}))?$/,
  );
  if (ordinalWeekday) {
    const months = [
    ...
    const calculate = (): Date | null => {
      ...
      candidate.setDate(
        1 + ((targetDay - candidate.getDay() + 7) % 7) + (Number(ordinalWeekday[1]) - 1) * 7,
      );
  ...
  if (namedDate) {
    ...
    const candidate = new Date(
      explicitYear || new Date(`${today}T12:00:00`).getFullYear(),
      month,
      Number(namedDate[2]),
      12,
    );
    if (!explicitYear && localDay(candidate) < today)
  ...
  if (numericDate) {
    ...
    if (!numericDate[3] && localDay(candidate) < today)
      candidate.setFullYear(candidate.getFullYear() + 1);
    return localDay(candidate);
  }
...
export function parseNaturalTask(
  ...
  const deadlineMatch = title.match(
    new RegExp(`\\s(?:deadline|due)\\s+(${NATURAL_DATE_SOURCE})(?=\\s|$)`, "i"),
  );
  if (deadlineMatch) {
  ...
  const dateMatch = title.match(
    new RegExp(
      `\\s(${NATURAL_DATE_SOURCE})(?:\\s+(?:at\\s+)?(\\d{1,2})(?::(\\d{2}))?\\s*(a|am|p|pm)?)?\\s*$`,
      "i",
    ),
  );
  if (dateMatch) {
