...
export function QuickFind({
  ...
  return (
    <WaDialog
      ref={dialog}
      class="objects-dialog dialog-quick-find"
      label="Quick find"
      without-header
      light-dismiss
    >
      <div class="quick-find">
          ...
          <input
            ...
            onKeyDown={(event) => {
              if (event.key === "ArrowDown") {
                ...
                setFocused(Math.min(results.length - 1, active + 1));
              }
              if (event.key === "ArrowUp") {
                event.preventDefault();
                setFocused(Math.max(0, active - 1));
              }
              if (event.key === "Enter") {
                event.preventDefault();
        ...
        <div class="search-results" role="listbox" aria-label="Quick Find results">
          {results.length ? (
            ...
            <div class="search-empty">No matching to-dos or lists</div>
          )}
        </div>
      </div>
    </WaDialog>
