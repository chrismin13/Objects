...
import {
  parseNaturalDate as parseNaturalDateCore,
  parseNaturalTask as parseNaturalTaskCore,
} from "./app/model";
import {
...
import {
  ...
  mountChecklistSortable,
  mountHeadingSortable,
  mountTaskSortables,
} from "./ui/sortable";
...
const icons = {
  ...
  sparkle:
    '<path d="m12 3-1.4 3.6L7 8l3.6 1.4L12 13l1.4-3.6L17 8l-3.6-1.4L12 3Z"/><path d="m5 14-.9 2.1L2 17l2.1.9L5 20l.9-2.1L8 17l-2.1-.9L5 14Z"/>',
...
function renderWeekdayPicker(
  selectedDays = [],
  dataAttribute = "data-weekday",
  ariaPrefix = "Repeat on",
) {
  const selected = new Set((selectedDays || []).map(Number));
  return orderedWeekdays()
    .map(
      (day) =>
        `<button class="chip ${selected.has(day.value) ? "active" : ""}" type="button" ${dataAttribute}="${day.value}" aria-pressed="${selected.has(day.value)}" aria-label="${esc(`${ariaPrefix} ${day.label}`)}"><span class="weekday-long">${day.short}</span><span class="weekday-short">${day.narrow}</span></button>`,
    )
    .join("");
}
...
function suppressRepeatedActivation(event) {
  if (
    ...
    !event.target.closest?.(
      'button, [role="button"], [data-task-id], [data-view], [data-modal-close], a[href], input[type="checkbox"], input[type="radio"], label[for]',
    )
  )
    return;
...
function acknowledgeChanges(changes, serializedAck) {
  ...
  for (const kind of ENTITY_KINDS) {
    ...
    const items = new Map(
      (baseline[kind] || []).filter((item) => !deleted.has(item.id)).map((item) => [item.id, item]),
    );
    for (const change of changes.entities?.[kind] || []) {
      items.set(change.id, {
        ...(items.get(change.id) || { id: change.id }),
        ...cloneData(change.patch),
      });
    }
...
function readRememberedSpace() {
  try {
    return localStorage.getItem(spaceSelectionStorageKey()) || "all";
  } catch (_) {
    return "all";
  }
}
...
function rememberActiveSpace() {
  try {
    localStorage.setItem(spaceSelectionStorageKey(), ui.activeSpaceId);
  } catch (_) {}
}
...
function scheduledSpaceOnLaunch() {
  ...
  const match = (schedule.rules || []).find(
    (rule) => spaceById(rule.spaceId) && scheduleRuleMatches(rule),
  );
...
function initializeActiveSpace() {
  ...
  ui.activeSpaceId =
    scheduled || (remembered === "all" || spaceById(remembered) ? remembered : "all");
}
...
function rememberPendingDeletion(taskId) {
  const revision = (ui.pendingEntry?.revision || 0) + 1;
  ui.pendingEntry = {
    task: { id: taskId, title: "", __deleted: true },
    revision,
...
export function mountObjects(serializedState, options) {
  app = $("#objects-shell") || $("#app");
  content = $("#content");
  sidebar = $("#sidebar-nav");
  sidebarPanel = $("#sidebar");
  inspector = $("#inspector");
  persistChanges = options.saveChanges;
  initializePersistentState = options.initializeState;
  performSignOut = options.signOut;
  ui.user = options.user;
  try {
    ui.saveReady = false;
    ui.saveQueued = false;
    ui.saveFailures = 0;
    ...
    void initializePersistentState(JSON.stringify(ui.syncedState))
      .then((state) => {
        ui.saveReady = true;
...
export function syncObjectsState(serializedState) {
  ...
  try {
    ...
    if (
      !remote.updatedAt ||
      (remote.updatedAt === ui.syncedState?.updatedAt &&
        remote.syncMutationId === ui.syncedState?.syncMutationId)
    )
      return;
    ...
    for (const kind of ENTITY_KINDS)
      merged[kind] = mergeCollection(previous[kind], local[kind], remote[kind]);
...
function normalizeState() {
  ...
  if (ui.state.settings.quickDraft && typeof ui.state.settings.quickDraft.value !== "string")
    ui.state.settings.quickDraft = null;
  if (!["immediately", "daily", "manually"].includes(ui.state.settings.logCompletedItems))
    ui.state.settings.logCompletedItems = "daily";
  ui.state.version = 7;
  ui.state.spaces ||= [];
  ui.state.areas ||= [];
  ui.state.projects ||= [];
  ...
  ui.state.calendarEvents = ui.state.calendarEvents.filter(
    (event) => event && typeof event.start === "string" && typeof event.title === "string",
  );
  ui.state.tasks ||= [];
  if (!ui.state.spaces.length) {
    const personalArea = ui.state.areas.find(
      (area) =>
        String(area.title || "")
          .trim()
          .toLocaleLowerCase() === "personal",
    );
    const workArea = ui.state.areas.find(
      (area) =>
        String(area.title || "")
          .trim()
          .toLocaleLowerCase() === "work",
    );
    const personalSpace = {
      id: "space-personal",
      title: personalArea?.title || "Personal",
      color: personalArea?.color || "#e49b3c",
      pinned: true,
      order: 0,
    };
    const workSpace = {
      id: "space-work",
      title: workArea?.title || "Work",
      color: workArea?.color || "#5b7cfa",
      pinned: true,
      order: 1,
    };
  ...
  const personalSpace =
    ui.state.spaces.find((space) => String(space.title).toLocaleLowerCase() === "personal") ||
    ui.state.spaces[0];
  const workSpace =
    ui.state.spaces.find((space) => String(space.title).toLocaleLowerCase() === "work") ||
    ui.state.spaces[1] ||
    personalSpace;
  if (!spaceById(ui.state.settings.defaultSpaceId))
    ui.state.settings.defaultSpaceId = personalSpace?.id || null;
  ui.state.settings.spaceSchedule ||= {
    enabled: false,
    rules: workSpace
      ? [
          {
            id: "rule-work-weekdays",
  ...
  ui.state.settings.spaceSchedule.rules = Array.isArray(ui.state.settings.spaceSchedule.rules)
    ? ui.state.settings.spaceSchedule.rules
        .filter((rule) => rule && spaceById(rule.spaceId))
        .map((rule, index) => ({
          id: rule.id || uid("space-rule"),
    ...
    : [];
  ui.state.areas.forEach((area) => {
    area.spaceId ??= null;
  });
  ui.state.projects.forEach((project) => {
    const parentArea = areaById(project.areaId);
    project.spaceId = parentArea
      ? parentArea.spaceId
      : project.spaceId || ui.state.settings.defaultSpaceId || null;
  });
  ui.state.calendarEvents.forEach((event) => {
    if (event.spaceId === undefined) {
      const namedSpace = ui.state.spaces.find((space) =>
        String(event.calendar || "")
          .toLocaleLowerCase()
          .includes(String(space.title).toLocaleLowerCase()),
      );
  ...
  ui.state.tasks.forEach((task, index) => {
    ...
    task.repeat ||= null;
    task.reminderSentAt ||= null;
    if (["completed", "canceled"].includes(task.status) && task.loggedAt === undefined)
      task.loggedAt = task.completedAt || new Date().toISOString();
    ...
    if (!parentProject && !parentArea && !spaceById(task.spaceId))
      task.spaceId = ui.state.settings.defaultSpaceId || null;
  });
  ui.state.projects.forEach((project, index) => {
    ...
    project.bucket ||= project.scheduledFor
      ? project.scheduledFor <= localDay()
        ? "today"
        : "upcoming"
      : "anytime";
    if (["completed", "canceled"].includes(project.status) && project.loggedAt === undefined)
      project.loggedAt = project.completedAt || new Date().toISOString();
  ...
  const discoveredTags = [
    ...(ui.state.settings.tags || []),
    ...ui.state.areas.flatMap((area) => area.tags),
    ...ui.state.projects.flatMap((project) => project.tags),
    ...ui.state.tasks.flatMap((task) => task.tags),
  ];
  ui.state.settings.tags = cleanTagList(discoveredTags);
  const canonicalTags = new Map(
    ui.state.settings.tags.map((tag) => [tag.toLocaleLowerCase(), tag]),
  );
  [...ui.state.areas, ...ui.state.projects, ...ui.state.tasks].forEach((item) => {
    item.tags = cleanTagList(item.tags).map(
      (tag) => canonicalTags.get(tag.toLocaleLowerCase()) || tag,
    );
  });
...
function createTagInPicker(button) {
  ...
  let tag = getKnownTags().find(
    (existing) => existing.toLocaleLowerCase() === requested.toLocaleLowerCase(),
  );
  const created = !tag;
  if (!tag) tag = registerTags([requested])[0];
  let checkbox = $$("[data-tag-choice]", picker).find(
    (choice) => choice.value.toLocaleLowerCase() === tag.toLocaleLowerCase(),
  );
  if (!checkbox) {
    const field = picker.dataset.tagPicker;
    $(".tag-picker-options", picker).insertAdjacentHTML(
      "beforeend",
      `<label><input type="checkbox" value="${esc(tag)}" data-tag-choice="${esc(field)}" checked><span>${esc(tag)}</span></label>`,
    );
...
function bindTagPicker(root) {
  $$("[data-create-tag]", root).forEach((button) =>
    button.addEventListener("click", () => createTagInPicker(button)),
  );
  root.addEventListener("keydown", (event) => {
    ...
    if (input && event.key === "Enter") {
      event.preventDefault();
      createTagInPicker(
        $(`[data-create-tag="${input.dataset.newTag}"]`, input.closest("[data-tag-picker]")),
      );
    }
  });
...
export function applyLogbookPolicyToItems(items, policy, now = new Date()) {
  ...
  for (const item of items) {
    ...
    if (policy === "daily" && (!item.completedAt || localDay(new Date(item.completedAt)) >= today))
      continue;
...
function logCompletedNow() {
  return applyLogbookPolicyToItems(
    [...ui.state.tasks, ...ui.state.projects].filter(matchesActiveSpace),
    "immediately",
  );
}
...
function applyLogbookPolicy() {
  return applyLogbookPolicyToItems(
    [...ui.state.tasks, ...ui.state.projects],
    ui.state.settings.logCompletedItems,
  );
}
    ...
    if (weekdays.length) {
      ...
      do {
        ...
        if (
          ...
          (weekDistance === 0 || weekDistance % interval === 0)
        )
          break;
      } while (true);
...
function cloneRecurringTask(template, scheduledFor) {
  return {
    ...
    scheduledFor,
    evening: false,
    reminderAt: template.repeat?.reminderTime
      ? `${scheduledFor}T${template.repeat.reminderTime}`
      : null,
    deadline: Number.isFinite(template.repeat?.deadlineOffset)
      ? addDays(scheduledFor, template.repeat.deadlineOffset)
      : template.deadline,
...
function materializeRecurringTasks() {
  ...
  let changed = false;
  const templates = ui.state.tasks.filter(
    (task) => task.repeat && !task.repeat.paused && task.status === "open",
  );
  for (const template of templates) {
    let next = template.repeat.nextDate || template.scheduledFor || today;
    let guard = 0;
    if (template.repeat.mode === "afterCompletion") {
      const pendingCopy = ui.state.tasks.some(
        (task) => task.repeatTemplateId === template.id && task.status === "open",
      );
    ...
    while (next <= today && guard < 60) {
      const exists = ui.state.tasks.some(
        (task) => task.repeatTemplateId === template.id && task.scheduledFor === next,
      );
      if (!exists) {
        ui.state.tasks.push(cloneRecurringTask(template, next));
        changed = true;
    ...
    if (template.repeat.nextDate !== next) {
      template.repeat.nextDate = next;
      changed = true;
    }
  }
...
function materializeRecurringProjects() {
  const today = localDay();
  let changed = false;
  for (const template of ui.state.projects.filter(
    (project) => project.repeat && !project.repeat.paused && project.status === "open",
  )) {
    let next = template.repeat.nextDate || template.scheduledFor || today;
    let guard = 0;
    if (template.repeat.mode === "afterCompletion") {
      const pendingCopy = ui.state.projects.some(
        (project) => project.repeatTemplateId === template.id && project.status === "open",
      );
      if (!pendingCopy && next <= today) {
        cloneProjectTemplate(template, next);
        changed = true;
      }
      continue;
    }
    while (next <= today && guard < 24) {
      if (
        !ui.state.projects.some(
          (project) => project.repeatTemplateId === template.id && project.scheduledFor === next,
        )
      ) {
        cloneProjectTemplate(template, next);
        changed = true;
...
function cloneProjectTemplate(template, scheduledFor) {
  ...
  ui.state.headings
    .filter((heading) => heading.projectId === template.id)
    .forEach((heading) => {
      const duplicate = { ...heading, id: uid("heading"), projectId: copy.id };
      headingMap.set(heading.id, duplicate.id);
      ui.state.headings.push(duplicate);
    });
  ui.state.tasks
    .filter((task) => task.projectId === template.id && task.status === "open")
    .forEach((task) =>
      ui.state.tasks.push({
        ...task,
        id: uid("task"),
        projectId: copy.id,
        headingId: headingMap.get(task.headingId) || null,
        repeat: null,
        ...
        completedAt: null,
        loggedAt: null,
        checklist: task.checklist.map((item) => ({ ...item, id: uid("check"), done: false })),
        createdAt: new Date().toISOString(),
...
export function parseNaturalTask(rawTitle) {
  return parseNaturalTaskCore(
    rawTitle,
    localDay(),
    Number(ui.state?.settings?.weekStartsOn ?? 1) === 0 ? 0 : 1,
  );
}

function parseNaturalDate(phrase, today = localDay()) {
  return parseNaturalDateCore(
    phrase,
    today,
    Number(ui.state?.settings?.weekStartsOn ?? 1) === 0 ? 0 : 1,
  );
...
function handleCaptureUrl() {
  ...
  const taskId =
    params.get("task") || (ui.state.tasks.some((task) => task.id === showId) ? showId : null);
  ...
  if (!requestedView && showId) {
    ...
    if (ui.state.projects.some((project) => project.id === showId)) {
      requestedView = "project";
    }
    if (ui.state.areas.some((area) => area.id === showId)) {
      requestedView = "area";
    }
    if (getKnownTags().some((tag) => tag.toLocaleLowerCase() === showId.toLocaleLowerCase())) {
      requestedView = "tag";
    }
  }
  ...
  if (taskId && snoozeMinutes > 0) {
    ...
    if (task) {
      ...
      task.reminderSentAt = null;
      scheduleSave();
      showToast(`Reminder snoozed for ${snoozeMinutes} minutes`);
    }
  }

  if (requestedView === "project") setView("project", showId);
  else if (requestedView === "area") setView("area", showId);
  else if (requestedView === "tag")
    setView(
      "tag",
      getKnownTags().find((tag) => tag.toLocaleLowerCase() === showId?.toLocaleLowerCase()) ||
        showId,
    );
  else if (
    requestedView &&
    ...
    const result = searchItems(params.get("query")).find((item) =>
      ["view", "heading"].includes(item.kind),
    );
    if (result?.kind === "view") setView(result.type, result.id || null);
    else if (result?.kind === "heading")
      setView(result.projectId ? "project" : "area", result.projectId || result.areaId);
  }
  ...
  if (title) {
    ...
    const notes = [params.get("notes"), sharedTitle ? sharedText : null, sharedUrl]
      .filter(Boolean)
      .join("\n");
    const listTarget = params.get("list-id") || params.get("list") || params.get("project");
    const project = ui.state.projects.find(
      (item) => item.id === listTarget || item.title.toLowerCase() === listTarget?.toLowerCase(),
    );
    const area = ui.state.areas.find(
      (item) => item.id === listTarget || item.title.toLowerCase() === listTarget?.toLowerCase(),
    );
    const requestedSpace = params.get("space-id") || params.get("space");
    const space = ui.state.spaces.find(
      (item) =>
        item.id === requestedSpace || item.title.toLowerCase() === requestedSpace?.toLowerCase(),
    );
    ...
    if (area && !project) {
      task.areaId = area.id;
      task.spaceId = area.spaceId || task.spaceId;
      task.bucket = "anytime";
    }
    const headingName = params.get("heading");
    const heading =
      headingName &&
      ui.state.headings.find(
        (item) =>
          !item.archived &&
          (item.id === headingName ||
            item.title.toLocaleLowerCase() === headingName.toLocaleLowerCase()) &&
          (!project || item.projectId === project.id) &&
          (!area || item.areaId === area.id || item.projectId === project?.id),
      );
    if (heading) {
      const parent = projectById(heading.projectId);
      const parentArea = areaById(heading.areaId);
      task.headingId = heading.id;
      task.projectId = heading.projectId || null;
      task.areaId = parent?.areaId || heading.areaId || null;
      task.spaceId = parent?.spaceId || parentArea?.spaceId || task.spaceId;
      task.bucket = task.bucket === "inbox" ? "anytime" : task.bucket;
    }
    const when = params.get("when");
    if (when) {
      if (when.toLowerCase() === "someday") {
        task.bucket = "someday";
        ...
        if (day) {
          ...
          task.bucket = day <= localDay() ? "today" : "upcoming";
          task.evening = when.toLowerCase() === "evening";
        }
      }
    }
    task.deadline = parseNaturalDate(params.get("deadline")) || task.deadline;
    if (params.get("tags"))
      task.tags = cleanTagList([...task.tags, ...registerTags(params.get("tags").split(","))]);
    const checklistItems = params.get("checklist-items") || params.get("checklist");
    if (checklistItems)
      task.checklist = checklistItems
        .split(/\r?\n/)
        .filter(Boolean)
        .map((item) => ({ id: uid("check"), title: item, done: false }));
    const requestedStatus =
      params.get("completed") === "true"
        ...
        : params.get("canceled") === "true"
          ? "canceled"
          : params.get("status");
    if (["completed", "canceled"].includes(requestedStatus)) {
      task.status = requestedStatus;
      task.completedAt = params.get("completion-date") || new Date().toISOString();
      task.loggedAt = null;
      applyLogbookPolicy();
    }
...
function bindStaticEvents() {
  ...
  $("#space-settings-button").addEventListener("click", () =>
    openAfterSidebarClose(openSpaceSettings),
  );
  ...
  $("#space-controls").addEventListener("click", (event) => {
    ...
    else if (event.target.closest("[data-space-overflow]"))
      openAfterSidebarClose(openSpaceSwitcher);
  });
  sidebar.addEventListener("dragstart", (event) => {
    ...
    event.dataTransfer.setData("text/plain", list.dataset.id);
  });
  ...
  content.addEventListener("input", (event) => {
    if (event.target.matches(".quick-add-input")) updateInlineTask(event.target);
  });
  content.addEventListener("focusout", (event) => {
    ...
    setTimeout(() => {
      ...
      finalizeInlineTask(input, {
        renderAfter: !document.activeElement?.matches(".quick-add-input"),
      });
    }, 0);
  });
  ...
  window.addEventListener("offline", () =>
    showToast("You are offline; changes will sync after reconnecting"),
  );
...
function contextTarget(element) {
  return (
    element?.closest?.(
      "[data-task-id], [data-project-card], [data-list-kind], [data-heading-id]",
    ) || null
  );
}
...
function bulkContextMenuItems(tasks) {
  ...
  const actionable = tasks.some(
    (task) => task.status === "open" && !task.repeat && !isTrashed(task),
  );
...
function openContextMenu(target, x, y) {
  ...
  if (taskRow) {
    ...
    if (kind === "task")
      items =
        (task.status === "open"
          ? menuButton("today", "Move to Today") +
            menuButton("someday", "Move to Someday") +
            menuButton("move", "Move…")
          : menuButton("restore-task", "Restore")) +
        menuButton("duplicate-task", "Duplicate") +
        menuButton("trash-task", "Move to Trash", true);
  } else if (headingRow) {
    kind = "heading";
    id = headingRow.dataset.headingId;
    items =
      menuButton("edit-heading", "Edit heading") +
      menuButton("new-heading-task", "New to-do here") +
      menuButton("delete-heading", "Delete heading", true);
  } else if (projectCard || list?.dataset.listKind === "project") {
    kind = "project";
    id = projectCard?.dataset.projectCard || list.dataset.id;
    const project = projectById(id);
    if (!project) return;
    items =
      menuButton("new-project-task", "New to-do") +
      menuButton("edit-project", "Project options…") +
      (isTrashed(project) ? "" : menuButton("delete-project", "Move list to Trash", true));
  } else if (list?.dataset.listKind === "area") {
    kind = "area";
    id = list.dataset.id;
    items =
      menuButton("new-area-task", "New standalone to-do") +
      menuButton("new-area-project", "New project") +
      menuButton("new-area-heading", "New heading") +
      menuButton("edit-area", "Area options…") +
      menuButton("remove-area", "Remove area", true);
  }
  if (!items) return;
  ...
  menu.addEventListener(
    "wa-after-show",
    () => menuItems.querySelector("wa-dropdown-item")?.focus(),
    { once: true },
  );
}
...
function handleContextMenuClick(event) {
  ...
  if (kind === "task") {
    ...
    if (action === "today" || action === "someday") {
      moveReminderToDate(task, action === "today" ? localDay() : null);
      task.bucket = action;
      task.scheduledFor = action === "today" ? localDay() : null;
      task.evening = false;
      scheduleSave();
      render();
    }
    ...
    if (action === "restore-task") {
      recordToDoIntent("menu", [task.id], { type: isTrashed(task) ? "restore" : "reopen" });
      task.status = "open";
    ...
    if (action === "duplicate-task") {
      ...
      ui.state.tasks.push(copy);
      scheduleSave();
      render();
    }
    if (action === "trash-task") {
      recordToDoIntent("menu", [task.id], { type: "trash" });
  ...
  if (kind === "heading") {
    ...
    if (action === "new-heading-task") {
      ...
      if (heading) {
        setView(heading.projectId ? "project" : "area", heading.projectId || heading.areaId);
        setTimeout(() => openQuickAdd($(`[data-quick-add="${id}"]`, content)), 0);
      }
    }
  ...
  if (kind === "area") {
    ...
    if (action === "new-area-heading") {
      setView("area", id);
      openHeadingModal();
    }
    if (action === "new-area-task") {
      setView("area", id);
      setTimeout(() => beginQuickAdd(), 0);
    }
...
function openSidebar() {
  ...
  if (matchMedia("(max-width: 820px)").matches) {
    ...
    showMobileDrawer(drawer, () =>
      finalizeSidebarClose({ restoreFocus: drawer.dataset.restoreFocus !== "false" }),
    );
  }
  syncSidebarAccessibility();
...
function headingBelongsTo(heading, type, id) {
  return type === "project"
    ? heading.projectId === id
    : type === "area"
      ? heading.areaId === id && !heading.projectId
      : false;
}

function headingsFor(type, id, includeArchived = false) {
  return ui.state.headings.filter(
    (heading) => headingBelongsTo(heading, type, id) && (includeArchived || !heading.archived),
  );
}
...
function projectAllowsTask(task) {
  ...
  return (
    !project ||
    (!project.repeat &&
      project.bucket !== "someday" &&
      (!project.scheduledFor || project.scheduledFor <= localDay()) &&
      (project.status === "open" || isCompletedButVisible(project)))
  );
}
function openTasks() {
  return ui.state.tasks.filter(
    (task) =>
      matchesActiveSpace(task) && task.status === "open" && !task.repeat && projectAllowsTask(task),
  );
}
function sourceTasks() {
  return ui.state.tasks.filter(
    (task) =>
      matchesActiveSpace(task) &&
      (task.status === "open" || isCompletedButVisible(task)) &&
      !task.repeat &&
      projectAllowsTask(task),
  );
}
function projectById(id) {
  return ui.state.projects.find((project) => project.id === id);
}
function areaById(id) {
  return ui.state.areas.find((area) => area.id === id);
}
function spaceById(id) {
  return ui.state?.spaces?.find((space) => space.id === id);
}
function itemSpaceId(item) {
  const project = projectById(item?.projectId);
  if (project) return areaById(project.areaId)?.spaceId ?? project.spaceId ?? null;
  const area = areaById(item?.areaId);
  if (area) return area.spaceId ?? null;
...
function spaceLabel(id) {
  return spaceById(id)?.title || "Unassigned";
}
function effectiveTags(task) {
  return [
    ...new Set([
      ...(areaById(task.areaId)?.tags || []),
      ...(projectById(task.projectId)?.tags || []),
      ...(task.tags || []),
    ]),
  ];
}
function effectiveProjectTags(project) {
  return [...new Set([...(areaById(project.areaId)?.tags || []), ...(project.tags || [])])];
}
function projectIsActive(project) {
  return (
    project &&
    matchesActiveSpace(project) &&
    (project.status === "open" || isCompletedButVisible(project)) &&
    !project.repeat
  );
}
function projectIsToday(project, today = localDay()) {
  return (
    projectIsActive(project) &&
    (project.bucket === "today" ||
      (project.scheduledFor && project.scheduledFor <= today) ||
      (project.deadline && project.deadline <= today))
  );
}

function upcomingTaskEntries(today, taskFilter) {
  const dated = ui.state.tasks
    .filter(
      (task) =>
        taskFilter(task) &&
  ...
  const repeating = ui.state.tasks
    .filter(
      (task) =>
        matchesActiveSpace(task) &&
        task.status === "open" &&
        task.repeat &&
        !task.repeat.paused &&
        task.repeat.nextDate > today &&
        !ui.state.tasks.some(
          (candidate) =>
            candidate.repeatTemplateId === task.id &&
            candidate.scheduledFor === task.repeat.nextDate,
        ),
    )
...
function upcomingProjectEntries(today) {
  ...
  const repeating = ui.state.projects
    .filter(
      (project) =>
        matchesActiveSpace(project) &&
        project.status === "open" &&
        project.repeat &&
        !project.repeat.paused &&
        project.repeat.nextDate > today &&
        !ui.state.projects.some(
          (candidate) =>
            candidate.repeatTemplateId === project.id &&
            candidate.scheduledFor === project.repeat.nextDate,
        ),
    )
...
function taskCount(type, id) {
  ...
  return openTasks().filter((task) => {
    ...
    if (type === "today")
      return (
        task.bucket === "today" ||
        (task.scheduledFor && task.scheduledFor <= today && task.bucket !== "someday") ||
        (task.deadline && task.deadline <= today)
      );
...
function projectProgress(projectId) {
  const tasks = ui.state.tasks.filter(
    (task) =>
...
function setActiveSpace(spaceId) {
  ...
  if (
    (currentArea && !matchesActiveSpace(currentArea)) ||
    (currentProject && !matchesActiveSpace(currentProject))
  )
    ui.view = { type: "today", id: null };
  if (ui.selectedTaskId && !matchesActiveSpace(currentTask()))
...
function renderSpaceControls() {
  ...
  if (active && !visible.some((space) => space.id === active.id))
    visible = visible.length < 2 ? [...visible, active] : [visible[0], active].filter(Boolean);
  ...
  if (scheduleButton)
    scheduleButton.setAttribute(
      "aria-label",
      enabled ? "Spaces and launch schedule, automatic selection on" : "Spaces and launch schedule",
    );
}
...
function renderSidebar() {
  ...
  const renderUserLists = (spaceId, title = "") => {
    const areas = ui.state.areas
      .filter((area) => area.spaceId === spaceId)
      .sort((a, b) => a.order - b.order);
    const areasHtml = areas
      .map((area) => {
        const areaProjects = ui.state.projects
          .filter(
            (project) =>
      ...
      .join("");
    const projects = ui.state.projects
      .filter(
        (project) =>
          project.spaceId === spaceId &&
          !project.areaId &&
    ...
    const projectsHtml = projects
      .map(
        (project) =>
          `<li><button class="nav-item ${ui.view.type === "project" && ui.view.id === project.id ? "active" : ""}" data-view="project" data-id="${project.id}" data-list-kind="project" draggable="true">${progressRing(projectProgress(project.id))}<span class="nav-title">${esc(project.title)}</span></button></li>`,
      )
      .join("");
  ...
  if (ui.activeSpaceId === "all") {
    userListsHtml = [...ui.state.spaces]
      .sort((a, b) => a.order - b.order)
      .map((space) => renderUserLists(space.id, space.title))
      .join("");
...
function viewDefinition() {
  ...
  const definitions = {
    inbox: {
      title: "Inbox",
      icon: "inbox",
      eyebrow: "Collect now, decide later",
      subtitle: "Unsorted thoughts and to-dos waiting for a home.",
      tasks: ui.state.tasks.filter((t) => taskFilter(t) && t.bucket === "inbox"),
    },
    today: {
      title: "Today",
      icon: "star",
      ...
      subtitle: "Your clear, focused plan for the day.",
      tasks: ui.state.tasks.filter(
        (t) =>
          taskFilter(t) &&
          (t.bucket === "today" ||
            (t.scheduledFor &&
      ...
      projects: ui.state.projects.filter((p) => projectIsToday(p, today)),
    },
    upcoming: {
      title: "Upcoming",
      icon: "calendar",
      eyebrow: "Plan ahead",
      subtitle: "Start dates, repeating plans, deadlines, and calendar events in one agenda.",
      tasks: upcomingTaskEntries(today, taskFilter),
      projects: upcomingProjectEntries(today),
    },
    anytime: {
      title: "Anytime",
      icon: "layers",
      eyebrow: "Available now",
      subtitle: "Everything active that you could make progress on.",
      tasks: ui.state.tasks.filter(
        (t) =>
          taskFilter(t) &&
          (t.bucket === "anytime" || t.bucket === "today") &&
    ...
    someday: {
      title: "Someday",
      icon: "archive",
      eyebrow: "Ideas for later",
      subtitle: "Possibilities worth keeping, without a commitment.",
      tasks: ui.state.tasks.filter(
        (t) =>
          matchesActiveSpace(t) &&
          (t.status === "open" || isCompletedButVisible(t)) &&
          !t.repeat &&
          t.bucket === "someday",
      ),
    ...
    logbook: {
      ...
      icon: "check",
      eyebrow: "Completed",
      subtitle: "A record of the progress you have made.",
      tasks: ui.state.tasks.filter((t) => matchesActiveSpace(t) && isLogged(t)),
      projects: ui.state.projects.filter((p) => matchesActiveSpace(p) && isLogged(p)),
    },
    trash: {
      title: "Trash",
      icon: "trash",
      eyebrow: "Discarded",
      subtitle: "Items moved here stay available for recovery.",
      tasks: ui.state.tasks.filter((t) => matchesActiveSpace(t) && isTrashed(t)),
      projects: ui.state.projects.filter((p) => matchesActiveSpace(p) && isTrashed(p)),
    },
    tomorrow: {
      title: "Tomorrow",
      icon: "calendar",
      eyebrow: formatDate(addDays(today, 1), { weekday: "long", month: "long", day: "numeric" }),
      subtitle: "Start dates, repeating plans, and deadlines for the next day.",
      tasks: upcomingTaskEntries(today, taskFilter).filter(
        (task) => task.agendaDay === addDays(today, 1),
      ),
      projects: upcomingProjectEntries(today).filter(
        (project) => project.agendaDay === addDays(today, 1),
      ),
    },
    deadlines: {
      title: "Deadlines",
      icon: "flag",
      eyebrow: "Commitments",
      subtitle: "Open items ordered by the date they must be finished.",
      tasks: ui.state.tasks.filter((t) => taskFilter(t) && t.deadline),
      projects: ui.state.projects
        .filter((p) => projectIsActive(p) && p.deadline)
    ...
    repeating: {
      ...
      icon: "repeat",
      eyebrow: "Templates",
      subtitle: "Routines that create fresh to-dos and projects on their schedule.",
      tasks: ui.state.tasks.filter((t) => matchesActiveSpace(t) && t.repeat),
      repeatingProjects: ui.state.projects.filter((p) => matchesActiveSpace(p) && p.repeat),
    },
  };
      ...
      icon: isLogged(project)
        ...
        : isTrashed(project)
          ...
          : project.repeat
            ? "repeat"
            : "list",
      eyebrow: project.repeat
        ? project.repeat.stopped
          ? "Stopped Repeating Project Template"
          : "Repeating Project Template"
        : project.status === "completed"
          ...
          : project.status === "canceled"
            ...
            : isTrashed(project)
              ? "In Trash"
              : areaById(project.areaId)?.title || "Project",
      subtitle:
        project.notes ||
        (project.repeat
          ? "These contents will be copied into future Project Occurrences."
          : "A focused set of steps toward one outcome."),
      deadline: project.deadline,
      tasks: ui.state.tasks.filter((t) => {
        ...
        const archivedHeading = ui.state.headings.some(
          (heading) => heading.id === t.headingId && heading.archived,
        );
  ...
  if (ui.view.type === "area") {
    ...
    return {
      ...
      icon: "circle",
      eyebrow: "Area",
      subtitle: "An ongoing part of life, with projects and standalone to-dos.",
      tasks: ui.state.tasks.filter((t) => t.areaId === area.id && !t.projectId && taskFilter(t)),
      projects: ui.state.projects.filter(
        (project) =>
          project.areaId === area.id &&
          projectIsActive(project) &&
          project.bucket !== "someday" &&
  ...
  if (ui.view.type === "tag") {
    return {
      ...
      icon: "tag",
      eyebrow: "Tag",
      subtitle: "Matching items from the active Space.",
      tasks: ui.state.tasks.filter((t) => taskFilter(t) && effectiveTags(t).includes(ui.view.id)),
      projects: ui.state.projects.filter(
        (p) => projectIsActive(p) && effectiveProjectTags(p).includes(ui.view.id),
      ),
    };
  }
  if (ui.view.type === "allProjects") {
    return {
      ...
      subtitle: "Every active outcome in this Space.",
      tasks: [],
      projects: ui.state.projects.filter(
        (p) => matchesActiveSpace(p) && p.status === "open" && !p.workspaceTemplate && !p.repeat,
      ),
    };
  }
  if (ui.view.type === "loggedProjects") {
    return {
      ...
      subtitle: "Finished outcomes and their progress history.",
      tasks: [],
      projects: ui.state.projects.filter((p) => matchesActiveSpace(p) && isLogged(p)),
    };
  }
...
function renderContent() {
  ...
  const visibleTasks = ui.activeTags.size
    ? view.tasks.filter((task) => matchesActiveTags(effectiveTags(task)))
    : view.tasks;
  const visibleProjects = ui.activeTags.size
    ? (view.projects || []).filter((project) => matchesActiveTags(effectiveProjectTags(project)))
    : view.projects || [];
  const sorted = [...visibleTasks].sort((a, b) => {
    ...
    if (ui.view.type === "repeating")
      return (a.repeat?.nextDate || "").localeCompare(b.repeat?.nextDate || "");
  ...
  const deadline = view.deadline
    ? ` · Deadline ${formatDate(view.deadline, { month: "short", day: "numeric" })}`
    : "";
  ...
  const calendar =
    ["today", "upcoming", "tomorrow"].includes(ui.view.type) && ui.state.settings.showCalendar
      ...
      : "";
  const projectSection = visibleProjects.length
    ? `<section class="section"><div class="section-header"><h2>Projects</h2></div>${renderProjectCards(visibleProjects)}</section>`
    : "";
  const sectionList = sections.some(
    (section) => section.tasks.length || section.heading || section.agenda,
  )
    ? `<div class="section-list">${sections.map(renderSection).join("")}</div>`
    : "";
  const repeatingManagement =
    ui.view.type === "repeating"
      ? renderRepeatingManagement(sorted, view.repeatingProjects || [])
      : "";
      ...
      onEnd: ({ movedIds, orderedIds, sectionKey }) => {
        ...
        if (ui.view.type === "upcoming")
          Object.assign(destination, {
            bucket: sectionKey <= localDay() ? "today" : "upcoming",
  ...
  if (
    ["project", "area"].includes(ui.view.type) &&
    !(ui.view.type === "project" && view.project?.repeat?.stopped)
  ) {
...
function renderRepeatingManagement(tasks, projects) {
  ...
  return `<div class="repeating-management">${groups
    .map(([state, title, description]) => {
      ...
      const taskRows = matchingTasks.length
        ? `<ul class="task-list">${matchingTasks.map(renderTask).join("")}</ul>`
        : "";
      const projectRows = matchingProjects.length
        ? `<div class="repeating-projects"><span class="repeating-kind-label">Projects</span>${renderProjectCards(matchingProjects)}</div>`
        : "";
...
function renderCalendarEvents(viewType) {
  const today = localDay();
  const events = ui.state.calendarEvents
    .filter((event) => {
    ...
    .slice(0, viewType === "upcoming" ? 8 : 12);
  if (!events.length) return "";
  return `<section class="calendar-strip" aria-label="Calendar events">${events
    .map((event) => {
      const day = event.start.slice(0, 10);
...
function renderProjectCards(projects) {
  if (!projects.length)
    return `<div class="empty-state">${icon("list")}<h2>No projects here</h2><p>Projects will appear here as their status changes.</p></div>`;
  return `<div class="project-card-list">${projects
    .map((project) => {
      const template = project.repeatTemplateId ? projectById(project.repeatTemplateId) : null;
      const repeatMeta = project.repeat
        ? `${esc(repeatLabel(project.repeat))} · `
        : template
          ? `${icon("repeat")} Occurrence of ${esc(template.title)} · `
          : "";
...
function buildSections(tasks) {
  if (["project", "area"].includes(ui.view.type)) {
    ...
    const archivedHeadings = viewHeadings
    ...
    const groups = groupBy(
      tasks.filter((task) => !archivedIds.has(task.headingId)),
      (task) => (activeIds.has(task.headingId) ? task.headingId : "no-heading"),
    );
    if (!groups.size) groups.set("no-heading", []);
    for (const heading of activeHeadings) {
      if (!groups.has(heading.id)) groups.set(heading.id, []);
    }
    const activeSections = [...groups.entries()]
      .sort(([a], [b]) => {
        if (a === "no-heading") return -1;
        if (b === "no-heading") return 1;
        return (
          (ui.state.headings.find((h) => h.id === a)?.order || 0) -
          (ui.state.headings.find((h) => h.id === b)?.order || 0)
        );
      })
      .map(([key, items]) => {
        ...
        items.sort((a, b) => {
          const rank = (task) =>
            task.bucket === "someday" ? 2 : task.scheduledFor && task.scheduledFor > today ? 1 : 0;
          return rank(a) - rank(b) || a.order - b.order;
        });
        return {
          key,
          title: heading?.title || (activeHeadings.length ? "To-dos" : ""),
          tasks: items,
          heading,
        };
      });
  ...
  if (ui.view.type === "upcoming") {
    ...
    return [...groups.entries()]
      ...
      .map(([day, items]) => ({
        key: day,
        title: relativeDateLabel(day),
        meta: formatDate(day, { month: "short", day: "numeric" }),
        tasks: items,
        agenda: true,
      }));
  }
  ...
  if (ui.view.type === "logbook") {
    ...
    return [...groups.entries()]
      ...
      .map(([day, items]) => ({
        key: day,
        title: day === localDay() ? "Today" : formatDate(day, { weekday: "long" }),
        meta: formatDate(day, { month: "short", day: "numeric" }),
        tasks: items,
      }));
  }
  if (ui.view.type === "today") {
    ...
    if (ui.state.settings.groupToday) {
      const addGrouped = (items, eveningMode = false) => {
        const groups = groupBy(
          ...
          (task) =>
            task.projectId ||
            task.areaId ||
            (ui.activeSpaceId === "all" ? task.spaceId : null) ||
            "Standalone",
        );
        for (const [key, group] of groups) {
          const parent = projectById(key) || areaById(key) || spaceById(key);
          result.push({
            key: `${eveningMode ? "evening-" : ""}${key}`,
            title: eveningMode ? "This Evening" : parent?.title || "To-dos",
            meta: eveningMode && parent ? parent.title : "",
            tasks: group,
            symbol: eveningMode ? "☾" : "",
          });
        }
      ...
      if (evening.length)
        result.push({ key: "evening", title: "This Evening", tasks: evening, symbol: "☾" });
    }
    return result;
  }
  if (["anytime", "someday"].includes(ui.view.type)) {
    ...
    const rank = (key) => {
      ...
      if (project)
        return (areaById(project.areaId)?.order ?? 1000) * 1000 + (project.order ?? 0) + 1;
    ...
    return [...groups.entries()]
      ...
      .map(([key, items]) => ({
        key,
        title: projectById(key)?.title || areaById(key)?.title || spaceById(key)?.title || "To-dos",
        tasks: items,
      }));
  }
...
function renderSection(section) {
  const stoppedProjectTemplate =
    ui.view.type === "project" && projectById(ui.view.id)?.repeat?.stopped;
  const headingActions =
    section.heading && !stoppedProjectTemplate
      ? `<span class="heading-actions">${section.archived ? `<button class="button heading-restore" data-action="restore-heading" data-heading-id="${section.heading.id}">Restore</button>` : `<button class="icon-button" data-action="heading-menu" data-heading-id="${section.heading.id}" aria-label="Heading options">${icon("more")}</button>`}</span>`
      : "";
  const title = section.title
    ? `<div class="section-header ${section.heading ? "heading-header" : ""} ${section.archived ? "archived-heading-header" : ""}" ${section.heading && !section.archived ? `data-heading-id="${section.heading.id}"` : ""}>${section.symbol ? `<span class="section-symbol">${section.symbol}</span>` : ""}<h2>${esc(section.title)}</h2>${section.meta ? `<span class="section-meta">${esc(section.meta)}</span>` : ""}${headingActions}</div>`
    : "";
  const emptyAgenda =
    section.agenda && !section.tasks.length ? '<p class="agenda-empty">No plans</p>' : "";
...
function renderTask(task) {
  ...
  const meta = [];
  if (project && ui.view.type !== "project")
    meta.push(`<span class="meta-item">${icon("list")}${esc(project.title)}</span>`);
  else if (area && !["area", "project"].includes(ui.view.type))
    meta.push(`<span class="meta-item">${esc(area.title)}</span>`);
  if (task.deadline)
    meta.push(
      `<span class="meta-item deadline">${icon("flag")} ${deadlineLabel(task.deadline)}</span>`,
    );
  if (task.reminderAt)
    meta.push(
      `<span class="meta-item reminder">${icon("bell")} ${formatReminderTime(task.reminderAt)}</span>`,
    );
  if (task.bucket === "someday" && ["project", "area"].includes(ui.view.type))
    meta.push(`<span class="meta-item">${icon("archive")} Someday</span>`);
  else if (task.scheduledFor && !["today", "upcoming", "tomorrow"].includes(ui.view.type))
    meta.push(
      `<span class="meta-item ${task.scheduledFor < localDay() ? "past-date" : ""}">${icon("calendar")} ${esc(scheduleDateLabel(task.scheduledFor))}</span>`,
    );
  if (task.repeat)
    meta.push(`<span class="meta-item">${icon("repeat")} ${repeatLabel(task.repeat)}</span>`);
  else if (projectTemplateItem)
    meta.push(`<span class="meta-item">${icon("repeat")} Template item</span>`);
  else if (task.repeatTemplateId) {
    const template = ui.state.tasks.find((item) => item.id === task.repeatTemplateId);
    meta.push(
      `<span class="meta-item repeat-occurrence">${icon("repeat")} Occurrence${template ? ` of ${esc(template.title)}` : ""}</span>`,
    );
  }
  ...
  if (task.checklist?.length)
    meta.push(
      `<span class="meta-item">${icon("list")} ${task.checklist.filter((i) => i.done).length}/${task.checklist.length}</span>`,
    );
  if (task.tags?.length)
    meta.push(
      ...task.tags
        .slice(0, 2)
        .map((tag) => `<span class="meta-item"><i class="tag-dot"></i>${esc(tag)}</span>`),
    );
  const star =
    ui.view.type === "anytime" && task.bucket === "today" ? icon("star", "today-star") : "";
...
function handleTaskGestureMove(event) {
  ...
  if (!gesture.horizontal) {
    ...
    gesture.horizontal = true;
    gesture.row.classList.add("task-swiping");
  }
  event.preventDefault();
...
function handleTaskGestureEnd(event) {
  finishTaskGesture(event);
}
function handleTaskGestureCancel(event) {
  finishTaskGesture(event, true);
}

function selectedTasks() {
  return ui.state.tasks.filter(
    (task) => ui.selectedTaskIds.has(task.id) && toDoRowCapabilities(task).selectable,
  );
}
...
function draggedTasks() {
  ...
  if (ui.selectedTaskIds.size > 1 && ui.selectedTaskIds.has(primary.id)) {
    return ui.state.tasks
      .filter((task) => ui.selectedTaskIds.has(task.id))
      .sort((a, b) => a.order - b.order);
  }
  return [primary];
}
  ...
  if (extendRange && ui.selectionAnchorId) {
    ...
    if (anchor >= 0 && target >= 0) {
      visibleIds
        ...
        .forEach((id) => ui.selectedTaskIds.add(id));
    }
  } else if (ui.selectedTaskIds.has(taskId)) {
    ui.selectedTaskIds.delete(taskId);
    if (ui.selectionAnchorId === taskId)
      ui.selectionAnchorId = [...ui.selectedTaskIds].at(-1) || null;
  } else {
...
function handleBulkAction(action) {
  ...
  if (action === "move") {
    openMoveTaskModal(actionable);
    return;
  }
  if (action === "tags") {
    openBulkTagsModal(actionable);
    return;
  }
  if (action === "today" || action === "someday") {
    ...
    clearTaskSelection();
    showToast(
      `${actionable.length} to-do${actionable.length === 1 ? "" : "s"} moved to ${action === "today" ? "Today" : "Someday"}`,
    );
    return;
  }
  if (action === "complete" || action === "cancel") {
    ...
    recordToDoIntent(
      ...
      { type: action === "complete" ? "complete" : "cancel" },
    );
    actionable.forEach((task) => {
    ...
    clearTaskSelection();
    showToast(
      `${actionable.length} to-do${actionable.length === 1 ? "" : "s"} ${action === "complete" ? "completed" : "canceled"}`,
    );
    return;
  }
  if (action === "restore") {
    const directRestores = tasks
      .filter((task) => !isTrashed(projectById(task.projectId)))
      .map((task) => task.id);
    recordToDoIntent("bulk", directRestores, { type: "restore" });
    tasks.forEach((task) => restoreTrashedTask(task));
    scheduleSave();
    clearTaskSelection();
    showToast(`${tasks.length} to-do${tasks.length === 1 ? "" : "s"} restored`);
    return;
  }
  if (action === "trash") {
    ...
    scheduleSave();
    clearTaskSelection();
    showToast(`${tasks.length} to-do${tasks.length === 1 ? "" : "s"} moved to Trash`);
  }
}
  ...
  renderPreact(
    h(BulkTagsDialog, {
      ...
      onSubmit: (states, additions) => {
        ...
        closeModal();
        clearTaskSelection();
        showToast(`Tags updated on ${tasks.length} to-do${tasks.length === 1 ? "" : "s"}`);
      },
    }),
    $("#modal-root"),
  );
...
function canQuickAdd() {
  return (
    !["logbook", "trash", "upcoming", "repeating", "allProjects", "loggedProjects"].includes(
      ui.view.type,
    ) &&
    !(
      ui.view.type === "project" &&
      (projectById(ui.view.id)?.status !== "open" || projectById(ui.view.id)?.repeat?.stopped)
    )
  );
}
...
function updateInlineTask(input) {
  ...
  if (!task) {
    task = createTaskFromParsed(
      ...
      { sectionKey, view: { ...ui.view }, select: false, render: false },
    );
...
function finalizeInlineTask(input, { renderAfter = true, cancel = false } = {}) {
  ...
  if (task && (cancel || !input.value.trim())) {
    ...
    if (ui.syncedState?.tasks?.some((item) => item.id === task.id))
      rememberPendingDeletion(task.id);
    else {
  ...
  if (renderAfter) {
    ...
    renderContent();
    if (task && !cancel && input.value.trim())
      setTimeout(() => $(`[data-task-id="${task.id}"] .task-main`, content)?.focus(), 20);
  }
  return task;
}
...
function repeatLabel(repeat) {
  ...
  const every =
    Number(repeat.interval) > 1
      ? `Every ${repeat.interval} ${repeat.frequency.replace("daily", "days").replace("weekly", "weeks").replace("monthly", "months").replace("yearly", "years")}`
...
function renderEmpty(view) {
  const messages = {
    today: [
      "A clear day",
      "Nothing is pressing. Choose something from Anytime, or enjoy the space.",
    ],
    ...
    someday: [
      ...
      "Ideas without a date can wait here without getting in the way.",
    ],
  ...
  const [title, text] = messages[ui.view.type] || [
    "A quiet list",
    `Add the first to-do to ${view.title}.`,
  ];
...
function scheduleDateLabel(day) {
  ...
  if (day < today)
    return formatDate(day, {
      ...
      day: "numeric",
      year:
        new Date(`${day}T12:00:00`).getFullYear() === new Date().getFullYear()
          ? undefined
          : "numeric",
    });
  if (day === addDays(today, 1)) return "Tomorrow";
...
function handleContentClick(event) {
  ...
  if (action === "empty-trash") {
    confirmAction(
      "Empty Trash?",
      "Every discarded to-do, project, and heading will be permanently deleted. This cannot be undone.",
      "Empty Trash",
      () => {
        const projectIds = new Set(
          ui.state.projects
            .filter((project) => matchesActiveSpace(project) && isTrashed(project))
            .map((project) => project.id),
        );
        ui.state.tasks = ui.state.tasks.filter(
          (task) =>
            !(matchesActiveSpace(task) && isTrashed(task)) && !projectIds.has(task.projectId),
        );
        ui.state.headings = ui.state.headings.filter(
          (heading) => !projectIds.has(heading.projectId),
        );
  ...
  if (action === "heading-menu")
    openHeadingModal(event.target.closest("[data-heading-id]").dataset.headingId);
  if (action === "restore-heading") {
    const heading = ui.state.headings.find(
      (item) => item.id === event.target.closest("[data-heading-id]").dataset.headingId,
    );
    if (heading) {
      heading.archived = false;
      scheduleSave();
      renderContent();
      showToast("Heading restored");
    }
  }
...
function handleDragStart(event) {
  ...
  if (heading && !event.target.closest("button")) {
    ui.draggedHeadingId = heading.dataset.headingId;
    event.dataTransfer.effectAllowed = "move";
    event.dataTransfer.setData("text/plain", ui.draggedHeadingId);
    return;
  }
  const row = event.target.closest("[data-task-id]");
  if (!row) return;
...
function handleDrop(event) {
  ...
  if (ui.draggingMagicAdd) {
    event.preventDefault();
    ui.draggingMagicAdd = false;
    if (ui.view.type === "project" && event.clientX < content.getBoundingClientRect().left + 78)
      openHeadingModal();
    else
      openQuickAdd(targetSection?.querySelector(".quick-add-row") || $(".quick-add-row", content));
    return;
  }
  ...
  if (ui.draggedHeadingId && targetSection && ["project", "area"].includes(ui.view.type)) {
    ...
    const targetHeading = ui.state.headings.find(
      (item) => item.id === targetSection.dataset.section,
    );
    if (heading && targetHeading && heading.id !== targetHeading.id) {
      heading.order =
        targetHeading.order +
        (event.clientY >
        targetSection.getBoundingClientRect().top + targetSection.getBoundingClientRect().height / 2
          ? 0.5
          : -0.5);
      headingsFor(ui.view.type, ui.view.id)
        .sort((a, b) => a.order - b.order)
        .forEach((item, index) => {
          item.order = index;
        });
  ...
  if (!source || (!target && !targetSection) || sources.some((item) => item.id === target?.id))
    return;
  event.preventDefault();
  const destinationKey =
    targetSection?.dataset.section || targetRow?.closest("[data-section]")?.dataset.section;
  if (ui.view.type === "upcoming" && destinationKey) {
    sources.forEach((item) => {
      if (item.repeat) {
        item.repeat.nextDate = destinationKey;
        item.scheduledFor = destinationKey;
      } else {
        moveReminderToDate(item, destinationKey);
        item.scheduledFor = destinationKey;
        item.bucket = destinationKey <= localDay() ? "today" : "upcoming";
        item.evening = false;
      }
    });
  }
  if (ui.view.type === "today") {
    ...
    sources.forEach((item) => {
      item.bucket = "today";
      item.scheduledFor = localDay();
      item.evening = evening;
      moveReminderToDate(item, item.scheduledFor);
    });
  }
  if (target) {
    ...
    if (["project", "area"].includes(ui.view.type))
      sources.forEach((item) => {
        item.headingId = sectionKey === "no-heading" ? null : sectionKey;
      });
    const sectionTasks = ui.state.tasks.filter(
      (task) => task.headingId === source.headingId && !sources.some((item) => item.id === task.id),
    );
...
function handleDragEnd() {
  ...
  ui.draggedHeadingId = null;
  ui.draggingMagicAdd = false;
  $$(".task-row.dragging, .task-row.drag-over", content).forEach((item) =>
...
function handleSidebarDrop(event) {
  ...
  if (destination && ui.draggingMagicAdd) {
    ...
    if (type === "area" && id) openNewListModal({ type: "project", areaId: id });
      ...
      setView(
        ["project", "area", "inbox", "today", "anytime", "someday"].includes(type) ? type : "inbox",
        id,
      );
  ...
  if (destination && ui.draggedList) {
    event.preventDefault();
    const source =
      ui.draggedList.kind === "area" ? areaById(ui.draggedList.id) : projectById(ui.draggedList.id);
    const targetKind = destination.dataset.listKind;
    const target =
      targetKind === "area"
        ? areaById(destination.dataset.id)
        : targetKind === "project"
          ? projectById(destination.dataset.id)
          : null;
    if (!source || source === target) return;
    if (ui.draggedList.kind === "project" && targetKind === "area") {
      source.areaId = target.id;
      source.spaceId = target.spaceId || source.spaceId;
      source.order =
        Math.max(
          0,
          ...ui.state.projects
            .filter((item) => item.areaId === target.id)
            .map((item) => item.order || 0),
        ) + 1;
    } else if (ui.draggedList.kind === "project" && targetKind === "project") {
      source.areaId = target.areaId || null;
      source.spaceId = target.spaceId || source.spaceId;
      source.order = (target.order || 0) + 0.5;
    } else if (ui.draggedList.kind === "area" && targetKind === "area")
      source.order = (target.order || 0) + 0.5;
    else return;
    ...
    if (ui.draggedList.kind === "project")
      ui.state.tasks
        .filter((item) => item.projectId === source.id)
        .forEach((item) => {
          item.areaId = source.areaId || null;
          item.spaceId = source.spaceId || item.spaceId;
        });
    ui.draggedList = null;
    scheduleSave();
    render();
  ...
  const type = destination.dataset.view;
  const id = destination.dataset.id || null;
  if (!["inbox", "today", "upcoming", "anytime", "someday", "area", "project"].includes(type))
    return;
  tasks.forEach((item) => {
    if (type === "inbox") {
      item.bucket = "inbox";
      ...
      item.scheduledFor =
        item.scheduledFor > localDay() ? item.scheduledFor : addDays(localDay(), 1);
      item.evening = false;
      moveReminderToDate(item, item.scheduledFor);
      ...
      item.headingId = null;
      item.areaId = id;
      item.spaceId = area?.spaceId || item.spaceId;
      if (item.bucket === "inbox") item.bucket = "anytime";
    } else if (type === "project" && id) {
      const project = projectById(id);
      item.projectId = id;
      item.headingId = null;
      item.areaId = project?.areaId || null;
      item.spaceId = project?.spaceId || item.spaceId;
      if (item.bucket === "inbox") item.bucket = "anytime";
    }
  });
  ui.draggedTaskId = null;
...
function openQuickAdd(row) {
  ...
  if (existing && existing !== $(".quick-add-input", row)) {
    ...
    renderContent();
    row = $$("[data-quick-add]", content).find(
      (candidate) => candidate.dataset.quickAdd === sectionKey,
    );
    if (!row) return;
  }
  row.hidden = false;
...
function createTaskFromParsed(parsed, options = {}) {
  ...
  const task = {
    ...
    headingId: null,
    areaId: null,
    spaceId: options.spaceId || currentCreationSpaceId(),
    tags: registerTags(parsed.tags || []),
    checklist: [],
    repeat: null,
    createdAt: new Date().toISOString(),
    completedAt: null,
  ...
  if (options.useCurrentView !== false) {
    if (targetView.type === "today") {
      ...
      if (project) {
        ...
        task.areaId = project.areaId || null;
        task.spaceId = project.spaceId || task.spaceId;
      } else if (area) {
        task.areaId = area.id;
        task.spaceId = area.spaceId || task.spaceId;
      }
    ...
    else if (targetView.type === "tomorrow") {
      task.bucket = "upcoming";
      task.scheduledFor = addDays(today, 1);
    } else if (targetView.type === "deadlines") {
      task.bucket = "anytime";
      task.deadline = today;
    } else if (targetView.type === "tag") {
      task.bucket = "anytime";
      task.tags = [...new Set([...task.tags, targetView.id])];
      ...
      task.spaceId = project?.spaceId || task.spaceId;
      task.headingId =
        project && options.sectionKey && options.sectionKey !== "no-heading"
          ? options.sectionKey
          : null;
      if (project?.repeat) {
        task.workspaceTemplate = true;
        task.workspaceTemplateId = project.id;
        task.workspaceBlueprintKey = uid("repeat-blueprint");
      }
    } else if (targetView.type === "area") {
      const area = areaById(targetView.id);
      task.bucket = "anytime";
      task.areaId = targetView.id;
      task.spaceId = area?.spaceId || task.spaceId;
      task.headingId =
        options.sectionKey && options.sectionKey !== "no-heading" ? options.sectionKey : null;
    }
  }
  if (task.projectId && task.bucket === "inbox" && options.useCurrentView === false)
    task.bucket = "anytime";
...
function toggleTask(taskId, row, source = "row") {
  ...
  if (isTrashed(task)) {
    ...
    scheduleSave();
    render();
    showToast(restoredProject ? "Project restored" : "To-do restored");
    return;
  }
  if (task.repeat) {
    selectTask(task.id);
    showToast("Edit or pause this repeating template in its details");
    return;
  }
  if (task.workspaceTemplateId) {
    selectTask(task.id);
    showToast("Edit this item as part of its Repeating Project Template");
    return;
  }
  if (task.status === "open") {
    ...
    ui.lastCompleted = {
      ...
      completedWithProjectId: task.completedWithProjectId || null,
    };
...
function undoComplete() {
  ...
  if (task) {
    ...
    task.status = ui.lastCompleted.status;
    task.completedAt = ui.lastCompleted.completedAt;
    task.loggedAt = ui.lastCompleted.loggedAt;
    task.completedWithProjectId = ui.lastCompleted.completedWithProjectId;
...
function restoreTrashedTask(task) {
  ...
  if (restoredProject) {
    ...
    ui.state.tasks
      .filter((item) => item.projectId === parent.id && isTrashed(item))
      .forEach((item) => {
...
function mountInspectorDrawer() {
  ...
  $('[data-inspector-action="close"]', inspector)?.addEventListener(
    "click",
    () => drawer.removeAttribute("open"),
    { once: true },
  );
...
function renderInspector(force = false) {
  ...
  if (!force && previousPane?.dataset.taskId === task.id) {
    syncSidebarAccessibility();
    return;
  }
  ...
  const spaceOptions = ui.state.spaces
    .map(
      (space) =>
        `<option value="${space.id}" ${itemSpaceId(task) === space.id ? "selected" : ""}>${esc(space.title)}</option>`,
    )
    .join("");
  const availableProjects = task.workspaceTemplateId
    ? ui.state.projects.filter((project) => project.id === task.workspaceTemplateId)
    : ui.state.projects.filter(
        (project) => project.status === "open" && !project.workspaceTemplate && !project.repeat,
      );
  const projectOptions = [
    `<option value="">No project</option>`,
    ...availableProjects.map(
      (p) =>
        `<option value="${p.id}" ${task.projectId === p.id ? "selected" : ""}>${esc(spaceLabel(p.spaceId))} › ${esc(p.title)}${p.repeat ? " (Repeating Template)" : ""}</option>`,
    ),
  ].join("");
    ...
    ...ui.state.headings
      .filter(
        (h) =>
          !h.archived &&
          (task.projectId
            ? h.projectId === task.projectId
            : h.areaId === task.areaId && !h.projectId),
      )
      .map(
        (h) =>
          `<option value="${h.id}" ${task.headingId === h.id ? "selected" : ""}>${esc(h.title)}</option>`,
      ),
  ].join("");
  ...
  const occurrenceTemplate = task.repeatTemplateId
    ? ui.state.tasks.find((item) => item.id === task.repeatTemplateId)
    : null;
  const occurrenceDate = task.scheduledFor
    ? formatDate(task.scheduledFor, { weekday: "long", month: "long", day: "numeric" })
    : "its scheduled date";
  inspector.innerHTML = `<div class="inspector-scroll" data-task-id="${esc(task.id)}">
    ...
    ${
      !repeat
        ? `<div class="detail-group"><span class="detail-label">When</span><div class="schedule-chips">
      ${[
        ["inbox", "Inbox"],
        ["today", "Today"],
        ...
        .map(
          ([value, label]) =>
            `<button class="chip ${schedule === value ? "active" : ""}" data-schedule="${value}">${label}</button>`,
        )
        .join("")}
...
function renderMarkdown(markdown = "") {
  ...
  for (const line of lines) {
    ...
    if (nextType) {
      ...
      if (taskItem)
        output.push(
          `<li class="markdown-task ${taskItem[1].toLowerCase() === "x" ? "done" : taskItem[1] === "~" ? "cancelled" : ""}"><span>${taskItem[1].toLowerCase() === "x" ? "✓" : taskItem[1] === "~" ? "—" : "○"}</span>${inlineMarkdown(taskItem[2])}</li>`,
        );
...
function currentTask() {
  return ui.state.tasks.find((item) => item.id === ui.selectedTaskId);
}
...
function noteFindLabel(task) {
  ...
  return matches.length
    ? `${Math.min(ui.noteFindIndex + 1, matches.length)} of ${matches.length}`
    : ui.noteFindQuery
      ? "No matches"
      : "";
}
...
function updateNoteFind(direction = 0, focusNote = false) {
  ...
  if (!matches.length) ui.noteFindIndex = 0;
    ...
    ui.noteFindIndex = (ui.noteFindIndex + direction + matches.length) % matches.length;
...
function handleInspectorChange(event) {
  ...
  const field = event.target.dataset.field;
  if (["deadline", "scheduledFor", "reminderAt"].includes(field))
    task[field] = event.target.value || null;
  if (field === "scheduledFor") {
    moveReminderToDate(task, event.target.value || null);
    task.bucket = event.target.value
      ? event.target.value <= localDay()
        ? "today"
        : "upcoming"
      : "anytime";
  }
  ...
  if (field === "deadline" && task.repeat)
    task.repeat.deadlineOffset = dayDistance(task.repeat.nextDate, task.deadline);
  if (field === "projectId") {
    ...
    if (
      !project ||
      !ui.state.headings.some(
        (heading) => heading.id === task.headingId && heading.projectId === project.id,
      )
    )
      task.headingId = null;
  }
  if (field === "spaceId") {
    ...
    if (projectById(task.projectId)?.spaceId !== task.spaceId) {
      task.projectId = null;
      task.areaId = null;
      task.headingId = null;
    } else if (areaById(task.areaId)?.spaceId !== task.spaceId) {
      task.areaId = null;
      task.headingId = null;
    }
  }
  ...
  if (repeatField && task.repeat) {
    ...
    else if (repeatField === "interval")
      task.repeat.interval = Math.max(1, Number(event.target.value) || 1);
    ...
    if (repeatField === "nextDate") {
      ...
      if (Number.isFinite(task.repeat.deadlineOffset))
        task.deadline = addDays(event.target.value, task.repeat.deadlineOffset);
    }
  }
  const checkRow = event.target.closest("[data-check-id]");
  ...
  renderSidebar();
  renderContent();
  if (field === "projectId" || field === "spaceId" || repeatField === "frequency")
    renderInspector(true);
}
...
function handleInspectorClick(event) {
  ...
  const closestTarget = (selector) => {
    ...
    return (
      event.composedPath?.().find((item) => item instanceof Element && item.matches(selector)) ||
      null
    );
  };
  ...
  if (action === "open-repeat-template" && occurrenceTemplate) {
    selectTask(occurrenceTemplate.id);
    return;
  }
  if (action === "trash") {
    if (task.repeatTemplateId) {
  ...
  if (action === "skip-occurrence") {
    ...
    task.status = "canceled";
    task.completedAt = new Date().toISOString();
    task.loggedAt = task.completedAt;
    task.completedWithProjectId = null;
    if (template?.repeat?.mode === "afterCompletion")
      template.repeat.nextDate = nextRepeatDate(localDay(), template.repeat);
    materializeRecurringTasks();
  ...
  if (action === "delete-repeat-template") {
    confirmAction(
      "Delete this Repeating Template forever?",
      "Its existing Occurrences remain in your history. This cannot be undone.",
      "Delete forever",
      () => {
        ui.workspaceChanges.push({
          type: "deleteRepeatingTemplate",
          id: task.id,
          confirmation: "DELETE REPEATING TEMPLATE",
        });
        ui.state.tasks = ui.state.tasks.filter((item) => item.id !== task.id);
        scheduleSave();
        closeInspector();
        render();
        showToast("Repeating Template deleted");
      },
    );
  }
  if (action === "remove-template-item" && task.workspaceTemplateId) {
    confirmAction(
      "Remove this item from the Repeating Project Template?",
      "Existing Project Occurrences keep their own copy. Future Occurrences will not include it.",
      "Remove item",
      () => {
        ui.state.tasks = ui.state.tasks.filter((item) => item.id !== task.id);
        scheduleSave();
        closeInspector();
        render();
        showToast("Item removed from Repeating Project Template");
      },
    );
  }
  if (action === "restore") {
  ...
  if (action === "delete-forever") {
    confirmAction("Delete this to-do forever?", "This cannot be undone.", "Delete forever", () => {
      ...
      scheduleSave();
      closeInspector();
      showToast("To-do permanently deleted");
    });
  }
  if (action === "duplicate") {
    const copy = {
      ...
      order: Date.now(),
      checklist: task.checklist.map((item) => ({ ...item, id: uid("check") })),
      repeat: task.repeat ? { ...task.repeat, weekdays: [...(task.repeat.weekdays || [])] } : null,
    };
    ui.state.tasks.push(copy);
    ui.selectedTaskId = copy.id;
    scheduleSave();
    render();
    showToast("To-do duplicated");
  }
  if (action === "move") {
  ...
  if (action === "copy-link") {
    void copyTaskLink(task);
    return;
  }
  if (action === "markdown") {
    ui.markdownPreview = !ui.markdownPreview;
    renderInspector(true);
  }
  if (action === "find-text") {
  ...
  if (action === "edit-repeat") {
    openRepeatingTemplateEditor(task);
    return;
  }
  if (action === "start-repeat") {
    openRepeatingTemplateEditor(task);
    return;
  }
  if (action === "stop-repeat" && task.repeat) {
  ...
  if (action === "add-check") {
    task.checklist.push({ id: uid("check"), title: "", done: false });
    scheduleSave();
    renderInspector(true);
    $$('.checklist-item input[type="text"]', inspector).at(-1)?.focus();
  }
  if (action === "move-check-up" || action === "move-check-down") {
    ...
    if (from >= 0 && to >= 0 && to < task.checklist.length) {
      ...
      task.checklist.splice(to, 0, moved);
      scheduleSave();
      renderInspector(true);
      renderContent();
      setTimeout(
        () => $(`[data-check-id="${id}"] [data-inspector-action="${action}"]`, inspector)?.focus(),
        20,
      );
    }
  }
...
function openRepeatingTemplateEditor(task) {
  ...
  const initial = task.repeat
    ? { ...task.repeat, weekdays: [...(task.repeat.weekdays || [])] }
    : {
        mode: "fixed",
        frequency: "weekly",
        interval: 1,
        weekdays: [Number.isNaN(nextWeekday) ? 1 : nextWeekday],
        nextDate,
        reminderTime: task.reminderAt?.slice(11, 16) || "",
        deadlineOffset: dayDistance(nextDate, task.deadline),
        paused: false,
      };
  modalReturnFocus = document.activeElement instanceof HTMLElement ? document.activeElement : null;
  modalUsesPreact = true;
  renderPreact(
    h(RepeatingTemplateDialog, {
      title: task.title,
      value: initial,
      ...
      onSave: (repeat) => {
        ...
        if (Number.isFinite(repeat.deadlineOffset))
          task.deadline = addDays(repeat.nextDate, repeat.deadlineOffset);
        scheduleSave();
        closeModal();
        render(true);
        showToast(savedRepeat.paused ? "Repeating schedule paused" : "Repeating schedule saved");
      },
      onStop: () => {
        if (!task.repeat) {
          closeModal();
        ...
        closeModal();
        render(true);
        showToast("Repeating schedule stopped");
      },
    }),
    $("#modal-root"),
  );
  ...
  renderPreact(
    h(RepeatingTemplateDialog, {
      ...
      onSave: (repeat) => {
        ...
        if (Number.isFinite(repeat.deadlineOffset))
          project.deadline = addDays(repeat.nextDate, repeat.deadlineOffset);
        scheduleSave();
        closeModal();
        render(true);
        showToast(repeat.paused ? "Repeating Project paused" : "Repeating Project schedule saved");
      },
      onStop: () => {
        ...
        closeModal();
        render(true);
        showToast("Repeating Project stopped");
      },
      onDelete: () => {
        closeModal();
        confirmAction(
          "Delete this Repeating Project Template forever?",
          "Existing Project Occurrences remain in history. This cannot be undone.",
          "Delete forever",
          () => {
            ui.workspaceChanges.push({
              type: "deleteRepeatingTemplate",
            ...
            ui.state.headings = ui.state.headings.filter(
              (item) => item.workspaceTemplateId !== project.id,
            );
            ui.state.tasks = ui.state.tasks.filter(
              (item) => item.workspaceTemplateId !== project.id,
            );
            scheduleSave();
            setView("repeating");
            showToast("Repeating Project Template deleted");
          },
        );
      },
    }),
...
function handleInspectorKeydown(event) {
  ...
  const task = currentTask();
  if (!task) return;
  const picker = createTagInPicker(
    $('[data-create-tag="task"]', input.closest("[data-tag-picker]")),
  );
  task.tags = selectedPickerTags(picker);
  scheduleSave();
  renderSidebar();
...
function taskAsText(task) {
  ...
  if (task.checklist?.length)
    lines.push("", ...task.checklist.map((item) => `${item.done ? "✓" : "○"} ${item.title}`));
...
async function shareTask(task) {
  ...
  try {
    if (navigator.share) await navigator.share(data);
      ...
      showToast("To-do copied for sharing");
    }
  } catch (error) {
...
function openMoveTaskModal(taskOrTasks) {
  ...
  const destinations = [
    ...
    ...ui.state.areas.flatMap((area) => [
      ...
      ...ui.state.headings
        .filter((heading) => heading.areaId === area.id && !heading.projectId && !heading.archived)
        .map((heading) => ({
          value: `heading:${heading.id}`,
          label: `${spaceLabel(area.spaceId)} › ${area.title} › ${heading.title}`,
        })),
    ]),
    ...ui.state.projects
      .filter((project) => project.status === "open" && !project.repeat)
      .flatMap((project) => [
        {
          value: `project:${project.id}`,
          label: `${spaceLabel(project.spaceId)} › ${project.title} (Project)`,
        },
        ...ui.state.headings
          .filter((heading) => heading.projectId === project.id && !heading.archived)
          .map((heading) => ({
            value: `heading:${heading.id}`,
            label: `${spaceLabel(project.spaceId)} › ${project.title} › ${heading.title}`,
          })),
      ]),
  ];
  const initialValue =
    single && task.headingId
      ? `heading:${task.headingId}`
      : single && task.projectId
      ...
      onSubmit: (value, newTitle) => {
        ...
        if (newTitle) {
          ...
          destinationProject = {
            ...
            spaceId:
              areaById(selectedAreaId)?.spaceId || itemSpaceId(task) || currentCreationSpaceId(),
            title: newTitle,
            notes: "",
            bucket: "anytime",
            scheduledFor: null,
            ...
            completedAt: null,
            loggedAt: null,
            order: ui.state.projects.length,
          };
        ...
        tasks.forEach((item) => {
          ...
          if (destinationProject) {
            item.projectId = destinationProject.id;
            item.areaId = destinationProject.areaId;
            item.spaceId = destinationProject.spaceId;
          } else if (value === "inbox") {
            item.bucket = "inbox";
            item.scheduledFor = null;
            item.evening = false;
            ...
            const area = areaById(value.slice(5));
            item.areaId = area.id;
            item.spaceId = area.spaceId || item.spaceId;
            if (item.bucket === "inbox") item.bucket = "anytime";
          } else if (value.startsWith("project:")) {
            const project = projectById(value.slice(8));
            item.projectId = project.id;
            item.areaId = project.areaId || null;
            item.spaceId = project.spaceId || item.spaceId;
            if (item.bucket === "inbox") item.bucket = "anytime";
          } else if (value.startsWith("heading:")) {
            const heading = ui.state.headings.find((candidate) => candidate.id === value.slice(8));
            const project = projectById(heading.projectId);
            const area = areaById(heading.areaId);
            item.headingId = heading.id;
            item.projectId = project?.id || null;
            item.areaId = project?.areaId || heading.areaId || null;
            item.spaceId = project?.spaceId || area?.spaceId || item.spaceId;
            if (item.bucket === "inbox") item.bucket = "anytime";
          }
        });
        scheduleSave();
        ...
        showToast(
          newTitle
            ? `Moved to new project “${newTitle}”`
            : `${tasks.length === 1 ? "To-do" : `${tasks.length} to-dos`} moved`,
        );
      },
    }),
    $("#modal-root"),
...
function searchItems(query, searchEverything = false) {
  ...
  const special = [
    ...
    ["deadlines", "Deadlines", "flag"],
    ["repeating", "Repeating", "repeat"],
    ["allProjects", "All Projects", "list"],
    ["loggedProjects", "Logged Projects", "check"],
  ].map(([type, title, iconName]) => ({
    kind: "view",
    type,
    title,
    meta: "Special list",
  ...
  const spaces = ui.state.spaces.map((space) => ({
    kind: "space",
    id: space.id,
    title: space.title,
    meta: "Space",
  ...
  const areas = ui.state.areas.map((area) => ({
    kind: "view",
    type: "area",
    id: area.id,
    title: area.title,
    meta: `${spaceLabel(area.spaceId)} · Area`,
    icon: "circle",
  }));
  const projects = ui.state.projects
    .filter(
      (p) => !p.workspaceTemplate && !p.repeat && (p.status === "open" || isCompletedButVisible(p)),
    )
    .map((p) => ({
      kind: "view",
      type: "project",
      id: p.id,
      title: p.title,
      meta: `${spaceLabel(p.spaceId)} · ${areaById(p.areaId)?.title || "Project"}`,
      icon: "list",
    }));
  const repeatingProjects = ui.state.projects
    .filter((p) => p.workspaceTemplate || p.repeat)
    .map((p) => ({
      kind: "view",
      type: "repeating",
      id: p.id,
      title: p.title,
      meta: "Repeating Project Template",
      icon: "repeat",
    }));
  const headings = ui.state.headings
    .filter((h) => !h.archived && !h.workspaceTemplateId)
    .map((heading) => {
      const parent = projectById(heading.projectId) || areaById(heading.areaId);
      return {
        kind: "heading",
        id: heading.id,
        projectId: heading.projectId,
        areaId: heading.areaId,
        title: heading.title,
        meta: `${spaceLabel(parent?.spaceId)} · ${parent?.title || "Heading"}`,
        icon: "heading",
      };
    });
  const tags = [
    ...new Set([
      ...ui.state.tasks.flatMap((task) => effectiveTags(task)),
      ...ui.state.projects.flatMap((project) => effectiveProjectTags(project)),
      ...ui.state.areas.flatMap((area) => area.tags || []),
    ]),
  ].map((tag) => ({ kind: "view", type: "tag", id: tag, title: tag, meta: "Tag", icon: "tag" }));
  const taskSource = searchEverything
    ? ui.state.tasks
    : ui.state.tasks.filter((task) => task.status === "open" || isCompletedButVisible(task));
  const tasks = taskSource.map((task) => ({
    kind: "task",
    id: task.id,
    title: task.title,
    meta: `${spaceLabel(itemSpaceId(task))} · ${task.repeat ? "Repeating template" : isLogged(task) ? "Logbook" : projectById(task.projectId)?.title || areaById(task.areaId)?.title || "To-do"}`,
    icon: task.repeat ? "repeat" : "circle",
    searchText: searchEverything
      ? `${task.title} ${task.notes || ""} ${effectiveTags(task).join(" ")} ${(task.checklist || []).map((i) => i.title).join(" ")}`
      : `${task.title} ${effectiveTags(task).join(" ")}`,
  }));
  const actions = [
    { kind: "settings", title: "Settings", meta: "App preferences", icon: "settings" },
    {
      kind: "space-settings",
      title: "Spaces & Schedule",
      meta: "Launch-time focus",
      icon: "clock",
    },
  ];
  ...
  const matches = [
    ...
    .slice(0, 24);
  if (q && !searchEverything)
    matches.push({
      kind: "continue",
      title: "Continue Search",
      meta: "Include notes, checklists, Logbook, and Trash",
      icon: "search",
    });
  return matches;
}
...
function chooseSearchResult(item) {
  ...
  if (item.kind === "settings") openSettings();
    ...
    setView(item.projectId ? "project" : "area", item.projectId || item.areaId);
  else {
...
function openNewListModal(defaults = {}) {
  const initialSpaceId =
    areaById(defaults.areaId)?.spaceId || defaults.spaceId || currentCreationSpaceId();
  modalReturnFocus = document.activeElement instanceof HTMLElement ? document.activeElement : null;
  modalUsesPreact = true;
  renderPreact(
    h(NewListDialog, {
      ...
      defaults: {
        type: defaults.type === "area" ? "area" : "project",
      ...
      onSubmit: ({ type, spaceId: requestedSpaceId, areaId, title }) => {
        ...
        if (type === "area") {
          const area = {
            ...
            title,
            color:
              spaceById(spaceId)?.color ||
              ["#5b7cfa", "#e49b3c", "#5ba67a", "#b06bd3"][ui.state.areas.length % 4],
            tags: [],
            order: ui.state.areas.length,
          };
          ui.state.areas.push(area);
          closeModal();
          setView("area", area.id);
        } else {
          const project = {
            id: uid("project"),
            spaceId: areaById(areaId)?.spaceId || spaceId,
            areaId,
            title,
            notes: "",
            bucket: "anytime",
...
function openHeadingModal(headingId = null) {
  ...
  const templateProject = projectById(
    heading?.workspaceTemplateId || (ui.view.type === "project" ? ui.view.id : null),
  );
  const defaultParent = heading?.projectId
    ? `project:${heading.projectId}`
    : heading?.areaId
      ? `area:${heading.areaId}`
      : `${ui.view.type}:${ui.view.id}`;
  const normalParentOptions = [
    ...ui.state.areas.map((area) => ({
      value: `area:${area.id}`,
      label: `${spaceLabel(area.spaceId)} › ${area.title} (Area)`,
    })),
    ...ui.state.projects
      .filter((project) => project.status === "open" && !project.repeat)
      .map((project) => ({
  ...
  const parentOptions = templateProject?.repeat
    ? [
        {
          value: `project:${templateProject.id}`,
          label: `${spaceLabel(templateProject.spaceId)} › ${templateProject.title} (Repeating Template)`,
        },
      ]
    : normalParentOptions;
  const initialParent = parentOptions.some((option) => option.value === defaultParent)
    ? defaultParent
    ...
    h(HeadingDialog, {
      ...
      onSave: (title, parent) => {
        ...
        if (heading) {
          ...
          ui.state.headings.push({
            id: uid("heading"),
            projectId: parentType === "project" ? parentId : null,
            areaId: parentType === "area" ? parentId : null,
            title,
            archived: false,
            order: headingsFor(parentType, parentId, true).length,
            ...(templateProject?.repeat
              ? {
                  workspaceTemplateId: templateProject.id,
                  workspaceBlueprintKey: uid("repeat-heading-blueprint"),
                }
              : {}),
          });
        scheduleSave();
      ...
      onAction: (action) => {
        ...
        if (action === "archive") {
          const unfinished = ui.state.tasks.filter(
            (task) => task.headingId === heading.id && task.status === "open" && !task.repeat,
          );
          if (unfinished.length) {
            showToast(
              `Complete or move ${unfinished.length} unfinished to-do${unfinished.length === 1 ? "" : "s"} first`,
            );
            return;
          }
          ...
          const project = {
            ...
            spaceId:
              ...
              areaById(projectAreaId)?.spaceId ||
              currentCreationSpaceId(),
          ...
          closeModal();
          setView("project", project.id);
          showToast("Heading converted to project");
          return;
        } else {
          const copy = {
            ...heading,
            id: uid("heading"),
            title: `${heading.title} copy`,
            order: heading.order + 0.5,
          };
          ui.state.headings.push(copy);
          ui.state.tasks
            .filter((task) => task.headingId === heading.id && task.status === "open")
            .forEach((task) =>
              ui.state.tasks.push({
                ...task,
                id: uid("task"),
...
function deleteHeading(headingId) {
  ...
  confirmAction(
    ...
    `${taskCount ? `${taskCount} to-do${taskCount === 1 ? "" : "s"} will remain in the parent list without a heading. ` : ""}The heading itself will be permanently deleted.`,
    "Delete heading",
    () => {
      ui.state.tasks
        .filter((task) => task.headingId === heading.id)
...
function moveProjectToTrash(projectId) {
  ...
  confirmAction(
    `Move “${project.title}” to Trash?`,
    "The list and all to-dos inside it can be restored from Trash.",
    "Move to Trash",
    () => {
      ...
      ui.state.tasks
        ...
        .forEach((task) => {
          task.previousStatus = task.status;
          task.status = "trashed";
          task.trashedAt = project.trashedAt;
          task.loggedAt = null;
        });
      scheduleSave();
      setView("trash");
      showToast("List moved to Trash");
    },
  );
}
...
function openProjectModalPreact(projectId) {
  ...
  if (!project) return;
  if (project.repeat) {
    openRepeatingProjectEditor(project);
    return;
  }
  const archivedHeadings = ui.state.headings.filter(
    (heading) => heading.projectId === project.id && heading.archived,
  );
  modalReturnFocus = document.activeElement instanceof HTMLElement ? document.activeElement : null;
  modalUsesPreact = true;
  renderPreact(
    h(ProjectDialog, {
      ...
      onSave: (draft) => {
        ...
        if (project.bucket === "upcoming" && !project.scheduledFor)
          project.scheduledFor = addDays(localDay(), 1);
        project.deadline = draft.deadline || null;
        project.tags = cleanTagList(draft.tags || []);
        const savedRepeat = draft.repeat
          ? {
              ...draft.repeat,
              weekdays: [...(draft.repeat.weekdays || [])],
              deadlineOffset: dayDistance(draft.repeat.nextDate, project.deadline),
              stopped: false,
            }
          : null;
        let newTemplateId = null;
        if (!project.repeat && savedRepeat) {
          ...
          const headingMap = new Map();
          ui.state.headings
            .filter((heading) => heading.projectId === project.id && !heading.workspaceTemplateId)
            .forEach((heading) => {
              const templateHeading = {
                ...cloneData(heading),
                id: uid("repeat-heading"),
                projectId: templateId,
                workspaceTemplateId: templateId,
                workspaceBlueprintKey: heading.id,
              };
              headingMap.set(heading.id, templateHeading.id);
              ui.state.headings.push(templateHeading);
            });
          ui.state.tasks
            .filter((task) => task.projectId === project.id && !task.workspaceTemplateId)
            .forEach((task) => {
              ui.state.tasks.push({
                ...cloneData(task),
                id: uid("repeat-task"),
          ...
          project.repeat = null;
        } else if (project.repeat && !savedRepeat)
          project.repeat = { ...project.repeat, paused: true, stopped: true };
      ...
      onAction: (action) => {
        ...
        if (action === "restore-trash") {
          ...
          ui.state.tasks
            .filter((task) => task.projectId === project.id && isTrashed(task))
            .forEach((task) => {
              task.status = task.previousStatus || "open";
              task.previousStatus = null;
              task.trashedAt = null;
          ...
          scheduleSave();
          closeModal();
          setView(isLogged(project) ? "logbook" : "project", isLogged(project) ? null : project.id);
        ...
        if (action === "delete-forever") {
          ...
          confirmAction(
            ...
            "The project, its headings, and every to-do inside it will be permanently deleted.",
            "Delete forever",
            () => {
              ui.state.projects = ui.state.projects.filter((item) => item.id !== project.id);
        ...
        if (action === "complete") {
          closeModal();
          resolveProjectCompletion(project);
          return;
        }
        if (action === "skip") {
          const skippedAt = new Date().toISOString();
          project.status = "canceled";
          project.completedAt = skippedAt;
          project.loggedAt = skippedAt;
          ui.state.tasks
            .filter(
              (task) => task.projectId === project.id && task.status === "open" && !task.repeat,
            )
            .forEach((task) => {
              task.status = "canceled";
              task.completedAt = skippedAt;
              task.loggedAt = skippedAt;
              task.completedWithProjectId = project.id;
            });
          const template = projectById(project.repeatTemplateId);
          if (template?.repeat?.mode === "afterCompletion")
            template.repeat.nextDate = nextRepeatDate(localDay(), template.repeat);
          ...
          closeModal();
          setView("upcoming");
          showToast("Project Occurrence skipped");
          return;
        }
        if (action === "cancel") finishProject(project, "canceled", "canceled");
          ...
          project.loggedAt = null;
          ui.state.tasks
            .filter((task) => task.completedWithProjectId === project.id)
            .forEach((task) => {
              task.status = "open";
              task.completedAt = null;
              task.loggedAt = null;
...
function resolveProjectCompletion(project) {
  const remaining = ui.state.tasks.filter(
    (task) => task.projectId === project.id && task.status === "open" && !task.repeat,
  );
  if (!remaining.length) {
    finishProject(project, "completed", "completed");
    return;
  }
  modalReturnFocus = document.activeElement instanceof HTMLElement ? document.activeElement : null;
  modalUsesPreact = true;
  renderPreact(
    h(FinishProjectDialog, {
      count: remaining.length,
      onClose: closeModal,
      onFinish: (status) => {
        closeModal();
        finishProject(project, "completed", status);
      },
    }),
    $("#modal-root"),
  );
...
function finishProject(project, projectStatus, remainingStatus) {
  ...
  project.loggedAt = null;
  ui.state.tasks
    .filter((task) => task.projectId === project.id && task.status === "open" && !task.repeat)
    .forEach((task) => {
      task.status = remainingStatus;
      task.completedAt = finishedAt;
      task.loggedAt = null;
      task.completedWithProjectId = project.id;
    });
...
function duplicateProject(project) {
  const copy = {
    ...
    tags: [...(project.tags || [])],
    repeat: project.repeat
      ? { ...project.repeat, weekdays: [...(project.repeat.weekdays || [])] }
      : null,
    status: "open",
    completedAt: null,
    loggedAt: null,
...
function removeArea(areaId) {
  ...
  confirmAction(
    `Remove “${area.title}”?`,
    "Its projects and to-dos will stay in the same Space. Projects become top-level lists, standalone to-dos become unfiled, and the area’s headings will be removed.",
    "Remove area",
    () => {
      ...
      ui.state.tasks
        .filter((task) => task.areaId === area.id)
        .forEach((task) => {
          task.areaId = task.projectId ? projectById(task.projectId)?.areaId || null : null;
          if (headingIds.has(task.headingId)) task.headingId = null;
        });
      ui.state.headings = ui.state.headings.filter((heading) => !headingIds.has(heading.id));
      scheduleSave();
      setView("inbox");
      showToast("Area removed; its projects and to-dos were kept");
    },
  );
}
    ...
    h(AreaDialog, {
      ...
      onSave: (draft) => {
        ...
        ui.state.projects
          .filter((project) => project.areaId === area.id)
          .forEach((project) => {
            project.spaceId = area.spaceId;
            ui.state.tasks
              .filter((task) => task.projectId === project.id)
              .forEach((task) => {
                task.spaceId = area.spaceId;
              });
          });
        ui.state.tasks
          .filter((task) => task.areaId === area.id)
...
function openSpaceSwitcher() {
  ...
  renderPreact(
    h(SpaceSwitcherDialog, {
      ...
      onManage: () => {
        closeModal();
        setTimeout(openSpaceSettings, 0);
      },
    }),
    $("#modal-root"),
  );
...
function applySpaceSettingsDraft(draft) {
  const spaces = draft.spaces.map((space, index) => ({
    ...space,
    title: space.title.trim() || `Space ${index + 1}`,
    order: index,
  }));
  let pinnedCount = 0;
  spaces.forEach((space) => {
    space.pinned = Boolean(space.pinned) && pinnedCount < 2;
    if (space.pinned) pinnedCount += 1;
  });
  if (!spaces.some((space) => space.pinned))
    spaces.slice(0, 2).forEach((space) => {
      space.pinned = true;
    });
  ui.state.spaces = spaces;
  setLaunchRulesEnabled(Boolean(draft.enabled));
  ui.state.settings.defaultSpaceId = spaces.some((space) => space.id === draft.defaultId)
    ? draft.defaultId
    : spaces[0]?.id || null;
...
function openSpaceSettings() {
  ...
  const rules = [...(ui.state.settings.spaceSchedule.rules || [])].sort(
    (a, b) => (a.order || 0) - (b.order || 0),
  );
  modalReturnFocus = document.activeElement instanceof HTMLElement ? document.activeElement : null;
  modalUsesPreact = true;
  renderPreact(
    h(SpacesSettingsDialog, {
      initial: {
        spaces,
        rules,
        enabled: ui.launchRulesEnabled,
        defaultId: ui.state.settings.defaultSpaceId,
      },
      onClose: closeModal,
      makeSpace: (index, pinned) => ({
        id: uid("space"),
        title: `Space ${index + 1}`,
        color: ["#5ba67a", "#b06bd3", "#e49b3c", "#5b7cfa"][index % 4],
        pinned,
        order: index,
      }),
      makeRule: (index, spaceId) => ({
        id: uid("space-rule"),
        spaceId,
        weekdays: [1, 2, 3, 4, 5],
        start: "09:00",
      ...
      onSave: (draft) => {
        ...
        closeModal();
        render();
        showToast("Space settings saved");
      },
      onDeleteSpace: (spaceId, draft) => {
        ...
        confirmAction(
          `Delete “${space.title}”?`,
          "Its areas, projects, to-dos, and calendar events will move to the default remaining Space.",
          "Delete Space",
          () => {
            applySpaceSettingsDraft(draft);
            const fallback =
              ui.state.spaces.find(
                (candidate) =>
                  candidate.id !== space.id && candidate.id === ui.state.settings.defaultSpaceId,
              ) || ui.state.spaces.find((candidate) => candidate.id !== space.id);
            if (!fallback) return;
            [...ui.state.areas, ...ui.state.projects, ...ui.state.tasks, ...ui.state.calendarEvents]
              .filter((item) => item.spaceId === space.id)
              .forEach((item) => {
                item.spaceId = fallback.id;
              });
            ui.state.spaces = ui.state.spaces.filter((candidate) => candidate.id !== space.id);
            ui.state.settings.spaceSchedule.rules = ui.state.settings.spaceSchedule.rules.filter(
              (rule) => rule.spaceId !== space.id,
            );
            if (ui.state.settings.defaultSpaceId === space.id)
              ui.state.settings.defaultSpaceId = fallback.id;
            if (ui.activeSpaceId === space.id) {
              ui.activeSpaceId = fallback.id;
              rememberActiveSpace();
            }
            scheduleSave();
            render();
            openSpaceSettings();
            showToast(`Deleted “${space.title}”`);
          },
        );
      },
    }),
...
function openSettings() {
  ...
  const pendingLogCount = [...ui.state.tasks, ...ui.state.projects].filter(
    (item) => matchesActiveSpace(item) && isCompletedButVisible(item),
  ).length;
  modalReturnFocus = document.activeElement instanceof HTMLElement ? document.activeElement : null;
  modalUsesPreact = true;
  renderPreact(
    h(SettingsDialog, {
      ...
      onAddEvent: ({ title, start, end }) => {
        ...
        if (!cleanTitle || !start) {
          showToast("Add an event title and start time");
          return;
        }
        if (cleanEnd < start) {
          showToast("The event end must be after its start");
          return;
        }
        ui.state.calendarEvents.push({
          id: uid("event"),
          spaceId: currentCreationSpaceId(),
          title: cleanTitle,
          start,
          end: cleanEnd,
          calendar: "Objects",
...
function updateTagsEverywhere(transform) {
  ui.state.settings.tags = cleanTagList(
    (ui.state.settings.tags || []).map(transform).filter(Boolean),
  );
  [...ui.state.areas, ...ui.state.projects, ...ui.state.tasks].forEach((item) => {
    item.tags = cleanTagList((item.tags || []).map(transform).filter(Boolean));
  });
}
...
function addTag(value) {
  ...
  if (getKnownTags().some((existing) => existing.toLocaleLowerCase() === tag.toLocaleLowerCase())) {
...
function renameTag(from, to) {
  ...
  updateTagsEverywhere((tag) => (tag === from ? normalized : tag));
  scheduleSave();
  openSettings();
  showToast(`Renamed “${from}” to “${normalized}”`);
}
...
function removeTag(tag) {
  ...
  confirmAction(
    `Remove “${tag}”?`,
    "The tag will be removed from every area, project, and to-do. The items themselves will not be deleted.",
    "Remove tag",
    () => {
      updateTagsEverywhere((value) => (value === tag ? null : value));
      scheduleSave();
      openSettings();
...
async function handleSettingsAction(action) {
  ...
  if (action === "notifications") {
    if (
      ...
      scheduleSave();
      openSettings();
      showToast("Notifications disabled in Objects");
      return;
    }
  ...
  if (action === "export") {
    ...
    anchor.download = `objects-backup-${localDay()}.json`;
    anchor.click();
...
function importJsonFile(event) {
  ...
  reader.onload = () => {
    try {
      ...
      closeModal();
      confirmAction(
        "Replace all Objects data?",
        "The imported backup will replace the current Spaces, tasks, projects, areas, settings, and calendar events.",
        "Import backup",
        () => {
          ui.state = state;
          normalizeState();
          initializeActiveSpace();
          ui.replaceWorkspace = true;
          scheduleSave();
          setView("today");
      ...
      showToast("This file is not a valid Objects backup");
    }
  };
...
function importIcsFile(event) {
  ...
  reader.onload = () => {
    ...
    for (const block of blocks) {
      ...
      const end = parseIcsDate(get("DTEND") || startRaw);
    ...
    scheduleSave();
    showToast(`Imported ${added} calendar event${added === 1 ? "" : "s"}`);
    openSettings();
  };
  reader.readAsText(file);
}
...
function handleGlobalKeydown(event) {
  ...
  if ((event.metaKey || event.ctrlKey) && /^[1-6]$/.test(event.key)) {
    ...
    event.preventDefault();
    setView(destinations[Number(event.key) - 1]);
    return;
  }
  if ((event.metaKey || event.ctrlKey) && event.key === "Enter" && ui.selectedTaskId) {
    event.preventDefault();
    toggleTask(ui.selectedTaskId, null, "keyboard");
    return;
  }
  if ((event.metaKey || event.ctrlKey) && event.key === "." && ui.selectedTaskId) {
    event.preventDefault();
    toggleTask(ui.selectedTaskId, null, "keyboard");
    return;
  }
  if (
    ...
    const task = currentTask();
    task.bucket = "today";
    task.scheduledFor = localDay();
    task.evening = false;
    moveReminderToDate(task, task.scheduledFor);
    scheduleSave();
    render(true);
    return;
  }
  ...
  if (
    ...
    event.preventDefault();
    renderInspector();
    setTimeout(() => $('[data-field="deadline"]', inspector)?.focus(), 20);
    return;
  }
  if (
    (event.metaKey || event.ctrlKey) &&
  ...
  if (
    ...
    event.preventDefault();
    const task = currentTask();
    if (task && repeatingEditorAccess(task) !== "unavailable") openRepeatingTemplateEditor(task);
    return;
  }
  if (
    (event.metaKey || event.ctrlKey) &&
    ...
    (ui.selectedTaskId || ui.selectedTaskIds.size)
  ) {
    event.preventDefault();
    openMoveTaskModal(ui.selectedTaskIds.size ? selectedTasks() : currentTask());
    return;
  }
  ...
  if (
    ...
    event.preventDefault();
    const task = currentTask();
    recordToDoIntent("keyboard", [task.id], { type: "cancel" });
    task.status = "canceled";
    task.completedAt = new Date().toISOString();
    task.loggedAt = null;
    applyLogbookPolicy();
    ...
    closeInspector();
    render();
    showToast("To-do canceled");
    return;
  }
  if ((event.metaKey || event.ctrlKey) && event.key.toLowerCase() === "d" && ui.selectedTaskId) {
    event.preventDefault();
    const task = currentTask();
    const copy = {
      ...task,
  ...
  if (
    ...
    if (event.key.length === 1 && !event.isComposing && event.key.trim()) {
      event.preventDefault();
...
async function flushSave() {
  ...
  if (!changes) {
    if (ui.renderAfterSave) {
      ui.renderAfterSave = false;
      render();
    }
    return;
  }
  ...
  try {
    ...
    if (pendingSnapshot && ui.pendingEntry?.revision === pendingSnapshot.revision) {
      const taskChanged = (changes.entities?.tasks || []).some(
        (change) => change.id === pendingSnapshot.task.id,
      );
