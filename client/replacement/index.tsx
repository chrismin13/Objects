...
function ReplacementHost({ appControls }: { appControls: ReplacementAppControls }) {
  ...
  useEffect(() => {
    ...
    void replacementRuntimeReady.then((runtime) => {
      if (active && rootRef.current) dispose = runtime.mountReplacement(rootRef.current, proxyRef.current, (toDo) => showToDoReminder(toDo, { replacement: true }), ownerIdentity, appControls);
    });
    return () => { active = false; dispose?.(); };
  }, [loading, ownerIdentity, appControls]);
...
export function ReplacementApp() {
  ...
  const appControls: ReplacementAppControls = {
    accountName: auth.displayName || auth.email || "Lakebed account",
    signOut,
    getPwaStatus,
