...
import {
  effectiveTagIdsForRepeatingTemplate,
  spaceIdForRepeatingTemplate,
} from "../../shared/workspace/location.ts";
import {
  directTargetUrl,
...
function discoveryDocument(): WorkspaceDocument {
  ...
  document.projects.push({
    id: "project-site",
    title: "Website",
    notes: "Public release plan",
    location: { kind: "area", areaId: "area-launch" },
  ...
  document.projects.push({
    id: "project-trash",
    title: "Discarded roadmap",
    notes: "Removed release plan",
  ...
  document.toDos.push(
    {
      ...
      notes: "Check the release wording",
      checklist: [{ id: "check-legal", title: "Ask legal", completed: false, order: 0 }],
    ...
    },
    {
      ...
      outcome: "completed",
      trashedAt: null,
      logbookAt: "2026-07-18T10:00:00.000Z",
      tags: [],
      occurrence: null,
    ...
    {
      id: "todo-trash",
      title: "Discard launch draft",
      notes: "Unused release",
      checklist: [],
      location: { kind: "unfiled", spaceId: "space-work" },
      schedule: { kind: "anytime" },
      reminder: null,
      ...
      toDos: [
        {
          ...
          notes: "Include conversion totals",
          headingKey: "heading-metrics",
          tags: [],
          checklist: [{ title: "Export dashboard", completed: false, order: 0 }],
          reminder: null,
          deadline: null,
          order: 0,
        },
...
test("Quick Find indexes navigation, organization, tags, active content, history, Trash, and Repeating Templates", () => {
  ...
  assert.deepEqual(
    new Set(releaseResults.map((result) => result.group)),
    new Set([
      "Organization",
      "Active work",
      "History",
  ...
  assert.ok(
    searchWorkspace(document, "legal", TODAY).some(
      (result) => result.target.kind === "toDo" && result.target.id === "todo-active",
    ),
  );
  assert.ok(searchWorkspace(document, "focused", TODAY).some((result) => result.group === "Tags"));
  assert.ok(
    searchWorkspace(document, "website", TODAY).some((result) => result.group === "Organization"),
  );
  assert.ok(
    searchWorkspace(document, "today", TODAY).some((result) => result.group === "Lists & views"),
  );
  assert.ok(
    searchWorkspace(document, "discarded", TODAY).some(
      (result) => result.group === "Trash" && result.target.kind === "project",
    ),
  );
  assert.deepEqual(
  ...
  assert.ok(
    searchWorkspace(document, "dashboard", TODAY).some(
      (result) =>
        result.target.kind === "repeatingTemplate" && result.target.id === "template-project",
    ),
  );
});
...
test("stale search results and inaccessible direct targets resolve to useful missing states", () => {
  ...
  assert.deepEqual(
    resolveDirectTarget(document, { kind: "project", id: "private-project" }, TODAY),
    {
      status: "missing",
  ...
  assert.equal(
    resolveDirectTarget(document, { kind: "project", id: "project-trash" }, TODAY).status,
    "resolved",
  );
  const trashedProject = resolveDirectTarget(
    document,
    { kind: "project", id: "project-trash" },
    TODAY,
  );
  assert.deepEqual(trashedProject.status === "resolved" ? trashedProject.view : null, {
    kind: "trash",
    date: TODAY,
  });
});
...
test("a direct target is recovered after the Workspace finishes loading", () => {
  ...
  if (recovered?.resolved.status === "resolved")
    assert.equal(recovered.resolved.selectedToDoId, "todo-active");
});
...
test("Quick Find suppresses an after-completion preview while its occurrence is still open", () => {
  ...
  document.toDos.push({
    ...
    checklist: [],
    location: { kind: "heading", headingId: "heading-copy" },
    schedule: { kind: "scheduled", date: template.nextDate, evening: false },
    reminder: null,
    deadline: null,
    outcome: "open",
    trashedAt: null,
    logbookAt: null,
    tags: [],
    occurrence: { templateId: template.id, scheduledDate: template.nextDate },
    createdAt: "2026-07-19T08:00:00.000Z",
    completedAt: null,
    order: 3,
  });
...
test("direct targets round-trip through shareable URLs and resolve every navigation target", () => {
  ...
  const projectTemplate = document.repeatingTemplates.find(
    (item) => item.id === "template-project",
  )!;
