...
test("device launch preferences choose a matching rule or remember a manual Space", () => {
  ...
  assert.equal(
    selectLaunchSpace(
      document,
      { launchRulesEnabled: true, manualSpaceId: "space-personal" },
  ...
  assert.equal(
    selectLaunchSpace(
      document,
      { launchRulesEnabled: false, manualSpaceId: "space-work" },
      new Date("2026-07-20T10:00:00"),
    ),
    "space-work",
  );
  assert.equal(
    selectLaunchSpace(
      document,
      { launchRulesEnabled: true, manualSpaceId: "space-work" },
  ...
  assert.equal(
    selectLaunchSpace(
      document,
      { launchRulesEnabled: false, manualSpaceId: "missing" },
      new Date("2026-07-20T10:00:00"),
    ),
    "space-personal",
  );
...
test("overnight launch rules use the previous weekday after midnight", () => {
  ...
  document.settings.launchRules = [
    { id: "night", spaceId: "space-work", weekdays: [1], start: "22:00", end: "02:00", order: 0 },
  ];

  assert.equal(
    selectLaunchSpace(
      document,
      { launchRulesEnabled: true, manualSpaceId: null },
      new Date("2026-07-21T01:00:00"),
    ),
    "space-work",
  );
});
...
test("HTTP-style capture can resolve relative dates in the caller's time zone", () => {
  ...
  const parsed = captureInputFromRecord(
    settingsDocument(),
    { title: "Tomorrow there", when: "tomorrow", submissionId: "zone-1" },
    athensToday,
  );
  ...
  if (parsed.ok)
    assert.deepEqual(parsed.input.schedule, {
      kind: "scheduled",
      date: "2026-07-21",
      evening: false,
    });
...
test("Application and Space settings persist through validated Workspace changes", () => {
  ...
  assert.equal(
    workspace.change({ type: "setDefaultSpace", spaceId: "space-work" }).status,
    "changed",
  );
  assert.equal(
    workspace.change({
...
test("invalid Space settings are rejected without changing the Workspace", () => {
  ...
  assert.equal(
    workspace.change({ type: "setDefaultSpace", spaceId: "missing" }).status,
    "rejected",
  );
  assert.equal(
    workspace.change({
      type: "replaceLaunchRules",
      rules: [
        { id: "bad", spaceId: "space-work", weekdays: [9], start: "29:00", end: "17:00", order: 0 },
      ],
    }).status,
    "rejected",
  );
...
test("capture links keep shared text, URLs, Location, Tags, Schedule, Reminder, and Deadline", () => {
  ...
  assert.deepEqual(
    toDo.tags.map((id) => workspace.read().tags.find((tag) => tag.id === id)?.title),
    ["Research", "Deep work"],
  );
});
...
test("capture retries reuse a submission receipt instead of creating a duplicate", () => {
  ...
  const parsed = captureInputFromRecord(
    workspace.read(),
    { title: "Inbox item", submissionId: "request-42" },
    "2026-07-19",
  );
  assert.equal(parsed.ok, true);
  if (!parsed.ok) return;
...
test("the HTTP capture seam uses Workspace validation and returns the saved item on a retry", () => {
  ...
  const first = captureIntoSnapshot(
    null,
    settingsDocument(),
    {
      title: "Endpoint item",
      submissionId: "http-42",
      spaceId: "space-work",
      when: "tomorrow",
    },
    dependencies,
  ...
  const retry = captureIntoSnapshot(
    first.next,
    first.next.document,
    {
      title: "Endpoint item",
  ...
  const foreignLocation = captureIntoSnapshot(
    first.next,
    first.next.document,
    {
      title: "Wrong owner",
      submissionId: "http-43",
      areaId: "another-account-area",
    },
    dependencies,
  );
...
test("the first HTTP capture starts from retained migrated data", () => {
  ...
  const captured = captureIntoSnapshot(
    base.current,
    base.initial,
    {
  ...
  assert.deepEqual(
    captured.next?.document.toDos.map((toDo) => toDo.title),
    ["Retained work", "Captured after cutover"],
  );
});
...
test("malformed capture data and another account's Location are rejected without partial work", () => {
  ...
  const malformed = captureInputFromRecord(
    workspace.read(),
    {
      title: "Bad capture",
      areaId: "another-account-area",
      reminderAt: "not-a-date",
      submissionId: "bad-request",
    },
    "2026-07-19",
  );
...
test("saved settings and capture receipts survive reload and remain isolated by account", async () => {
  ...
  const capture = captureInputFromRecord(
    workspace.read(),
    { title: "Private", submissionId: "private-1" },
    "2026-07-19",
  );
