...
test("quick entry saves a normal to-do and understands dates, times, deadlines, and tags", () => {
  ...
  assert.deepEqual(
    workspace.read().tags.map((tag) => tag.title),
    ["people"],
  );

  const relativeId = changedId(
    workspace.change({
      type: "createToDo",
      title: "Renew card in 3 weeks",
      quickEntry: { referenceDate: "2026-07-19" },
    }),
  );
  assert.deepEqual(toDo(workspace, relativeId).schedule, {
    kind: "scheduled",
    date: "2026-08-09",
    evening: false,
  });
...
test("unfinished quick entry text is saved, restored, and cleared explicitly", () => {
  ...
  assert.equal(
    workspace.change({
      ...
      view: { kind: "today", date: "2026-07-19" },
    }).status,
    "changed",
  );
...
test("a to-do can enter a weekly repeating schedule through Workspace", () => {
  ...
  assert.deepEqual(toDo(workspace, id).schedule, {
    kind: "scheduled",
    date: "2026-07-26",
    evening: false,
  });
  assert.equal(
    toDo(workspace, id).occurrence?.templateId,
    workspace.read().repeatingTemplates[0]?.id,
  );
});
...
test("the inspector edit path updates fields and manages independent checklist items", () => {
  ...
  const first = changedId(
    workspace.change({ type: "addChecklistItem", toDoId: id, title: "Book train" }),
  );
  const second = changedId(
    workspace.change({ type: "addChecklistItem", toDoId: id, title: "Reserve room" }),
  );
  workspace.change({
    type: "updateChecklistItem",
    toDoId: id,
    itemId: first,
    changes: { completed: true, title: "Book night train" },
  });
  ...
  assert.deepEqual(updated.checklist, [
    { id: first, title: "Book night train", completed: true, order: 0 },
  ]);
  workspace.change({ type: "setToDoTags", id, titles: ["travel", "important", "travel"] });
  assert.deepEqual(
    toDo(workspace, id).tags.map(
      (tagId) => workspace.read().tags.find((tag) => tag.id === tagId)?.title,
    ),
    ["travel", "important"],
  );
});
...
test("checklist order changes through the public Workspace seam", () => {
  ...
  assert.deepEqual(
    workspace
      ...
      ?.checklist.map((item) => [item.id, item.order]),
    [
      [orderedIds[0], 0],
      [orderedIds[1], 1],
      [orderedIds[2], 2],
...
test("moving a scheduled to-do keeps a same-day reminder at the same time", () => {
  ...
  const id = changedId(
    workspace.change({
      ...
      title: "Prepare",
      schedule: { kind: "scheduled", date: "2026-07-20", evening: false },
      reminderAt: "2026-07-20T16:45:00.000Z",
    }),
  );
  workspace.change({
    type: "updateToDo",
    id,
    changes: { schedule: { kind: "scheduled", date: "2026-07-23", evening: false } },
  });
...
test("immediate, daily, and manual Logbook policies remain distinct", () => {
  ...
  const dailyId = changedId(
    workspace.change({
      type: "createToDo",
      title: "Daily",
      schedule: { kind: "scheduled", date: "2026-07-19", evening: false },
    }),
  );
  ...
  const workSpaceId = changedId(
    workspace.change({ type: "createSpace", title: "Work", color: "#5577dd" }),
  );
  const workId = changedId(
    workspace.change({
      type: "createToDo",
      title: "Work only",
      location: { kind: "unfiled", spaceId: workSpaceId },
    }),
  );
...
test("Trash is separate from Outcome and destructive actions require exact confirmation", () => {
  ...
  workspace.change({ type: "trashToDo", id });
  assert.equal(
    workspace.change({ type: "permanentlyDeleteToDo", id, confirmation: "yes" }).outcome,
  ...
  assert.equal(
    workspace.change({ type: "emptyTrash", spaceId: "space-personal", confirmation: "" }).outcome,
    "confirmation-required",
  );
  assert.equal(
    workspace.change({
      ...
      confirmation: EMPTY_TRASH_CONFIRMATION,
    }).status,
    "changed",
  );
});
...
test("all standard views are derived, filtered, and ordered through Workspace", () => {
  const workspace = setup();
  const create = (title: string, schedule: ToDo["schedule"], deadline: string | null = null) =>
    changedId(workspace.change({ type: "createToDo", title, schedule, deadline }));
  const inbox = create("Inbox", { kind: "inbox" });
  const anytime = create("Anytime", { kind: "anytime" });
  const someday = create("Someday", { kind: "someday" });
  const overdue = create(
    "Overdue",
    { kind: "scheduled", date: "2026-07-18", evening: false },
    "2026-07-22",
  );
  ...
  const future = create(
    "Future",
    { kind: "scheduled", date: "2026-07-25", evening: false },
    "2026-07-21",
  );
  ...
  const projectWorkspace = createWorkspace(withProject, {
    now: () => NOW,
    createId: (kind) => `${kind}-view`,
  });

  assert.deepEqual(
    workspace.view({ kind: "inbox", date: "2026-07-19" }).map((item) => item.id),
    [inbox],
  );
  assert.deepEqual(
    workspace.view({ kind: "today", date: "2026-07-19" }).map((item) => item.id),
    [overdue, today, evening],
  );
  assert.deepEqual(
    workspace.view({ kind: "thisEvening", date: "2026-07-19" }).map((item) => item.id),
    [evening],
  );
  assert.deepEqual(
    workspace.view({ kind: "tomorrow", date: "2026-07-20" }).map((item) => item.id),
    [tomorrow],
  );
  assert.deepEqual(
    workspace.view({ kind: "upcoming", date: "2026-07-19" }).map((item) => item.id),
    [tomorrow, future],
  );
  assert.deepEqual(
    workspace.view({ kind: "anytime", date: "2026-07-19" }).map((item) => item.id),
    [anytime],
  );
  assert.deepEqual(
    workspace.view({ kind: "someday", date: "2026-07-19" }).map((item) => item.id),
    [someday],
  );
  assert.deepEqual(
    projectWorkspace.view({ kind: "deadlines", date: "2026-07-19" }).map((item) => item.id),
    ["project-deadline", future, overdue],
  );
});
...
test("Trash and every Location view are derived through Workspace", () => {
  ...
  const workspace = createWorkspace(document, {
    now: () => NOW,
    createId: (kind) => `${kind}-location`,
  });
  const id = changedId(
    workspace.change({
      type: "createToDo",
      title: "Write copy",
      location: { kind: "heading", headingId: "heading-copy" },
    }),
  );
  ...
  for (const view of [
    ...
    assert.deepEqual(
      workspace.view(view).map((item) => item.id),
      [id],
    );
  ...
  workspace.change({
    type: "updateToDo",
    id,
    changes: { location: { kind: "area", areaId: "area-work" } },
  });
  ...
  assert.deepEqual(
    workspace.view({ kind: "area", id: "area-work", date: "2026-07-19" }).map((item) => item.id),
    [id],
  );
...
test("malformed and rejected lifecycle actions never change the Workspace", () => {
  ...
  assert.equal(
    workspace.change({
      ...
      changes: {
        ...
        schedule: { kind: "scheduled", date: "not-a-date", evening: false },
      },
    }).status,
    "rejected",
  );
  ...
  assert.equal(
    workspace.change({ type: "addChecklistItem", toDoId: id, title: " " }).status,
    "rejected",
  );
  assert.equal(
    workspace.change({ type: "setToDoTags", id, titles: ["x".repeat(101)] }).status,
    "rejected",
  );
