...
test("ICS parsing rejects invalid ranges and duplicate events clearly", () => {
  ...
  const malformed = parseIcsCalendar(
...
test("manual calendar events can be created, edited, imported once, and deleted through Workspace", () => {
  ...
  assert.equal(
    workspace.change({ type: "updateCalendarEvent", id: eventId, changes: { title: "Team lunch" } })
      .status,
    "changed",
  );
  ...
  assert.equal(
    workspace.change({ type: "importIcsCalendar", source: ics, spaceId: "space-work" }).status,
    "changed",
  );
  const duplicate = workspace.change({
  ...
  assert.deepEqual(
    workspace.read().calendarEvents.map((event) => event.title),
    ["Demo"],
  );
});
...
test("Today, Tomorrow, and Upcoming agendas order calendar events beside scheduled Projects and to-dos", () => {
  ...
  assert.deepEqual(
    tomorrow.map((item) => [item.kind, item.title]),
    [
  ...
  assert.ok(
    tomorrow[0]!.actionable === false && tomorrow.slice(1).every((item) => item.actionable),
  );
  assert.deepEqual(
    agendaForView(document, { kind: "today", date: "2026-07-19" }, "space-work"),
    [],
  );
  assert.deepEqual(
    agendaForView(document, { kind: "upcoming", date: "2026-07-19" }, "space-work").map(
      (item) => item.title,
    ),
    ["Stand-up", "Launch", "Send brief"],
  );
});

test("an all-day ICS end date is exclusive when agenda days are calculated", () => {
  const workspace = setup();
  const zeroLength = workspace.change({
    type: "createCalendarEvent",
    title: "Broken offsite",
    spaceId: "space-work",
    start: "2026-07-21T00:00:00.000Z",
  ...
  const document = workspace.read();
  assert.deepEqual(
    agendaForView(document, { kind: "tomorrow", date: "2026-07-21" }, "space-work").map(
      (item) => item.title,
    ),
    ["Offsite"],
  );
  assert.deepEqual(
    agendaForView(document, { kind: "tomorrow", date: "2026-07-22" }, "space-work"),
    [],
  );
});
...
test("agenda Tag filters use effective Tags and hide untagged calendar events", () => {
  ...
  const agenda = agendaForView(
    workspace.read(),
    { kind: "tomorrow", date: "2026-07-20" },
    "space-work",
    "UTC",
    [tagId],
  );
...
test("calendar events sort by their displayed time in the requested time zone", () => {
  ...
  assert.deepEqual(
    agenda.map((item) => item.title),
    ["Early local event", "Late local event"],
  );
});
...
test("local dates and midnight event ends respect the requested time zone", () => {
  ...
  assert.deepEqual(
    agendaForView(workspace.read(), { kind: "tomorrow", date: "2026-07-20" }, "space-work", "UTC"),
    [],
  );
});
