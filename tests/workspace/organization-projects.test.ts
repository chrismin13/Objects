...
test("moving organization parents changes inherited Location without rewriting the to-do", () => {
  ...
  const areaId = changedId(
    workspace.change({ type: "createArea", title: "Clients", spaceId: "space-personal" }),
    "area",
  );
  ...
  assert.deepEqual(workspace.locationOfToDo(toDoId), {
    headingId,
    projectId,
    areaId,
    spaceId: "space-work",
  ...
  workspace.change({
    type: "updateProject",
    id: projectId,
    changes: { location: { kind: "space", spaceId: "space-personal" } },
  });
  assert.deepEqual(workspace.locationOfToDo(toDoId), {
    headingId,
    projectId,
  ...
  workspace.change({
    type: "updateHeading",
    id: headingId,
    changes: { location: { kind: "area", areaId } },
  });
...
test("Spaces can be edited and reordered, then deleted by moving all content to a chosen Space", () => {
  const workspace = setup();
  const areaId = changedId(
    workspace.change({ type: "createArea", title: "Home", spaceId: "space-personal" }),
    "area",
  );
  const projectId = changedId(
    workspace.change({
  ...
  const areaToDoId = changedId(
    workspace.change({ type: "createToDo", title: "Filed", location: { kind: "area", areaId } }),
    "toDo",
  );
  const projectToDoId = changedId(
    workspace.change({
      type: "createToDo",
      title: "Packed",
      location: { kind: "project", projectId },
  ...
  workspace.change({
    type: "updateSpace",
    id: "space-work",
    changes: { title: "Studio", color: "#123456", pinned: false },
  });
  workspace.change({ type: "reorderSpace", id: "space-work", toIndex: 0 });
  assert.deepEqual(
    workspace.read().spaces.map((space) => [space.title, space.color, space.pinned, space.order]),
    [
  ...
  const rejected = workspace.change({
    type: "deleteSpace",
    id: "space-personal",
    moveToSpaceId: "space-work",
    confirmation: "yes",
  });
  assert.equal(rejected.outcome, "confirmation-required");
  const deleted = workspace.change({
    type: "deleteSpace",
    id: "space-personal",
    moveToSpaceId: "space-work",
  ...
  assert.deepEqual(
    workspace.read().spaces.map((space) => space.id),
    ["space-work"],
  );
  assert.equal(workspace.read().settings.defaultSpaceId, "space-work");
  for (const id of [unfiledId, areaToDoId, projectToDoId])
    assert.equal(workspace.locationOfToDo(id)?.spaceId, "space-work");
...
test("removing an Area keeps its Projects and to-dos in the same Space", () => {
  ...
  const headingId = changedId(
    workspace.change({ type: "createHeading", title: "Calls", location: { kind: "area", areaId } }),
    "heading",
  );
  const directId = changedId(
    workspace.change({ type: "createToDo", title: "Direct", location: { kind: "area", areaId } }),
    "toDo",
  );
  const headingToDoId = changedId(
    workspace.change({
      type: "createToDo",
      title: "Under heading",
      location: { kind: "heading", headingId },
  ...
  assert.equal(
    workspace.change({ type: "removeArea", id: areaId, confirmation: "" }).outcome,
    "confirmation-required",
  );
  assert.equal(
    workspace.change({ type: "removeArea", id: areaId, confirmation: REMOVE_AREA_CONFIRMATION })
      .status,
    "changed",
  );
  assert.equal(workspace.read().areas.length, 0);
  assert.equal(workspace.read().headings.length, 0);
  assert.deepEqual(workspace.read().projects[0]?.location, {
    kind: "space",
    spaceId: "space-personal",
  });
  assert.deepEqual(storedToDo(workspace, directId).location, {
    kind: "unfiled",
    spaceId: "space-personal",
  });
  assert.deepEqual(storedToDo(workspace, headingToDoId).location, {
    kind: "unfiled",
    spaceId: "space-personal",
  });
  assert.deepEqual(storedToDo(workspace, projectToDoId).location, { kind: "project", projectId });
...
test("Project Closure requires every open to-do outcome and restore reopens only closure changes", () => {
  ...
  assert.deepEqual(
    closed.affected.map((item) => item.id),
    [projectId, completeId, cancelId],
  );
...
test("Projects report progress, duplicate with fresh identity, and retain children through Trash", () => {
  ...
  assert.deepEqual(workspace.projectProgress(projectId), {
    total: 2,
    open: 1,
    completed: 1,
    canceled: 0,
  ...
  const copyId = changedId(
    workspace.change({ type: "duplicateProject", id: projectId }),
    "project",
  );
  const document = workspace.read();
  const copiedHeading = document.headings.find(
    (heading) => heading.location.kind === "project" && heading.location.projectId === copyId,
  );
  const copiedToDos = document.toDos.filter(
    (item) => workspace.locationOfToDo(item.id)?.projectId === copyId,
  );
  assert.ok(copiedHeading);
  assert.equal(copiedToDos.length, 2);
  assert.ok(
    copiedToDos.every(
      (item) => item.outcome === "open" && item.id !== firstId && item.id !== secondId,
    ),
  );
  assert.notEqual(copiedHeading.id, headingId);
  assert.notEqual(
    copiedToDos.find((item) => item.checklist.length)?.checklist[0]?.id,
    storedToDo(workspace, firstId).checklist[0]?.id,
  );

  workspace.change({ type: "trashProject", id: projectId });
  assert.deepEqual(
    workspace.view({ kind: "trash", date: "2026-07-19" }).map((item) => item.id),
    [projectId],
  );
  assert.equal(
    workspace
      .read()
      .toDos.filter((item) => workspace.locationOfToDo(item.id)?.projectId === projectId).length,
    2,
  );
  workspace.change({ type: "restoreProjectFromTrash", id: projectId });
  assert.equal(
    workspace.read().projects.find((project) => project.id === projectId)?.trashedAt,
    null,
  );
});
...
test("Project Closure changes and restores a repeating occurrence as normal child work", () => {
  ...
  const workspace = createWorkspace(document, {
    now: () => NOW,
    createId: (kind) => `${kind}-closure-${++nextId}`,
  });
  ...
  assert.deepEqual(storedToDo(workspace, occurrenceId).occurrence, {
    templateId: "template-weekly",
    scheduledDate: "2026-07-19",
  });
  workspace.change({ type: "restoreProject", id: projectId });
...
test("permanent Project deletion requires confirmation and removes its complete child tree", () => {
  ...
  assert.equal(
    workspace.change({ type: "permanentlyDeleteProject", id: projectId, confirmation: "yes" })
      .outcome,
    "confirmation-required",
  );
  assert.equal(
  ...
  assert.equal(
    workspace.read().projects.some((project) => project.id === projectId),
    false,
  );
  assert.equal(
    workspace.read().headings.some((heading) => heading.id === headingId),
    false,
  );
  assert.equal(
    workspace.read().toDos.some((item) => item.id === toDoId),
    false,
  );
  assert.ok(
    workspace
      ...
      .permanentDeletions.some(
        (marker) => marker.entityKind === "project" && marker.entityId === projectId,
      ),
  );
});
...
test("Headings duplicate with their to-dos and archive without changing their contents", () => {
  ...
  const copyHeadingId = changedId(
    workspace.change({ type: "duplicateHeading", id: headingId }),
    "heading",
  );
  const copyToDo = workspace
    .read()
    .toDos.find(
      (item) => item.location.kind === "heading" && item.location.headingId === copyHeadingId,
    );
  assert.ok(copyToDo);
  ...
  assert.equal(
    workspace.read().headings.find((heading) => heading.id === headingId)?.archivedAt,
    NOW,
  );
  assert.deepEqual(storedToDo(workspace, toDoId).location, { kind: "heading", headingId });
...
test("deleting a Heading keeps its to-dos in the same parent", () => {
  ...
  assert.equal(
    workspace.change({ type: "deleteHeading", id: headingId, confirmation: "" }).outcome,
    "confirmation-required",
  );
  assert.equal(
    workspace.change({
...
test("converting a Heading into a Project keeps its contents together at the same higher Location", () => {
  ...
  const parentProjectId = changedId(
    workspace.change({
      type: "createProject",
      title: "Parent",
      location: { kind: "area", areaId },
  ...
  const headingId = changedId(
    workspace.change({
      type: "createHeading",
      title: "Calls",
      location: { kind: "project", projectId: parentProjectId },
    }),
    "heading",
  );
  const toDoId = changedId(
  ...
  const newProjectId = changedId(
    workspace.change({ type: "convertHeadingToProject", id: headingId }),
    "project",
  );
  ...
  assert.deepEqual(storedToDo(workspace, toDoId).location, {
    kind: "project",
...
test("effective Tags combine Area, Project, and direct tags without copying them onto to-dos", () => {
  ...
  assert.deepEqual(
    workspace
      .view({ kind: "anytime", date: "2026-07-19", tagIds: [areaTagId, projectTagId] })
      .map((item) => item.id),
    [toDoId],
  );
  assert.deepEqual(
    workspace.view({ kind: "anytime", date: "2026-07-19", tagIds: [areaTagId, "missing"] }),
    [],
  );
  ...
  workspace.change({
    type: "updateProject",
    id: projectId,
    changes: { location: { kind: "space", spaceId: "space-work" } },
  });
  assert.deepEqual(workspace.effectiveTagIdsForToDo(toDoId), [projectTagId, directTagId]);
  assert.deepEqual(
    workspace.view({ kind: "anytime", date: "2026-07-19", tagIds: [areaTagId] }),
    [],
  );
});
...
test("Tags can be renamed and deleted from every direct and inherited use", () => {
  ...
  assert.equal(
    workspace.change({ type: "deleteTag", id: tagId, confirmation: "" }).outcome,
    "confirmation-required",
  );
  assert.equal(
    workspace.change({ type: "deleteTag", id: tagId, confirmation: DELETE_TAG_CONFIRMATION })
      .status,
    "changed",
  );
  assert.equal(
    workspace.read().tags.some((tag) => tag.id === tagId),
    false,
  );
