...
async function runAdapterContract(adapter: WorkspaceSyncAdapter) {
  ...
  const stale = await adapter.save({
    expectedRevision: 0,
  ...
  if (stale.status === "conflict")
    assert.equal(stale.snapshot.document.spaces[0].title, "Personal");

  const malformed = structuredClone(document);
  malformed.format = "broken" as "objects-workspace";
  const rejected = await adapter.save({
    expectedRevision: 1,
...
test("the Lakebed adapter cannot cross an unconfirmed account boundary", async () => {
  ...
  let saves = 0;
  let subscriptions = 0;
  let remoteNotification: () => void = () => undefined;
  ...
  await assert.rejects(
    blocked.save({
      expectedRevision: 0,
      mutationId: "blocked",
      document: createEmptyWorkspace("2026-07-19T08:00:00.000Z"),
    }),
    /Session unavailable/,
  );
...
test("adapter scopes isolate accounts and preserve permanent-deletion markers", async () => {
  ...
  deleted.permanentDeletions.push({
    entityKind: "space",
    entityId: "space-personal",
...
test("multi-device saves merge unrelated fields and report same-field conflicts", async () => {
  ...
  assert.deepEqual(sameField.conflicts, [
    { path: "toDos/todo-1/title", resolution: "local-change-kept" },
  ]);
});
...
test("an acknowledged mutation remains safe to retry after later saves", async () => {
  ...
  await saveChanged(adapter, firstSnapshot, "second", (next) => {
...
test("a stale device cannot recreate permanently deleted work", async () => {
  ...
  await saveChanged(deletingDevice, commonBase, "delete", (next) => {
    next.toDos = [];
    next.permanentDeletions.push({
      entityKind: "toDo",
      entityId: "todo-deleted",
      deletedAt: "2026-07-19T09:00:00.000Z",
    });
  });
  ...
  assert.deepEqual(staleEdit.conflicts, [
    { path: "toDos/todo-deleted/title", resolution: "permanent-deletion-kept" },
  ]);
});
...
test("several devices materialize one Occurrence for the same Template date", async () => {
  ...
  const occurrence = (id: string) => ({
    ...
    notes: "",
    checklist: [],
    location: { kind: "unfiled" as const, spaceId: "space-personal" },
    schedule: { kind: "scheduled" as const, date: "2026-07-19", evening: false },
    reminder: null,
    deadline: null,
    ...
    logbookAt: null,
    tags: [],
    occurrence: { templateId: "template-1", scheduledDate: "2026-07-19" },
  ...
  assert.deepEqual(
    second.snapshot.document.toDos.map((item) => item.id),
    ["occurrence-a"],
  );
...
test("concurrent parent moves and checklist removal keep a complete valid result", async () => {
  ...
  assert.equal(merged.status, "acknowledged");
  if (merged.status !== "acknowledged") return;
  assert.deepEqual(merged.snapshot.document.toDos[0].location, {
    kind: "project",
    projectId: "project-b",
  });
  assert.deepEqual(merged.snapshot.document.toDos[0].checklist, [
    { id: "check-2", title: "Keep this", completed: true, order: 0 },
  ]);
  assert.ok(
    merged.conflicts.some(
      (conflict) =>
        conflict.path === "toDos/todo-1/checklist/check-1/title" &&
        conflict.resolution === "remote-removal-kept",
    ),
  );
  assert.deepEqual(
    createEmptyWorkspace("2026-07-19T09:00:00.000Z").format,
    merged.snapshot.document.format,
  );
});
...
test("a concurrent checklist reorder keeps one device's complete intended order", async () => {
  ...
  await saveChanged(firstDevice, commonBase, "order-first", (next) =>
    reorder(next, ["check-b", "check-z", "check-a"]),
  );
  const merged = await saveChanged(secondDevice, commonBase, "order-second", (next) =>
    reorder(next, ["check-z", "check-a", "check-b"]),
  );

  assert.equal(merged.status, "acknowledged");
  if (merged.status !== "acknowledged") return;
  assert.deepEqual(
    merged.snapshot.document.toDos[0].checklist.map((item) => [item.id, item.order]),
    [
      ["check-z", 0],
      ["check-a", 1],
      ["check-b", 2],
  ...
  assert.ok(
    merged.conflicts.some(
      (conflict) =>
        conflict.path === "toDos/todo-1/checklist/order" &&
        conflict.resolution === "local-change-kept",
    ),
  );
});
...
test("concurrent capture and calendar retries keep one durable result", async () => {
  ...
  const addCapturedWork = (next: WorkspaceSyncSnapshot["document"], suffix: string) => {
    ...
    next.calendarEvents.push({
      ...
      calendar: "Work",
      allDay: false,
      sourceUid: "calendar-uid-1",
    });
  };
  await saveChanged(firstDevice, commonBase, "capture-a", (next) => addCapturedWork(next, "a"));
  const merged = await saveChanged(secondDevice, commonBase, "capture-b", (next) =>
    addCapturedWork(next, "b"),
  );

  assert.equal(merged.status, "acknowledged");
  if (merged.status !== "acknowledged") return;
  assert.deepEqual(
    merged.snapshot.document.toDos.map((item) => item.id),
    ["todo-a"],
  );
  assert.deepEqual(
    merged.snapshot.document.captureReceipts.map((item) => item.toDoId),
    ["todo-a"],
  );
  assert.deepEqual(
    merged.snapshot.document.calendarEvents.map((item) => item.id),
    ["event-a"],
  );
});
