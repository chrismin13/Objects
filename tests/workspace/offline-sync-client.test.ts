...
import {
  ...
  type WorkspaceSyncPersistence,
} from "../../shared/workspace/sync-client.ts";
...
test("a representative backup replaces the visible Workspace exactly after save and reopen", async () => {
  ...
  const seeded = await durable.save({
    expectedRevision: 0,
    mutationId: "seed-current",
    document: current,
  });
  ...
  const previousState = workspaceDocumentToInterfaceState(
    firstSession.read().snapshot!.document,
    "2026-07-20",
  );
  ...
  assert.ok(changeSet);
  const replacement = applyInterfaceChangeSetToWorkspace(
    firstSession.read().snapshot!.document,
    changeSet!,
    {
      now: () => FIXTURE_NOW,
  ...
  const reopened = createWorkspaceSyncClient(
    durable,
    memoryPersistence(),
  ...
  const visibleAfterReopen = workspaceDocumentToInterfaceState(
  ...
  const normalizeVirtualOrder = <T extends { workspaceTemplateId?: unknown; order?: unknown }>(
    state: T,
  ) => {
...
test("pending changes survive reload and an interrupted acknowledgement", async () => {
  ...
  const firstSession = createWorkspaceSyncClient(
    uncertainAdapter,
    persistence,
  ...
  const recoveredSession = createWorkspaceSyncClient(
    durable,
    persistence,
...
test("an order-only local change enters the durable queue and syncs", async () => {
  ...
  await client.flush();
  assert.deepEqual(
    (await durable.load())!.document.toDos.map((item) => item.id),
    ["todo-b", "todo-a"],
  );
});
