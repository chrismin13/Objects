...
} from "../../shared/workspace/legacy-storage.ts";
import {
...
test("a current portable backup round trip preserves all supported Workspace data and relationships", () => {
  ...
  source.settings.launchRules.push({
    id: "rule-work",
    spaceId: "space-work",
    weekdays: [1, 2, 3, 4, 5],
    start: "09:00",
  ...
  source.toDos.push({
    ...
    reminder: { at: "2026-07-20T08:30:00.000Z", sentAt: null },
    deadline: "2026-07-21",
    outcome: "completed",
    trashedAt: "2026-07-21T12:00:00.000Z",
    logbookAt: "2026-07-20T12:00:00.000Z",
    tags: ["tag-focus"],
    occurrence: null,
  ...
  source.permanentDeletions.push({
    entityKind: "toDo",
    entityId: "todo-deleted",
    deletedAt: "2026-07-18T08:00:00.000Z",
  });

  const target = deterministicWorkspace();
  const result = target.importPortableBackup(
    exportPortableBackup(source),
    FULL_IMPORT_CONFIRMATION,
  );
...
test("a current backup with invalid settings and entity enums is rejected atomically", () => {
  const workspace = deterministicWorkspace();
  const before = workspace.read();
  const malformed = createEmptyWorkspace("2026-07-19T08:00:00.000Z") as unknown as Record<
    string,
    unknown
  >;
  ...
  (malformed.spaces as unknown[]).push({
    id: "space-1",
    title: "Personal",
    color: "#000000",
    pinned: true,
  ...
  const result = workspace.importPortableBackup(
...
test("a representative current backup imports with corrections and a clear report", () => {
    ...
    tasks: [
      {
        ...
        scheduledFor: "2026-07-19",
        evening: true,
        reminderAt: "2026-07-19T08:00:00",
        deadline: "2026-07-20",
        tags: ["Focused"],
        checklist: [{ id: "check-1", title: "Read", done: true }],
        repeat: {
          mode: "fixed",
          frequency: "weekly",
          interval: 1,
  ...
  assert.deepEqual(imported.toDos[0].schedule, {
    kind: "scheduled",
    date: "2026-07-19",
    evening: true,
  });
...
test("retained normalized rows reassemble into the same portable legacy import", () => {
  const serialized = assembleLegacyWorkspace({
    ...
    spaces: [
      {
        entityId: "space-work",
        data: JSON.stringify({ title: "Work", color: "#5577dd", pinned: true, order: 0 }),
      },
    ],
    areas: [
      {
        entityId: "area-client",
        data: JSON.stringify({
          title: "Client",
          spaceId: "space-work",
          color: "#448866",
    ...
    headings: [
      {
        entityId: "heading-final",
        data: JSON.stringify({ title: "Final", projectId: "project-launch", order: 0 }),
      },
    ],
    calendarEvents: [],
    tasks: [
      {
        entityId: "todo-review",
        data: JSON.stringify({
          title: "Review",
          headingId: "heading-final",
          projectId: "project-launch",
          areaId: "area-client",
          spaceId: "space-work",
          status: "open",
    ...
    checklistItems: [
      {
        ...
        taskId: "todo-review",
        position: "000000",
        data: JSON.stringify({ title: "Read copy", done: true }),
      },
    ],
  });
  ...
  assert.deepEqual(workspace.read().toDos[0].location, {
    kind: "heading",
    headingId: "heading-final",
  });
...
test("retained data merges into an existing replacement snapshot exactly once", () => {
  ...
  legacy.toDos.push({
    ...
    notes: "",
    checklist: [],
    location: { kind: "unfiled", spaceId: "space-legacy" },
    schedule: { kind: "inbox" },
    reminder: null,
    deadline: null,
    outcome: "open",
  ...
  assert.deepEqual(merged.document.toDos.map((toDo) => toDo.title).sort(), [
    "Replacement-only work",
    "Retained work",
  ]);
  assert.deepEqual(merged.document.spaces.map((space) => space.id).sort(), [
    "space-legacy",
    "space-new",
  ]);
...
test("legacy repetition keeps one meaningful Outcome for a duplicated Template date", () => {
  ...
  const backup = {
    ...
    tasks: [
      ...
      },
      {
        ...
        loggedAt: "2026-07-15T11:00:00.000Z",
        order: 1,
        repeat: null,
        repeatTemplateId: "todo-repeat",
      },
    ],
  };
  ...
  assert.ok(
    result.report.messages.some((message) =>
      message.message.includes("duplicate Repeating occurrence"),
    ),
  );
});
    ...
    tasks: [
      {
        ...
        spaceId: "space-home",
        projectId: "project-clean",
        headingId: "heading-kitchen",
        areaId: null,
        reminderAt: "2026-07-20T08:30:00.000Z",
        deadline: "2026-07-21",
        tags: ["Home"],
        checklist: [{ id: "check-spray", title: "Use spray", done: false }],
        repeat: null,
        createdAt: "2026-07-19T08:00:00.000Z",
        completedAt: null,
        order: 0,
    ...
    toDos: [
      {
        ...
        title: "Wipe counters",
        notes: "",
        headingKey: "heading-kitchen",
        tags: [workspace.read().tags[0].id],
        checklist: [{ title: "Use spray", completed: false, order: 0 }],
        schedule: { kind: "anytime" },
        reminder: { kind: "offset", days: 0, time: "08:30" },
        deadline: { kind: "fixed", date: "2026-07-21" },
        order: 0,
      },
    ],
  });
