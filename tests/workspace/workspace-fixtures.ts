...
export function representativeWorkspace(prefix: string): WorkspaceDocument {
  ...
  const workspace = createWorkspace(document, {
    now: () => FIXTURE_NOW,
    createId: (kind: WorkspaceEntityKind | "projectClosure" | "undo") =>
  ...
  const personalId = changedId(
    workspace.change({ type: "createSpace", title: `${prefix} Personal`, color: "#e49b3c" }),
    "space",
  );
  const workId = changedId(
    workspace.change({ type: "createSpace", title: `${prefix} Work`, color: "#2f80ed" }),
    "space",
  );
  workspace.change({ type: "setDefaultSpace", spaceId: personalId });
  const tagId = changedId(
    workspace.change({ type: "createTag", title: `${prefix} Important` }),
    "tag",
  );
  ...
  changedId(
    workspace.change({
      type: "createToDo",
      title: `${prefix} Project child`,
      location: { kind: "project", projectId },
    }),
    "toDo",
  );
  const completedId = changedId(
    workspace.change({
      type: "createToDo",
  ...
  const trashedId = changedId(
    workspace.change({
      ...
      location: { kind: "unfiled", spaceId: personalId },
    }),
    "toDo",
  );
  workspace.change({ type: "trashToDo", id: trashedId });
  const repeatingId = changedId(
    workspace.change({
      type: "createToDo",
      title: `${prefix} Weekly`,
      location: { kind: "unfiled", spaceId: personalId },
  ...
  const repeatingProjectId = changedId(
    workspace.change({
      type: "createProject",
      title: `${prefix} Monthly project`,
      location: { kind: "space", spaceId: personalId },
    }),
    "project",
  );
  const repeatingHeadingId = changedId(
    workspace.change({
      type: "createHeading",
      title: `${prefix} Monthly steps`,
      location: { kind: "project", projectId: repeatingProjectId },
    }),
    "heading",
  );
  changedId(
    workspace.change({
      type: "createToDo",
      title: `${prefix} Monthly child`,
      location: { kind: "heading", headingId: repeatingHeadingId },
      checklist: ["Prepare"],
    }),
    "toDo",
  );
