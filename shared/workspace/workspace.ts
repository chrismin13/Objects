...
export type ToDoChanges = Partial<
  Pick<ToDo, "title" | "notes" | "location" | "schedule" | "reminder" | "deadline" | "tags">
>;
export type ProjectChanges = Partial<
  Pick<Project, "title" | "notes" | "location" | "schedule" | "deadline" | "tags">
>;
...
export type CalendarEventChanges = Partial<
  Pick<CalendarEvent, "title" | "spaceId" | "start" | "end" | "calendar" | "allDay">
>;
export type ApplicationSettingsChanges = Partial<
  Pick<
...
export type RepeatingTemplateChanges = Partial<
  Pick<
    RepeatingTemplate,
    | "title"
    | "notes"
...
export type WorkspaceChange =
  ...
  | {
      ...
      toDoOutcomes: Array<{ id: EntityId; outcome: Exclude<Outcome, "open"> }>;
    }
  ...
  | {
      ...
      tags?: EntityId[];
      checklist?: string[];
      quickEntry?: { referenceDate: string };
    }
  ...
  | {
      ...
      toDoId: EntityId;
      itemId: EntityId;
      changes: Partial<Pick<ChecklistItem, "title" | "completed">>;
    }
  ...
  | {
      ...
      movedIds: EntityId[];
      orderedIds: EntityId[];
      destination?: { location?: ToDoLocation; schedule?: Schedule };
    }
  | {
...
export function nextRepeatingDate(
  currentDate: string,
  pattern: RepeatingPattern,
  firstDate = currentDate,
): string {
  if (
    !isIsoDate(currentDate) ||
    !isIsoDate(firstDate) ||
...
export function repeatingRuleSummary(
  pattern: RepeatingPattern,
  mode: RepeatingTemplate["mode"],
): string {
  ...
  const days =
    pattern.frequency === "weekly" && pattern.weekdays.length
      ? ` on ${[...new Set(pattern.weekdays)]
          .sort()
...
function parseQuickEntry(value: string, referenceDate: string): ParsedQuickEntry {
  ...
  const deadlineMatch =
    /\bdue\s+(today|tomorrow|(?:next\s+)?(?:sunday|monday|tuesday|wednesday|thursday|friday|saturday)|\d{4}-\d{2}-\d{2})\b/i.exec(
      title,
    );
  if (deadlineMatch) {
  ...
  const scheduleMatch =
    /\b(this evening|today|tomorrow|someday|anytime|inbox|(?:next\s+)?(?:sunday|monday|tuesday|wednesday|thursday|friday|saturday)|\d{4}-\d{2}-\d{2})\b/i.exec(
      title,
    );
  if (scheduleMatch) {
    const phrase = scheduleMatch[1].toLowerCase();
    if (phrase === "someday" || phrase === "anytime" || phrase === "inbox")
      schedule = { kind: phrase } as Schedule;
    else {
      const date =
        phrase === "today" || phrase === "this evening"
          ? referenceDate
          : phrase === "tomorrow"
            ? addDays(referenceDate, 1)
            : /^\d{4}/.test(phrase)
...
export function createWorkspace(
  ...
  document.captureReceipts = Array.isArray(document.captureReceipts)
  ...
  function rejectAs(
    outcome: "validation-failed" | "not-found" | "confirmation-required",
    errors: string[],
  ): WorkspaceChangeResult {
  ...
  function reorderedCompleteCollection<T extends { id: EntityId; order: number }>(
    items: T[],
  ...
  function projectContentsErrors(contents: RepeatingProjectContents): string[] {
    ...
    for (const heading of contents.headings) {
      if (!heading.key || headingKeys.has(heading.key))
        errors.push("Project Heading identities must be present and unique.");
    ...
    for (const toDo of contents.toDos) {
      if (!toDo.key || toDoKeys.has(toDo.key))
        errors.push("Project to-do identities must be present and unique.");
      if (!toDo.title.trim()) errors.push("Project to-do titles cannot be empty.");
      if (toDo.headingKey && !headingKeys.has(toDo.headingKey))
        errors.push("A Project to-do points to a missing Heading.");
      if (toDo.tags.some((tagId) => !document.tags.some((tag) => tag.id === tagId)))
        errors.push("A Project to-do uses a Tag that no longer exists.");
      if (toDo.checklist.some((item) => !item.title.trim()))
        errors.push("Project to-do checklist titles cannot be empty.");
      if (toDo.schedule?.kind === "scheduled" && !Number.isInteger(toDo.schedule.offsetDays))
        errors.push("Project to-do Schedule offsets must be whole numbers.");
      if (toDo.reminder?.kind === "at-time" && !isTime(toDo.reminder.time))
        errors.push("Choose a valid Project to-do Reminder time.");
      if (
        toDo.reminder?.kind === "offset" &&
        (!Number.isInteger(toDo.reminder.days) || !isTime(toDo.reminder.time))
      )
        errors.push("Project to-do Reminder offsets must use a whole number and valid time.");
      if (toDo.reminder?.kind === "fixed" && !isIsoDateTime(toDo.reminder.at))
        errors.push("Choose a valid fixed Project to-do Reminder.");
      if (toDo.deadline?.kind === "offset" && !Number.isInteger(toDo.deadline.days))
        errors.push("Project to-do Deadline offsets must be whole numbers.");
      if (toDo.deadline?.kind === "fixed" && !isIsoDate(toDo.deadline.date))
        errors.push("Choose a valid fixed Project to-do Deadline.");
      toDoKeys.add(toDo.key);
    }
    return [...new Set(errors)];
  }

  function createToDoOccurrence(
    template: Extract<RepeatingTemplate, { itemKind: "toDo" }>,
    ...
    return {
      ...
      reminder: template.reminderTime
        ? { at: `${scheduledDate}T${template.reminderTime}:00.000Z`, sentAt: null }
        : null,
      deadline:
        template.deadlineOffsetDays === null
          ? null
          : addDays(scheduledDate, template.deadlineOffsetDays),
      outcome: "open",
      trashedAt: null,
      logbookAt: null,
      tags: [...template.tags],
  ...
  function projectContentsFromProject(
    projectId: EntityId,
    firstDate: string,
  ): RepeatingProjectContents {
    const headings = document.headings
      .filter(
        (heading) =>
          heading.location.kind === "project" && heading.location.projectId === projectId,
      )
      .map((heading) => ({
        key: heading.id,
        title: heading.title,
    ...
    const toDos = projectToDos(projectId).map((toDo) => ({
      ...
      schedule:
        toDo.schedule.kind === "scheduled"
          ? {
              ...
              offsetDays: daysBetween(firstDate, toDo.schedule.date),
      ...
      deadline: toDo.deadline
        ? { kind: "offset" as const, days: daysBetween(firstDate, toDo.deadline) }
        : null,
      order: toDo.order,
    }));
  ...
  function createProjectOccurrence(
    template: Extract<RepeatingTemplate, { itemKind: "project" }>,
    scheduledDate: string,
  ): AffectedEntity[] {
    ...
    const toDos = template.projectContents.toDos.map((blueprint, index): ToDo => {
      const schedule: Schedule =
        blueprint.schedule?.kind === "scheduled"
          ? {
              kind: "scheduled",
              date: addDays(scheduledDate, blueprint.schedule.offsetDays),
              evening: blueprint.schedule.evening,
            }
      ...
      return {
        ...
        location:
          blueprint.headingKey && headingIds.has(blueprint.headingKey)
            ? { kind: "heading", headingId: headingIds.get(blueprint.headingKey)! }
            : { kind: "project", projectId: project.id },
        schedule,
        reminder:
          blueprint.reminder?.kind === "at-time"
            ? { at: `${reminderDate}T${blueprint.reminder.time}:00.000Z`, sentAt: null }
            : blueprint.reminder?.kind === "offset"
              ? {
                  at: `${addDays(scheduledDate, blueprint.reminder.days)}T${blueprint.reminder.time}:00.000Z`,
                  sentAt: null,
                }
              : blueprint.reminder?.kind === "fixed"
                ? { at: blueprint.reminder.at, sentAt: null }
                : null,
        deadline:
          blueprint.deadline?.kind === "offset"
            ? addDays(scheduledDate, blueprint.deadline.days)
            : blueprint.deadline?.kind === "fixed"
              ? blueprint.deadline.date
              : null,
        outcome: "open",
        trashedAt: null,
        logbookAt: null,
  ...
  function occurrenceExists(template: RepeatingTemplate, scheduledDate: string): boolean {
    ...
    return collection.some(
      (item) =>
        item.occurrence?.templateId === template.id &&
        item.occurrence.scheduledDate === scheduledDate,
    );
  }

  function advanceAfterCompletionSchedule(item: ToDo | Project): RepeatingTemplate | null {
    if (!item.occurrence) return null;
    const template = document.repeatingTemplates.find(
      (candidate) => candidate.id === item.occurrence!.templateId,
    );
    if (!template || template.mode !== "after-completion" || template.state === "stopped")
      return null;
  ...
  function locationExists(location: ToDoLocation): boolean {
    if (location.kind === "unfiled")
      return document.spaces.some((space) => space.id === location.spaceId);
    if (location.kind === "area") return document.areas.some((area) => area.id === location.areaId);
    if (location.kind === "project")
      return document.projects.some((project) => project.id === location.projectId);
    ...
    if (projectId) {
      ...
      if (project.location.kind === "area") areaId = project.location.areaId;
        ...
        return {
          ...(headingId ? { headingId } : {}),
          projectId,
          spaceId: project.location.spaceId,
        };
    }
  ...
  function spaceIdForView(view: WorkspaceView): EntityId | null {
    ...
    if (view.kind === "area")
      return document.areas.find((area) => area.id === view.id)?.spaceId ?? null;
  ...
  function effectiveTagIdsForToDo(id: EntityId): EntityId[] {
    ...
    const areaTags = location.areaId
      ? (document.areas.find((area) => area.id === location.areaId)?.tags ?? [])
      : [];
    const projectTags = location.projectId
      ? (document.projects.find((project) => project.id === location.projectId)?.tags ?? [])
      : [];
  ...
  function repeatingPreviewDates(
    template: RepeatingTemplate,
    fromDate: string,
    ...
    if (
      ...
      !patternIsValid(template.pattern) ||
      !isIsoDate(template.nextDate)
    )
      return [];
    const collection = template.itemKind === "toDo" ? document.toDos : document.projects;
    const hasOpenAfterCompletionOccurrence =
      template.mode === "after-completion" &&
      collection.some(
        (item) =>
          item.occurrence?.templateId === template.id && item.outcome === "open" && !item.trashedAt,
      );
    ...
    for (let guard = 0; guard < REPEATING_BATCH_LIMIT && dates.length < limit; guard += 1) {
      ...
      const next = nextRepeatingDate(
        date,
        template.pattern,
        template.firstDate ?? template.nextDate,
      );
      if (next <= date) break;
      date = next;
    }
  ...
  function repeatingPreviews(
    fromDate: string,
    throughDate: string,
    limit = REPEATING_BATCH_LIMIT,
  ): RepeatingPreview[] {
    if (
      !isIsoDate(fromDate) ||
      !isIsoDate(throughDate) ||
    ...
    for (const template of document.repeatingTemplates) {
      ...
      previews.push(
        ...repeatingPreviewDates(template, fromDate, throughDate, remaining).map((date) =>
          repeatingPreview(template, date),
        ),
      );
  ...
  function validate(): string[] {
    const errors: string[] = [];
    if (!(["system", "light", "dark"] as unknown[]).includes(document.settings.theme))
      errors.push("Workspace appearance must be System, Light, or Dark.");
    if (typeof document.settings.groupToday !== "boolean")
      errors.push("Group Today must be on or off.");
    if (typeof document.settings.notifications !== "boolean")
      errors.push("Notifications must be on or off.");
    if (document.settings.weekStartsOn !== 0 && document.settings.weekStartsOn !== 1)
      errors.push("The first weekday must be Sunday or Monday.");
    if (typeof document.settings.showCalendar !== "boolean")
      errors.push("Calendar visibility must be on or off.");
    if (
      !(["immediately", "daily", "manually"] as unknown[]).includes(
        document.settings.logCompletedItems,
      )
    )
      errors.push("The Logbook policy is invalid.");
    if (!Array.isArray(document.settings.launchRules))
      errors.push("Workspace launch rules are malformed.");
    if (document.settings.quickDraft && typeof document.settings.quickDraft.value !== "string")
      errors.push("The unfinished quick entry is malformed.");
    ...
    const collections: Array<[string, Array<{ id: string }>]> = [
      ...
      ["Tag", document.tags],
      ["To-do", document.toDos],
      ["Repeating Template", document.repeatingTemplates],
      ["Calendar Event", document.calendarEvents],
    ];
    ...
    for (const area of document.areas) {
      if (!document.spaces.some((space) => space.id === area.spaceId))
        errors.push(`Area “${area.id}” has no valid Space.`);
      for (const tagId of area.tags)
        if (!document.tags.some((tag) => tag.id === tagId))
          errors.push(`Area “${area.id}” has an unknown Tag.`);
    }
    if (
      document.settings.defaultSpaceId &&
      !document.spaces.some((space) => space.id === document.settings.defaultSpaceId)
    ) {
      errors.push("The default Space does not exist.");
    }
    for (const rule of Array.isArray(document.settings.launchRules)
      ? document.settings.launchRules
      : []) {
      if (!document.spaces.some((space) => space.id === rule.spaceId))
        errors.push(`Launch rule “${rule.id}” has no valid Space.`);
      if (!isTime(rule.start) || !isTime(rule.end))
        errors.push(`Launch rule “${rule.id}” has an invalid time.`);
      if (rule.weekdays.some((day) => !Number.isInteger(day) || day < 0 || day > 6))
        errors.push(`Launch rule “${rule.id}” has an invalid weekday.`);
    }
    const receiptIds = new Set<string>();
    for (const receipt of document.captureReceipts) {
      if (!receipt.submissionId || receiptIds.has(receipt.submissionId))
        errors.push("A capture submission identity is missing or duplicated.");
      receiptIds.add(receipt.submissionId);
      if (
        !document.toDos.some((toDo) => toDo.id === receipt.toDoId) &&
        !document.permanentDeletions.some(
          (marker) => marker.entityKind === "toDo" && marker.entityId === receipt.toDoId,
        )
      ) {
        errors.push(`Capture receipt “${receipt.submissionId}” points to a missing to-do.`);
      }
      if (!isIsoDateTime(receipt.createdAt))
        errors.push(`Capture receipt “${receipt.submissionId}” has an invalid creation date.`);
    }
    if (
      document.settings.quickDraft?.updatedAt &&
      !isIsoDateTime(document.settings.quickDraft.updatedAt)
    ) {
    ...
    const validateSchedule = (schedule: unknown, label: string) => {
      ...
      if (
        ...
        (!isIsoDate(candidate.date) || typeof candidate.evening !== "boolean")
      ) {
    ...
    for (const project of document.projects) {
      ...
      if (!["open", "completed", "canceled"].includes(String(project.outcome)))
        errors.push(`Project “${project.id}” has an invalid Outcome.`);
      if (project.deadline && !isIsoDate(project.deadline))
        errors.push(`Project “${project.id}” has an invalid Deadline.`);
      if (project.completedAt && !isIsoDateTime(project.completedAt))
        errors.push(`Project “${project.id}” has an invalid completion date.`);
      if (project.trashedAt && !isIsoDateTime(project.trashedAt))
        errors.push(`Project “${project.id}” has an invalid Trash date.`);
      if (project.logbookAt && !isIsoDateTime(project.logbookAt))
        errors.push(`Project “${project.id}” has an invalid Logbook date.`);
      for (const tagId of project.tags)
        if (!document.tags.some((tag) => tag.id === tagId))
          errors.push(`Project “${project.id}” has an unknown Tag.`);
    }
    for (const heading of document.headings) {
      ...
      if (heading.archivedAt && !isIsoDateTime(heading.archivedAt))
        errors.push(`Heading “${heading.id}” has an invalid archive date.`);
    }
    for (const toDo of document.toDos) {
      ...
      if (!["open", "completed", "canceled"].includes(String(toDo.outcome)))
        errors.push(`To-do “${toDo.id}” has an invalid Outcome.`);
      if (!isIsoDateTime(toDo.createdAt))
        errors.push(`To-do “${toDo.id}” has an invalid creation date.`);
      if (toDo.deadline && !isIsoDate(toDo.deadline))
        errors.push(`To-do “${toDo.id}” has an invalid Deadline.`);
      if (
        toDo.reminder &&
        (!isIsoDateTime(toDo.reminder.at) ||
          (toDo.reminder.sentAt && !isIsoDateTime(toDo.reminder.sentAt)))
      ) {
        errors.push(`To-do “${toDo.id}” has an invalid Reminder.`);
      }
      if (toDo.completedAt && !isIsoDateTime(toDo.completedAt))
        errors.push(`To-do “${toDo.id}” has an invalid completion date.`);
      if (toDo.trashedAt && !isIsoDateTime(toDo.trashedAt))
        errors.push(`To-do “${toDo.id}” has an invalid Trash date.`);
      if (toDo.logbookAt && !isIsoDateTime(toDo.logbookAt))
        errors.push(`To-do “${toDo.id}” has an invalid Logbook date.`);
      for (const tagId of toDo.tags)
        if (!document.tags.some((tag) => tag.id === tagId))
          errors.push(`To-do “${toDo.id}” has an unknown Tag.`);
    }
    for (const template of document.repeatingTemplates) {
      ...
      if (!hasValidLocation)
        errors.push(`Repeating Template “${template.id}” has no valid Location.`);
      if (template.firstDate && !isIsoDate(template.firstDate))
        errors.push(`Repeating Template “${template.id}” has an invalid first date.`);
      if (!isIsoDate(template.nextDate))
        errors.push(`Repeating Template “${template.id}” has an invalid next date.`);
      if (!isIsoDateTime(template.createdAt))
        errors.push(`Repeating Template “${template.id}” has an invalid creation date.`);
      if (!["daily", "weekly", "monthly", "yearly"].includes(template.pattern.frequency))
        errors.push(`Repeating Template “${template.id}” has an invalid frequency.`);
      if (!Number.isInteger(template.pattern.interval) || template.pattern.interval < 1)
        errors.push(`Repeating Template “${template.id}” has an invalid interval.`);
      if (template.pattern.weekdays.some((day) => !Number.isInteger(day) || day < 0 || day > 6))
        errors.push(`Repeating Template “${template.id}” has an invalid weekday.`);
      if (!["on-schedule", "after-completion"].includes(template.mode))
        errors.push(`Repeating Template “${template.id}” has an invalid mode.`);
      if (!["active", "paused", "stopped"].includes(template.state))
        errors.push(`Repeating Template “${template.id}” has an invalid state.`);
      if (template.reminderTime && !isTime(template.reminderTime))
        errors.push(`Repeating Template “${template.id}” has an invalid reminder time.`);
      if (template.deadlineOffsetDays !== null && !Number.isInteger(template.deadlineOffsetDays))
        errors.push(`Repeating Template “${template.id}” has an invalid Deadline offset.`);
      for (const tagId of template.tags)
        if (!document.tags.some((tag) => tag.id === tagId))
          errors.push(`Repeating Template “${template.id}” has an unknown Tag.`);
      const projectContents = template.projectContents as RepeatingProjectContents | null;
      if (template.itemKind === "project" && !projectContents)
        errors.push(`Repeating Project Template “${template.id}” has no Project contents.`);
      if (template.itemKind === "toDo" && projectContents)
        errors.push(`Repeating to-do Template “${template.id}” cannot contain Project contents.`);
      if (projectContents) {
        const headingKeys = new Set(projectContents.headings.map((heading) => heading.key));
        for (const toDo of projectContents.toDos) {
          if (toDo.headingKey && !headingKeys.has(toDo.headingKey))
            errors.push(
              `Repeating Project Template “${template.id}” has a to-do with an unknown Heading.`,
            );
          if (toDo.reminder?.kind === "at-time" && !isTime(toDo.reminder.time))
            errors.push(
              `Repeating Project Template “${template.id}” has an invalid child Reminder.`,
            );
          if (
            toDo.reminder?.kind === "offset" &&
            (!Number.isInteger(toDo.reminder.days) || !isTime(toDo.reminder.time))
            ...
            errors.push(
              `Repeating Project Template “${template.id}” has an invalid child Reminder offset.`,
            );
          if (toDo.reminder?.kind === "fixed" && !isIsoDateTime(toDo.reminder.at))
            errors.push(
              `Repeating Project Template “${template.id}” has an invalid child Reminder.`,
          ...
          if (toDo.deadline?.kind === "offset" && !Number.isInteger(toDo.deadline.days))
            errors.push(
              `Repeating Project Template “${template.id}” has an invalid child Deadline.`,
            );
          if (toDo.deadline?.kind === "fixed" && !isIsoDate(toDo.deadline.date))
            errors.push(
              `Repeating Project Template “${template.id}” has an invalid child Deadline.`,
          ...
          if (toDo.schedule?.kind === "scheduled" && !Number.isInteger(toDo.schedule.offsetDays))
            errors.push(
              `Repeating Project Template “${template.id}” has an invalid child Schedule offset.`,
            );
        }
      }
    }
    for (const toDo of document.toDos) {
      ...
      const template = document.repeatingTemplates.find(
        (item) => item.id === toDo.occurrence!.templateId,
      );
      if (!template || template.itemKind !== "toDo")
        errors.push(`To-do “${toDo.id}” has no valid Repeating Template.`);
      if (!isIsoDate(toDo.occurrence.scheduledDate))
        errors.push(`To-do “${toDo.id}” has an invalid Occurrence date.`);
    }
    for (const project of document.projects) {
      if (!project.occurrence) continue;
      const template = document.repeatingTemplates.find(
        (item) => item.id === project.occurrence!.templateId,
      );
      if (!template || template.itemKind !== "project")
        errors.push(`Project “${project.id}” has no valid Repeating Template.`);
      if (!isIsoDate(project.occurrence.scheduledDate))
        errors.push(`Project “${project.id}” has an invalid Occurrence date.`);
    }
    for (const event of document.calendarEvents) {
      if (!document.spaces.some((space) => space.id === event.spaceId))
        errors.push(`Calendar event “${event.id}” has no valid Space.`);
      if (typeof event.allDay !== "boolean")
        errors.push(`Calendar event “${event.id}” has an invalid all-day setting.`);
      if (
        !isIsoDateTime(event.start) ||
        !isIsoDateTime(event.end) ||
    ...
    for (const closure of document.projectClosures) {
      if (!document.projects.some((project) => project.id === closure.projectId))
        errors.push(`Project Closure “${closure.id}” has no valid Project.`);
      if (!isIsoDateTime(closure.closedAt))
        errors.push(`Project Closure “${closure.id}” has an invalid date.`);
      for (const toDoId of closure.changedToDoIds)
        if (!document.toDos.some((toDo) => toDo.id === toDoId))
          errors.push(`Project Closure “${closure.id}” has an unknown to-do.`);
    }
    for (const marker of document.permanentDeletions) {
      if (
        ![
          ...
          "repeatingTemplate",
          "calendarEvent",
        ].includes(String(marker.entityKind))
      ) {
        errors.push(
          `Permanent-deletion marker for “${marker.entityId}” has an invalid entity kind.`,
        );
      }
      if (!isIsoDateTime(marker.deletedAt))
        errors.push(`Permanent-deletion marker for “${marker.entityId}” has an invalid date.`);
    }
    if (!isIsoDateTime(document.sync.updatedAt))
      errors.push("Workspace sync metadata has an invalid update date.");
    if (
  ...
  return {
    ...
    view(view) {
      const matchesTagFilter = (item: ToDo | Project): boolean => {
        ...
        const effectiveTags =
          "checklist" in item
            ? effectiveTagIdsForToDo(item.id)
            : effectiveTagIdsForProject(item.id);
      ...
      if (view.kind === "deadlines") {
        return [...document.toDos, ...document.projects]
          ...
          .sort(
            (left, right) =>
              left.deadline!.localeCompare(right.deadline!) || left.order - right.order,
          )
      ...
      if (view.kind === "logbook") {
        return [...document.projects, ...document.toDos]
          .filter(
            (item) => item.logbookAt !== null && item.trashedAt === null && matchesTagFilter(item),
          )
          .sort(
            (left, right) =>
              left.logbookAt!.localeCompare(right.logbookAt!) || left.order - right.order,
          )
          .map((item) => copyDocument(item));
      }
      return document.toDos
        .filter((toDo) => {
          ...
          const parentProject = parentProjectId
            ? document.projects.find((project) => project.id === parentProjectId)
            : null;
          if (parentProject?.trashedAt || parentProject?.logbookAt) return false;
          if (toDo.trashedAt || toDo.logbookAt) return false;
          if (view.kind === "today")
            return toDo.schedule.kind === "scheduled" && toDo.schedule.date <= view.date;
          if (view.kind === "thisEvening")
            return (
              toDo.schedule.kind === "scheduled" &&
              toDo.schedule.date === view.date &&
              toDo.schedule.evening
            );
          if (view.kind === "tomorrow")
            return toDo.schedule.kind === "scheduled" && toDo.schedule.date === view.date;
    ...
    importPortableBackup(serialized, confirmation) {
      ...
      if (!parsed.ok) {
        return {
          ...
          affected: [],
          undo: null,
          errors: parsed.report.messages
            .filter((message) => message.kind === "rejected")
      ...
      if (errors.length) {
        ...
        parsed.report.messages.push(
          ...errors.map((message) => ({ kind: "rejected" as const, message })),
        );
        return {
          status: "rejected",
          outcome: "import-rejected",
      ...
      const affected: AffectedEntity[] = [
        ...
        ...document.repeatingTemplates.map((item) => ({
          kind: "repeatingTemplate" as const,
          id: item.id,
        })),
    ...
    change(change) {
      ...
      if (change.type === "updateSettings") {
        ...
        if (changes.theme !== undefined && !["system", "light", "dark"].includes(changes.theme))
          return fail(["Choose System, Light, or Dark appearance."]);
        if (
          changes.weekStartsOn !== undefined &&
          changes.weekStartsOn !== 0 &&
          changes.weekStartsOn !== 1
        )
          return fail(["Choose Sunday or Monday as the first day of the week."]);
        if (changes.theme !== undefined) document.settings.theme = changes.theme;
        if (changes.groupToday !== undefined)
          document.settings.groupToday = Boolean(changes.groupToday);
        if (changes.notifications !== undefined)
          document.settings.notifications = Boolean(changes.notifications);
        if (changes.weekStartsOn !== undefined)
          document.settings.weekStartsOn = changes.weekStartsOn;
        if (changes.showCalendar !== undefined)
          document.settings.showCalendar = Boolean(changes.showCalendar);
        return finishChange(previous, "application-settings-updated", []);
      }
      if (change.type === "setDefaultSpace") {
        if (!document.spaces.some((space) => space.id === change.spaceId))
          return failAs("not-found", ["The selected default Space no longer exists."]);
        document.settings.defaultSpaceId = change.spaceId;
        return finishChange(previous, "default-space-updated", [
          { kind: "space", id: change.spaceId },
        ]);
      }
      if (change.type === "replaceLaunchRules") {
        ...
        for (const rule of change.rules) {
          if (!rule.id || ruleIds.has(rule.id))
            return fail(["Every launch rule needs a unique identity."]);
          ruleIds.add(rule.id);
          if (!document.spaces.some((space) => space.id === rule.spaceId))
            return fail(["Every launch rule must use an available Space."]);
          if (
            !rule.weekdays.length ||
            rule.weekdays.some((day) => !Number.isInteger(day) || day < 0 || day > 6)
          )
            return fail(["Choose at least one valid weekday for every launch rule."]);
          if (!isTime(rule.start) || !isTime(rule.end) || rule.start === rule.end)
            return fail(["Choose a valid time range for every launch rule."]);
        }
      ...
      if (change.type === "createCalendarEvent") {
        ...
        if (!title || title.length > 500)
          return fail(["Enter a calendar event title between 1 and 500 characters."]);
        if (!document.spaces.some((space) => space.id === change.spaceId))
          return fail(["Choose an available Space for this calendar event."]);
        if (
          !isIsoDateTime(change.start) ||
          !isIsoDateTime(change.end) ||
          Date.parse(change.end) < Date.parse(change.start) ||
        ...
        return finishChange(
          ...
          [{ kind: "calendarEvent", id }],
          "Undo new calendar event",
        );
      }
      if (change.type === "updateCalendarEvent") {
        ...
        if (change.changes.spaceId !== undefined) {
          if (!document.spaces.some((space) => space.id === change.changes.spaceId))
            return fail(["Choose an available Space for this calendar event."]);
        ...
        if (change.changes.calendar !== undefined)
          event.calendar = change.changes.calendar.trim() || "Objects";
        ...
        return finishChange(
          ...
          [{ kind: "calendarEvent", id: event.id }],
          "Undo calendar event changes",
        );
      }
      if (change.type === "deleteCalendarEvent") {
        ...
        return finishChange(
          ...
          [{ kind: "calendarEvent", id: change.id }],
          "Undo deleted calendar event",
        );
      }
      if (change.type === "importIcsCalendar") {
        if (!document.spaces.some((space) => space.id === change.spaceId))
          return fail(["Choose an available Space for imported calendar events."]);
        const parsed = parseIcsCalendar(change.source);
        if (parsed.status === "rejected") return fail(parsed.errors);
        const duplicate = parsed.events.find((candidate) =>
          document.calendarEvents.some((event) =>
            candidate.uid
      ...
      if (change.type === "createRepeatingTemplate") {
        ...
        if (!title || title.length > 500)
          return fail(["Enter a Repeating Template title between 1 and 500 characters."]);
        ...
        if (draft.reminderTime && !isTime(draft.reminderTime))
          return fail(["Choose a valid Reminder time."]);
        if (
          draft.deadlineOffsetDays !== undefined &&
          draft.deadlineOffsetDays !== null &&
          !Number.isInteger(draft.deadlineOffsetDays)
        )
          return fail(["Choose a whole-number Deadline offset."]);
        const tags = [...new Set(draft.tags ?? [])];
        if (tags.some((tagId) => !document.tags.some((tag) => tag.id === tagId)))
          return fail(["A selected Tag no longer exists."]);
        if (draft.itemKind === "project") {
          const errors = projectContentsErrors(
            draft.projectContents ?? { headings: [], toDos: [] },
          );
        ...
        if (base.checklist.some((item) => !item.title))
          return fail(["Checklist item titles cannot be empty."]);
        const template: RepeatingTemplate =
          draft.itemKind === "toDo"
            ? {
                ...base,
                itemKind: "toDo",
                location: copyDocument(draft.location),
            ...
            : {
                ...
                itemKind: "project",
                location: copyDocument(draft.location),
                projectContents: copyDocument(draft.projectContents ?? { headings: [], toDos: [] }),
              };
        if (
          template.itemKind === "toDo"
            ? !locationExists(template.location)
            : !projectLocationExists(template.location)
        )
          return fail(["The selected Repeating Template Location no longer exists."]);
        document.repeatingTemplates.push(template);
        return finishChange(
          previous,
          "repeating-template-created",
          [{ kind: "repeatingTemplate", id }],
          `Delete “${title}”`,
        );
      }

      if (change.type === "generateRepeatingOccurrences") {
        if (!isIsoDate(change.throughDate))
          return fail(["Choose a valid date for generating Occurrences."]);
        const requestedSize = change.batchSize ?? REPEATING_BATCH_LIMIT;
        if (!Number.isInteger(requestedSize) || requestedSize < 1)
          return fail(["Choose a positive batch size."]);
        const batchSize = Math.min(requestedSize, REPEATING_BATCH_LIMIT);
        const affected: AffectedEntity[] = [];
        let generated = 0;
        for (const template of document.repeatingTemplates) {
          if (template.state !== "active") continue;
          if (!patternIsValid(template.pattern) || !isIsoDate(template.nextDate))
            return fail([`Repeating Template “${template.title}” has a damaged rule.`]);
          for (
            let step = 0;
            template.nextDate <= change.throughDate &&
            generated < batchSize &&
            step < REPEATING_BATCH_LIMIT;
            step += 1
          ) {
            const scheduledDate = template.nextDate;
            const collection = template.itemKind === "toDo" ? document.toDos : document.projects;
            const hasOpenOccurrence =
              template.mode === "after-completion" &&
              collection.some(
                (item) =>
                  item.occurrence?.templateId === template.id &&
            ...
            const next = nextRepeatingDate(
              scheduledDate,
              template.pattern,
              template.firstDate ?? scheduledDate,
            );
            if (next <= scheduledDate)
              return fail([`Repeating Template “${template.title}” could not advance safely.`]);
        ...
        return finishChange(
          previous,
          generated ? "repeating-occurrences-generated" : "repeating-occurrences-current",
          affected,
        );
      }
      ...
      if (change.type === "updateRepeatingTemplate") {
        ...
        if (template.state === "stopped")
          return fail(["A stopped Repeating Template is read-only."]);
        const changes = change.changes;
        ...
        if (changes.tags !== undefined) {
          if (changes.tags.some((tagId) => !document.tags.some((tag) => tag.id === tagId)))
            return fail(["A selected Tag no longer exists."]);
        ...
        if (changes.reminderTime !== undefined) {
          if (changes.reminderTime && !isTime(changes.reminderTime))
            return fail(["Choose a valid Reminder time."]);
          template.reminderTime = changes.reminderTime;
        }
        if (changes.deadlineOffsetDays !== undefined) {
          if (changes.deadlineOffsetDays !== null && !Number.isInteger(changes.deadlineOffsetDays))
            return fail(["Choose a whole-number Deadline offset."]);
          template.deadlineOffsetDays = changes.deadlineOffsetDays;
        }
        if (changes.checklist !== undefined) {
          const checklist = changes.checklist.map(
            (title, order): ChecklistItem => ({
              id: dependencies.createId("checklistItem"),
              title: title.trim(),
              completed: false,
              order,
          ...
          if (checklist.some((item) => !item.title))
            return fail(["Checklist item titles cannot be empty."]);
        ...
        if (changes.location !== undefined) {
          if (template.itemKind === "toDo") {
            ...
            if (!locationExists(location))
              return fail(["The selected to-do Location no longer exists."]);
            template.location = copyDocument(location);
          } else {
            const location = changes.location as ProjectLocation;
            if (!projectLocationExists(location))
              return fail(["The selected Project Location no longer exists."]);
            template.location = copyDocument(location);
          }
        }
        if (changes.projectContents !== undefined) {
          if (template.itemKind !== "project" || !changes.projectContents)
            return fail(["Only a repeating Project can contain Project defaults."]);
        ...
        return finishChange(
          previous,
          "repeating-template-updated",
          [{ kind: "repeatingTemplate", id: template.id }],
          `Undo changes to “${template.title}”`,
        );
      }

      if (
        change.type === "pauseRepeatingTemplate" ||
        change.type === "resumeRepeatingTemplate" ||
        change.type === "stopRepeatingTemplate"
      ) {
        const template = document.repeatingTemplates.find((item) => item.id === change.id);
        if (!template) return failAs("not-found", ["The Repeating Template no longer exists."]);
        if (template.state === "stopped")
          return fail(["A stopped Repeating Template cannot be changed."]);
        if (change.type === "pauseRepeatingTemplate") {
          if (template.state === "paused")
            return fail(["This Repeating Template is already paused."]);
          template.state = "paused";
          return finishChange(
            previous,
            "repeating-template-paused",
            [{ kind: "repeatingTemplate", id: template.id }],
        ...
        if (change.type === "resumeRepeatingTemplate") {
          if (template.state !== "paused")
            return fail(["Only a paused Repeating Template can resume."]);
          template.state = "active";
          return finishChange(
            previous,
            "repeating-template-resumed",
            [{ kind: "repeatingTemplate", id: template.id }],
            `Pause “${template.title}”`,
          );
        }
        template.state = "stopped";
        return finishChange(previous, "repeating-template-stopped", [
          { kind: "repeatingTemplate", id: template.id },
        ]);
      }

      if (change.type === "deleteRepeatingTemplate") {
        if (change.confirmation !== DELETE_REPEATING_TEMPLATE_CONFIRMATION)
          return failAs("confirmation-required", [
            `Type ${DELETE_REPEATING_TEMPLATE_CONFIRMATION} to permanently delete this Repeating Template.`,
          ]);
        const template = document.repeatingTemplates.find((item) => item.id === change.id);
        if (!template) return failAs("not-found", ["The Repeating Template no longer exists."]);
        if (template.state !== "stopped")
          return fail(["Stop this Repeating Template before permanently deleting it."]);
        for (const toDo of document.toDos)
          if (toDo.occurrence?.templateId === template.id) toDo.occurrence = null;
        for (const project of document.projects)
          if (project.occurrence?.templateId === template.id) project.occurrence = null;
        document.repeatingTemplates = document.repeatingTemplates.filter(
          (item) => item.id !== template.id,
        );
        document.permanentDeletions.push({
          entityKind: "repeatingTemplate",
          entityId: template.id,
          deletedAt: dependencies.now(),
        });
        return finishChange(previous, "repeating-template-permanently-deleted", [
          { kind: "repeatingTemplate", id: template.id },
        ]);
      }
      if (change.type === "emptyTrash") {
        if (change.confirmation !== EMPTY_TRASH_CONFIRMATION)
          return failAs("confirmation-required", [
            `Type ${EMPTY_TRASH_CONFIRMATION} to permanently delete this Space's Trash.`,
          ]);
        if (!document.spaces.some((space) => space.id === change.spaceId))
          return failAs("not-found", ["The active Space no longer exists."]);
        const removed = document.toDos.filter(
          (item) => item.trashedAt && locationOfToDo(item.id)?.spaceId === change.spaceId,
        );
        ...
        for (const item of removed)
          document.permanentDeletions.push({ entityKind: "toDo", entityId: item.id, deletedAt });
        return finishChange(
          previous,
          "trash-emptied",
          removed.map((item) => ({ kind: "toDo", id: item.id })),
        );
      }
      ...
      if (change.type === "createSpace") {
        const title = change.title.trim();
        if (!title || title.length > 500)
          return fail(["Enter a Space title between 1 and 500 characters."]);
        const space: Space = {
          id: dependencies.createId("space"),
          title,
          color: change.color,
          pinned: change.pinned ?? true,
      ...
      if (change.type === "reorderSpace") {
        ...
        if (
          ...
          change.toIndex < 0 ||
          change.toIndex >= document.spaces.length
        )
          return fail(["Choose a valid Space position."]);
        const [space] = document.spaces.splice(currentIndex, 1);
        document.spaces.splice(change.toIndex, 0, space);
        document.spaces.forEach((item, order) => {
          item.order = order;
        });
        return finishChange(
          previous,
          "space-reordered",
          [{ kind: "space", id: space.id }],
          `Undo moving “${space.title}”`,
        );
      }
      if (change.type === "deleteSpace") {
        if (change.confirmation !== DELETE_SPACE_CONFIRMATION)
          return failAs("confirmation-required", [
            `Type ${DELETE_SPACE_CONFIRMATION} to delete this Space.`,
          ]);
        const space = document.spaces.find((item) => item.id === change.id);
        if (!space) return failAs("not-found", ["The Space no longer exists."]);
        if (change.moveToSpaceId === space.id)
          return fail(["Choose a different Space for the content."]);
        if (!document.spaces.some((item) => item.id === change.moveToSpaceId))
          return fail(["The destination Space no longer exists."]);
        ...
        for (const template of document.repeatingTemplates) {
          if (template.location.kind === "unfiled" && template.location.spaceId === space.id)
            template.location = { kind: "unfiled", spaceId: change.moveToSpaceId };
          if (template.location.kind === "space" && template.location.spaceId === space.id)
            template.location = { kind: "space", spaceId: change.moveToSpaceId };
        }
        for (const event of document.calendarEvents)
          if (event.spaceId === space.id) event.spaceId = change.moveToSpaceId;
        for (const rule of document.settings.launchRules)
          if (rule.spaceId === space.id) rule.spaceId = change.moveToSpaceId;
        if (document.settings.defaultSpaceId === space.id)
          document.settings.defaultSpaceId = change.moveToSpaceId;
      ...
      if (change.type === "createArea") {
        const title = change.title.trim();
        if (!title || title.length > 500)
          return fail(["Enter an Area title between 1 and 500 characters."]);
        if (!document.spaces.some((space) => space.id === change.spaceId))
          return fail(["The selected Space no longer exists."]);
        const tags = [...new Set(change.tags ?? [])];
        if (tags.some((tagId) => !document.tags.some((tag) => tag.id === tagId)))
          return fail(["A selected Tag no longer exists."]);
        const area: Area = {
          id: dependencies.createId("area"),
          title,
          spaceId: change.spaceId,
          color: change.color ?? "#808080",
          tags,
          order: document.areas.length,
        };
      ...
      if (change.type === "updateArea") {
        ...
        if (change.changes.tags !== undefined) {
          if (change.changes.tags.some((tagId) => !document.tags.some((tag) => tag.id === tagId)))
            return fail(["A selected Tag no longer exists."]);
      ...
      if (change.type === "reorderAreas") {
        ...
        return finishChange(
          previous,
          "areas-reordered",
          reordered.map(({ id }) => ({ kind: "area", id })),
          "Undo Area order",
        );
      }
      if (change.type === "removeArea") {
        if (change.confirmation !== REMOVE_AREA_CONFIRMATION)
          return failAs("confirmation-required", [
            `Type ${REMOVE_AREA_CONFIRMATION} to remove this Area.`,
          ]);
        const area = document.areas.find((item) => item.id === change.id);
        if (!area) return failAs("not-found", ["The Area no longer exists."]);
        const directHeadingIds = new Set(
          document.headings
            .filter(
              (heading) => heading.location.kind === "area" && heading.location.areaId === area.id,
            )
        ...
        for (const toDo of document.toDos) {
          ...
          const inRemovedHeading =
            toDo.location.kind === "heading" && directHeadingIds.has(toDo.location.headingId);
        ...
        for (const template of document.repeatingTemplates) {
          if (template.location.kind === "area" && template.location.areaId === area.id) {
            template.location =
              template.itemKind === "project"
                ? { kind: "space", spaceId: area.spaceId }
                : { kind: "unfiled", spaceId: area.spaceId };
          }
          if (
            template.itemKind === "toDo" &&
            template.location.kind === "heading" &&
            directHeadingIds.has(template.location.headingId)
          ) {
        ...
        document.headings = document.headings.filter(
          (heading) => !directHeadingIds.has(heading.id),
        );
      ...
      if (change.type === "createProject") {
        const title = change.title.trim();
        if (!title || title.length > 500)
          return fail(["Enter a Project title between 1 and 500 characters."]);
        if (!projectLocationExists(change.location))
          return fail(["The selected Project Location no longer exists."]);
        const schedule = change.schedule ?? { kind: "anytime" as const };
        if (schedule.kind === "scheduled" && !isIsoDate(schedule.date))
          return fail(["Choose a valid Schedule date."]);
        if (change.deadline && !isIsoDate(change.deadline))
          return fail(["Choose a valid Deadline."]);
        const tags = [...new Set(change.tags ?? [])];
        if (tags.some((tagId) => !document.tags.some((tag) => tag.id === tagId)))
          return fail(["A selected Tag no longer exists."]);
        const project: Project = {
      ...
      if (change.type === "updateProject") {
        ...
        if (change.changes.location !== undefined) {
          if (!projectLocationExists(change.changes.location))
            return fail(["The selected Project Location no longer exists."]);
          project.location = change.changes.location;
        }
        if (change.changes.schedule !== undefined) {
          if (
            change.changes.schedule.kind === "scheduled" &&
            !isIsoDate(change.changes.schedule.date)
          )
            return fail(["Choose a valid Schedule date."]);
          project.schedule = change.changes.schedule;
        }
        if (change.changes.deadline !== undefined) {
          if (change.changes.deadline && !isIsoDate(change.changes.deadline))
            return fail(["Choose a valid Deadline."]);
      ...
      if (change.type === "reorderProjects") {
        ...
        return finishChange(
          previous,
          "projects-reordered",
          reordered.map(({ id }) => ({ kind: "project", id })),
          "Undo Project order",
        );
      }
      if (change.type === "duplicateProject") {
        ...
        const copiedHeadings = document.headings
          .filter(
            (heading) =>
              heading.location.kind === "project" && heading.location.projectId === project.id,
          )
          .map((heading): Heading => {
            const id = dependencies.createId("heading");
            headingIds.set(heading.id, id);
            return {
              ...copyDocument(heading),
              id,
              location: { kind: "project", projectId: copy.id },
              archivedAt: null,
              order: document.headings.length + headingIds.size - 1,
            };
          });
      ...
      if (change.type === "closeProject") {
        ...
        const openToDos = projectToDos(project.id).filter(
          (toDo) => toDo.outcome === "open" && !toDo.trashedAt,
        );
        const supplied = new Map(change.toDoOutcomes.map((choice) => [choice.id, choice.outcome]));
        if (supplied.size !== change.toDoOutcomes.length)
          return fail(["Choose one Outcome for each remaining open to-do."]);
        if (
          openToDos.length !== supplied.size ||
          openToDos.some((toDo) => !supplied.has(toDo.id))
        ) {
        ...
        document.projectClosures.push({
          id: dependencies.createId("projectClosure"),
          projectId: project.id,
          projectOutcome: change.outcome,
          changedToDoIds: openToDos.map((toDo) => toDo.id),
          closedAt: now,
        });
        const repeatingTemplate =
          change.outcome === "completed" ? advanceAfterCompletionSchedule(project) : null;
        return finishChange(
          previous,
          change.outcome === "completed" ? "project-completed" : "project-canceled",
          [
            { kind: "project", id: project.id },
            ...openToDos.map((toDo) => ({ kind: "toDo" as const, id: toDo.id })),
            ...(repeatingTemplate
              ? [{ kind: "repeatingTemplate" as const, id: repeatingTemplate.id }]
              : []),
          ],
      ...
      if (change.type === "restoreProject") {
        ...
        const closure = [...document.projectClosures]
          .reverse()
      ...
      if (change.type === "restoreProjectFromTrash") {
        ...
        return finishChange(
          ...
          [{ kind: "project", id: project.id }],
          `Move “${project.title}” back to Trash`,
        );
      }
      if (change.type === "permanentlyDeleteProject") {
        if (change.confirmation !== PERMANENT_DELETE_CONFIRMATION)
          return failAs("confirmation-required", [
            `Type ${PERMANENT_DELETE_CONFIRMATION} to delete this Project.`,
          ]);
        const project = document.projects.find((item) => item.id === change.id);
        if (!project) return failAs("not-found", ["The Project no longer exists."]);
        if (!project.trashedAt)
          return fail(["Only a Project in Trash can be permanently deleted."]);
        const headings = document.headings.filter(
          (heading) =>
            heading.location.kind === "project" && heading.location.projectId === project.id,
        );
        const headingIds = new Set(headings.map((heading) => heading.id));
        const toDos = document.toDos.filter(
          (toDo) =>
        ...
        document.projectClosures = document.projectClosures.filter(
          (closure) => closure.projectId !== project.id,
        );
        document.permanentDeletions.push(
          { entityKind: "project", entityId: project.id, deletedAt },
          ...headings.map((heading) => ({
            entityKind: "heading" as const,
            entityId: heading.id,
            deletedAt,
          })),
      ...
      if (change.type === "createHeading") {
        const title = change.title.trim();
        if (!title || title.length > 500)
          return fail(["Enter a Heading title between 1 and 500 characters."]);
        if (!headingLocationExists(change.location))
          return fail(["The selected Heading Location no longer exists."]);
        const heading: Heading = {
          id: dependencies.createId("heading"),
          title,
          location: change.location,
          archivedAt: null,
          order: document.headings.length,
        };
      ...
      if (change.type === "updateHeading") {
        ...
        if (change.changes.location !== undefined) {
          if (!headingLocationExists(change.changes.location))
            return fail(["The selected Heading Location no longer exists."]);
      ...
      if (change.type === "reorderHeadings") {
        ...
        return finishChange(
          previous,
          "headings-reordered",
          reordered.map(({ id }) => ({ kind: "heading", id })),
          "Undo Heading order",
        );
      }
      if (change.type === "duplicateHeading") {
        ...
        const copiedToDos = document.toDos
          .filter(
            (toDo) => toDo.location.kind === "heading" && toDo.location.headingId === heading.id,
          )
          .map((toDo, index) =>
            duplicateToDoRecord(
              toDo,
      ...
      if (change.type === "deleteHeading") {
        if (change.confirmation !== DELETE_HEADING_CONFIRMATION)
          return failAs("confirmation-required", [
            `Type ${DELETE_HEADING_CONFIRMATION} to delete this Heading.`,
          ]);
        const heading = document.headings.find((item) => item.id === change.id);
        if (!heading) return failAs("not-found", ["The Heading no longer exists."]);
        const parentLocation: ToDoLocation =
          heading.location.kind === "project"
            ? { kind: "project", projectId: heading.location.projectId }
            : { kind: "area", areaId: heading.location.areaId };
        const children = document.toDos.filter(
          (toDo) => toDo.location.kind === "heading" && toDo.location.headingId === heading.id,
        );
        ...
        for (const template of document.repeatingTemplates) {
          if (
            template.itemKind === "toDo" &&
            template.location.kind === "heading" &&
            template.location.headingId === heading.id
          )
            template.location = parentLocation;
        }
      ...
      if (change.type === "convertHeadingToProject") {
        ...
        if (heading.location.kind === "area")
          location = { kind: "area", areaId: heading.location.areaId };
        else {
      ...
      if (change.type === "createTag") {
        ...
        if (!title || title.length > 100)
          return fail(["Enter a Tag name between 1 and 100 characters."]);
        if (document.tags.some((tag) => tag.title.toLowerCase() === title.toLowerCase()))
      ...
      if (change.type === "updateTag") {
        ...
        if (
          document.tags.some(
            (item) => item.id !== tag.id && item.title.toLowerCase() === title.toLowerCase(),
          )
        )
          return fail(["A Tag with this name already exists."]);
        tag.title = title;
      ...
      if (change.type === "reorderTags") {
        ...
        return finishChange(
          previous,
          "tags-reordered",
          reordered.map(({ id }) => ({ kind: "tag", id })),
          "Undo Tag order",
        );
      }
      if (change.type === "deleteTag") {
        if (change.confirmation !== DELETE_TAG_CONFIRMATION)
          return failAs("confirmation-required", [
            `Type ${DELETE_TAG_CONFIRMATION} to delete this Tag.`,
          ]);
        const tag = document.tags.find((item) => item.id === change.id);
        if (!tag) return failAs("not-found", ["The Tag no longer exists."]);
        for (const area of document.areas) area.tags = area.tags.filter((id) => id !== tag.id);
        for (const project of document.projects)
          project.tags = project.tags.filter((id) => id !== tag.id);
        ...
        for (const template of document.repeatingTemplates) {
          ...
          if (template.projectContents) {
            for (const toDo of template.projectContents.toDos)
              toDo.tags = toDo.tags.filter((id) => id !== tag.id);
          }
        }
        document.tags = document.tags.filter((item) => item.id !== tag.id);
        document.tags.forEach((item, order) => {
          item.order = order;
        });
        document.permanentDeletions.push({
          entityKind: "tag",
      ...
      if (change.type === "createToDo") {
        const parsed = change.quickEntry
          ? parseQuickEntry(change.title, change.quickEntry.referenceDate)
          : null;
        ...
        if (change.quickEntry && !isIsoDate(change.quickEntry.referenceDate))
          return fail(["Quick entry needs a valid reference date."]);

        const location =
          change.location ??
          (document.settings.defaultSpaceId
            ? { kind: "unfiled" as const, spaceId: document.settings.defaultSpaceId }
            : null);
        ...
        for (const tagTitle of parsed?.tagTitles ?? []) {
          ...
          if (!tag) {
            tag = {
              ...
              title: tagTitle,
              order: document.tags.length,
            };
            document.tags.push(tag);
          }
          if (!tagIds.includes(tag.id)) tagIds.push(tag.id);
        }
        if (tagIds.some((tagId) => !document.tags.some((tag) => tag.id === tagId)))
          return fail(["A selected Tag no longer exists."]);

        const createdAt = dependencies.now();
        const toDoId = dependencies.createId("toDo");
        const checklist = (change.checklist ?? []).map(
          (itemTitle, order): ChecklistItem => ({
            id: dependencies.createId("checklistItem"),
            title: itemTitle.trim(),
            completed: false,
      ...
      if (change.type === "captureToDo") {
        ...
        if (!capture.submissionId || capture.submissionId.length > 200)
          return fail(["Capture needs a valid submission identity."]);
        const existingReceipt = document.captureReceipts.find(
          (receipt) => receipt.submissionId === capture.submissionId,
        );
        if (existingReceipt) {
          ...
          if (
            ...
            document.permanentDeletions.some(
              (marker) =>
                marker.entityKind === "toDo" && marker.entityId === existingReceipt.toDoId,
            )
          ) {
          ...
          return {
            status: "changed",
            outcome: "capture-reused",
            affected: [{ kind: "toDo", id: existingToDo.id }],
            undo: null,
          };
        }
        ...
        for (const rawTitle of capture.tags) {
          ...
          if (!title || title.length > 100)
            return fail(["Tag names must be between 1 and 100 characters."]);
          let tag = document.tags.find(
      ...
      if (change.type === "reorderToDos") {
        ...
        if (!movedIds.length || orderedIds.length !== change.orderedIds.length)
          return fail(["Choose valid to-dos to move and order."]);
        if (
          movedIds.some((id) => !document.toDos.some((item) => item.id === id)) ||
          orderedIds.some((id) => !document.toDos.some((item) => item.id === id))
        )
          return failAs("not-found", ["A to-do being moved no longer exists."]);
        if (change.destination?.location && !locationExists(change.destination.location))
          return fail(["The selected to-do Location no longer exists."]);
        if (
          change.destination?.schedule?.kind === "scheduled" &&
          !isIsoDate(change.destination.schedule.date)
        )
          return fail(["Choose a valid Schedule date."]);
        for (const toDo of document.toDos) {
        ...
        document.toDos.forEach((item, order) => {
          item.order = order;
        });
        return finishChange(
          previous,
          "to-dos-reordered",
          movedIds.map((id) => ({ kind: "toDo", id })),
          "Undo moving selected to-dos",
        );
      }
      ...
      if (change.type === "makeToDoRepeating") {
        ...
        if (toDo.trashedAt || toDo.outcome !== "open")
          return fail(["Restore or reopen this to-do before making it repeat."]);
        ...
        document.repeatingTemplates.push({
          ...
          tags: [...toDo.tags],
          checklist: toDo.checklist.map((item, order) => ({
            ...copyDocument(item),
            completed: false,
            order,
          })),
        ...
        return finishChange(previous, "to-do-repetition-created", [
          { kind: "toDo", id: toDo.id },
      ...
      if (change.type === "makeProjectRepeating") {
        const project = document.projects.find((item) => item.id === change.id);
        if (!project) return failAs("not-found", ["The Project no longer exists."]);
        if (project.trashedAt || project.outcome !== "open")
          return fail(["Restore or reopen this Project before making it repeat."]);
        if (project.occurrence)
          return fail(["This Project already belongs to a repeating schedule."]);
        ...
        document.repeatingTemplates.push({
          ...
          reminderTime: null,
          deadlineOffsetDays: project.deadline
            ? daysBetween(change.firstDate, project.deadline)
            : null,
      ...
      if (change.type === "skipOccurrence") {
        if (change.itemKind === "project") {
          ...
          if (project.trashedAt || project.outcome !== "open")
            return fail(["Only an open Project Occurrence outside Trash can be skipped."]);
          const now = dependencies.now();
          const openToDos = projectToDos(project.id).filter(
            (toDo) => toDo.outcome === "open" && !toDo.trashedAt,
          );
        ...
        if (occurrence.trashedAt || occurrence.outcome !== "open")
          return fail(["Only an open Occurrence outside Trash can be skipped."]);
      ...
      if (change.type === "logProject") {
        ...
        if (!project) return failAs("not-found", ["The Project no longer exists."]);
        if (project.trashedAt)
          return fail(["Restore this Project before changing its Logbook placement."]);
        if (project.outcome === "open")
          return fail(["Finish or cancel this Project before moving it to the Logbook."]);
        ...
        return finishChange(
          ...
          "project-logged",
          [{ kind: "project", id: project.id }],
          `Remove “${project.title}” from the Logbook`,
        );
      }
      ...
      if (
        toDo.trashedAt &&
        ["completeToDo", "cancelToDo", "reopenToDo", "logToDo"].includes(change.type)
      ) {
      ...
      if (change.type === "updateToDo") {
        ...
        if (changes.title !== undefined) {
          const title = changes.title.trim();
          if (!title || title.length > 500)
            return fail(["Enter a to-do title between 1 and 500 characters."]);
          toDo.title = title;
        }
        if (changes.notes !== undefined) toDo.notes = changes.notes;
        if (changes.location !== undefined) {
          if (!locationExists(changes.location))
            return fail(["The selected to-do Location no longer exists."]);
          toDo.location = changes.location;
        }
        if (changes.schedule !== undefined) {
          if (changes.schedule.kind === "scheduled" && !isIsoDate(changes.schedule.date))
            return fail(["Choose a valid Schedule date."]);
        ...
        if (changes.reminder !== undefined) {
          if (
            changes.reminder &&
            (!isIsoDateTime(changes.reminder.at) ||
              (changes.reminder.sentAt && !isIsoDateTime(changes.reminder.sentAt)))
          )
            return fail(["Choose a valid Reminder."]);
          toDo.reminder = changes.reminder;
        }
        if (changes.deadline !== undefined) {
          if (changes.deadline && !isIsoDate(changes.deadline))
            return fail(["Choose a valid Deadline."]);
        ...
        return finishChange(
          ...
          "to-do-updated",
          [{ kind: "toDo", id: toDo.id }],
          `Undo changes to “${toDo.title}”`,
        );
      }

      if (change.type === "setToDoTags") {
        const titles = [...new Set(change.titles.map((title) => title.trim()).filter(Boolean))];
        if (titles.some((title) => title.length > 100))
          return fail(["Tag names must be 100 characters or fewer."]);
        ...
        return finishChange(
          ...
          "to-do-tags-updated",
          [{ kind: "toDo", id: toDo.id }],
          `Undo Tag changes to “${toDo.title}”`,
        );
      }
      ...
      if (change.type === "addChecklistItem") {
        ...
        const item: ChecklistItem = {
          ...
          title,
          completed: false,
          order: toDo.checklist.length,
        };
        toDo.checklist.push(item);
        return finishChange(
          previous,
          "checklist-item-added",
          [{ kind: "checklistItem", id: item.id }],
          `Remove “${item.title}”`,
        );
      }
      if (change.type === "updateChecklistItem") {
        ...
        return finishChange(previous, "checklist-item-updated", [
          { kind: "checklistItem", id: item.id },
        ]);
      }
      ...
      if (change.type === "reorderChecklistItems") {
        ...
        const orderedIds = [...new Set(change.orderedIds)];
        if (
          orderedIds.length !== toDo.checklist.length ||
          orderedIds.some((id) => !currentIds.has(id))
        ) {
          return fail([
            "Checklist ordering must include every current checklist item exactly once.",
          ]);
        }
        ...
        return finishChange(
          ...
          [{ kind: "toDo", id: toDo.id }],
          `Undo checklist order changes to “${toDo.title}”`,
        );
      }
      ...
      if (change.type === "completeToDo" || change.type === "cancelToDo") {
        ...
        if (document.settings.logCompletedItems === "immediately")
          toDo.logbookAt = dependencies.now();
        const template =
          change.type === "completeToDo" ? advanceAfterCompletionSchedule(toDo) : null;
      ...
      if (change.type === "reopenToDo") {
        ...
        return finishChange(
          ...
          "to-do-reopened",
          [{ kind: "toDo", id: toDo.id }],
          `Close “${toDo.title}” again`,
        );
      }
      if (change.type === "logToDo") {
        if (toDo.outcome === "open")
          return fail(["Complete or cancel this to-do before logging it."]);
        ...
        return finishChange(
          ...
          "to-do-logged",
          [{ kind: "toDo", id: toDo.id }],
          `Remove “${toDo.title}” from Logbook`,
        );
      }
      ...
      if (change.type === "restoreToDo") {
        ...
        return finishChange(
          ...
          "to-do-restored",
          [{ kind: "toDo", id: toDo.id }],
          `Move “${toDo.title}” back to Trash`,
        );
      }
      if (change.type === "permanentlyDeleteToDo") {
        if (change.confirmation !== PERMANENT_DELETE_CONFIRMATION)
          return failAs("confirmation-required", [
            `Type ${PERMANENT_DELETE_CONFIRMATION} to delete this to-do.`,
          ]);
      ...
      if (change.type === "duplicateToDo") {
        const copy = duplicateToDoRecord(
          toDo,
          copyDocument(toDo.location),
    ...
    changeMany(changes) {
      ...
      const uniqueAffected = [
        ...new Map(affected.map((item) => [`${item.kind}:${item.id}`, item])).values(),
      ];
      return finishChange(
        previous,
        "to-dos-updated",
        uniqueAffected,
        "Undo changes to selected to-dos",
      );
    },
