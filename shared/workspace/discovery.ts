...
export type DirectTarget =
  ...
  | {
      kind: "space" | "area" | "project" | "heading" | "tag" | "toDo" | "repeatingTemplate";
      id: EntityId;
    };
...
export type RecoveredDirectTarget = {
  target: DirectTarget;
  resolved: ResolvedDirectTarget | MissingDirectTarget;
};
...
function matchText(query: string, values: Array<string | undefined>): string | null {
  ...
  return searchableText(values).includes(lowered)
    ? (values.find((value) => value?.toLocaleLowerCase().includes(lowered)) ?? values[0] ?? "")
    : null;
}
...
function result(
  ...
  return {
    id: id ?? `${target.kind}:${"id" in target ? target.id : target.viewKind}`,
    group,
    title,
    detail,
    match,
...
function hasOccurrenceOnNextDate(
  ...
  return (
    document.toDos.some(
      (toDo) =>
        toDo.occurrence?.templateId === templateId && toDo.occurrence.scheduledDate === nextDate,
    ) ||
    document.projects.some(
      (project) =>
        project.occurrence?.templateId === templateId &&
        project.occurrence.scheduledDate === nextDate,
    )
  );
}
...
export function searchWorkspace(
  ...
  if (!query)
    return SPECIAL_VIEW_KINDS.map((viewKind) =>
      result("Lists & views", VIEW_LABELS[viewKind], "Special view", VIEW_LABELS[viewKind], {
        kind: "view",
        viewKind,
      }),
    );
  ...
  for (const viewKind of SPECIAL_VIEW_KINDS) {
    ...
    if (match !== null)
      results.push(
        result("Lists & views", title, "Special view", match, { kind: "view", viewKind }),
      );
  }

  const organization = [
    ...document.spaces.map((item) => ({
      item,
      kind: "space" as const,
      label: "Space",
      values: [item.title],
    ...
    ...document.headings.map((item) => ({
      item,
      kind: "heading" as const,
      label: item.archivedAt ? "Archived Heading" : "Heading",
      values: [item.title],
    })),
  ];
  for (const entry of organization) {
    ...
    if (match !== null)
      results.push(
        result("Organization", entry.item.title, entry.label, match, {
          kind: entry.kind,
          id: entry.item.id,
        }),
      );
  ...
  for (const project of document.projects) {
    ...
    const group: SearchGroup = project.trashedAt
      ? "Trash"
      : project.logbookAt || project.outcome !== "open"
        ? "History"
        : "Organization";
    const detail =
      group === "Trash"
        ? "Project in Trash"
        : group === "History"
          ? "Project in Logbook or completed work"
          : "Project";
  ...
  for (const tag of document.tags) {
    ...
    if (match !== null)
      results.push(result("Tags", tag.title, "Tag", match, { kind: "tag", id: tag.id }));
  }

  for (const toDo of document.toDos) {
    const match = matchText(query, toDoSearchText(toDo, document));
    if (match === null) continue;
    const group: SearchGroup = toDo.trashedAt
      ? "Trash"
      : toDo.logbookAt || toDo.outcome !== "open"
        ? "History"
        : "Active work";
    const detail =
      group === "Trash"
        ? "To-do in Trash"
        : group === "History"
          ? "To-do in Logbook or completed work"
          : "Active to-do";
  ...
  for (const template of document.repeatingTemplates) {
    ...
    const values = [
      ...
      ...(projectContents?.toDos.flatMap((toDo) => [
        toDo.title,
        toDo.notes,
        ...toDo.checklist.map((item) => item.title),
      ]) ?? []),
    ...
    const hasBlockedAfterCompletionPreview =
      template.mode === "after-completion" &&
      hasOccurrenceOnNextDate(document, template.id, template.nextDate);
    if (
      template.state === "active" &&
      template.nextDate >= today &&
      !hasBlockedAfterCompletionPreview
    ) {
      results.push(
        result(
          "Repeating Previews",
          template.title,
          `${template.nextDate} · ${template.itemKind === "toDo" ? "To-do" : "Project"} preview`,
          match,
          target,
          `preview:${template.id}:${template.nextDate}`,
        ),
      );
    }
    results.push(
      result(
        "Repeating Templates",
        template.title,
        `${template.state} ${template.itemKind === "toDo" ? "to-do" : "Project"} Template`,
        match,
        target,
      ),
    );
...
function missing(label: string): MissingDirectTarget {
  return {
    status: "missing",
    message: `This ${label} is no longer available or you do not have access to it.`,
  };
}
...
function viewForToDo(toDo: ToDo, today: string): WorkspaceView {
  ...
  if (toDo.schedule.kind === "scheduled")
    return toDo.schedule.date <= today
      ? { kind: "today", date: today }
      : { kind: "upcoming", date: today };
...
export function resolveDirectTarget(
  ...
  target: DirectTarget,
  today: string,
): ResolvedDirectTarget | MissingDirectTarget {
  if (target.kind === "view") {
    const date = target.viewKind === "tomorrow" ? datePlus(today, 1) : today;
    return {
      status: "resolved",
      view: { kind: target.viewKind, date },
      selectedToDoId: null,
      tagIds: [],
      repeatingTemplateId: null,
      activeSpaceId: document.settings.defaultSpaceId,
    };
  }
  if (target.kind === "space") {
    ...
    return {
      status: "resolved",
      view: { kind: "space", id: target.id, date: today },
      selectedToDoId: null,
      tagIds: [],
      repeatingTemplateId: null,
  ...
  if (target.kind === "area") {
    ...
    return {
      status: "resolved",
      view: { kind: "area", id: target.id, date: today },
      selectedToDoId: null,
      tagIds: [],
      repeatingTemplateId: null,
  ...
  if (target.kind === "project") {
    ...
    const view: WorkspaceView = project.trashedAt
      ...
      : project.logbookAt || project.outcome !== "open"
        ? { kind: "logbook", date: today }
        : { kind: "project", id: target.id, date: today };
    return {
      status: "resolved",
      view,
      selectedToDoId: null,
      tagIds: [],
      repeatingTemplateId: null,
      activeSpaceId: spaceIdForProject(document, project),
    };
  }
  if (target.kind === "heading") {
    ...
    return {
      status: "resolved",
      view: { kind: "heading", id: target.id, date: today },
      selectedToDoId: null,
      tagIds: [],
      repeatingTemplateId: null,
      activeSpaceId,
  ...
  if (target.kind === "repeatingTemplate") {
    ...
    return {
      status: "resolved",
      view: { kind: "repeating", date: today },
      selectedToDoId: null,
      tagIds: [],
      repeatingTemplateId: target.id,
      activeSpaceId:
        spaceIdForRepeatingTemplate(document, template) ?? document.settings.defaultSpaceId,
    };
  }
  ...
  return {
    status: "resolved",
    view: viewForToDo(toDo, today),
    selectedToDoId: toDo.id,
    tagIds: [],
    repeatingTemplateId: null,
    activeSpaceId: spaceIdForToDo(document, toDo),
  };
}
...
export function parseDirectTarget(search: string): DirectTarget | null {
  ...
  if (
    ["space", "area", "project", "heading", "tag", "toDo", "repeatingTemplate"].includes(kind ?? "")
  ) {
