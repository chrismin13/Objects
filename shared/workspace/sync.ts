import type {
  ...
  Project,
  ProjectClosure,
  RepeatingProjectHeadingBlueprint,
...
function toSyncTree(document: WorkspaceDocument): JsonObject {
  ...
  for (const name of [
    ...
    // Every named collection is an entity array with an `id`; the union of
    // array types cannot infer a single element type, so widen explicitly.
    tree[name] = mapBy(document[name] as { id: string }[], (item) => item.id);
  }
  tree.permanentDeletions = mapBy(
  ...
  for (const toDo of document.toDos)
    record(toDos[toDo.id]).checklist = mapBy(toDo.checklist, (item) => item.id);
...
function fromSyncTree(tree: JsonObject, sync: WorkspaceDocument["sync"]): WorkspaceDocument {
  ...
  document.spaces = values<Space>(document.spaces).sort(byOrder);
  document.areas = values<Area>(document.areas).sort(byOrder);
  document.projects = values<Project>(document.projects).sort(byOrder);
  document.headings = values<Heading>(document.headings).sort(byOrder);
  document.tags = values<Tag>(document.tags).sort(byOrder);
  document.toDos = values<ToDo>(document.toDos).sort(byOrder);
  document.repeatingTemplates = values<RepeatingTemplate>(document.repeatingTemplates);
  document.projectClosures = values<ProjectClosure>(document.projectClosures).sort((left, right) =>
    left.closedAt.localeCompare(right.closedAt),
  );
  document.calendarEvents = values<CalendarEvent>(document.calendarEvents).sort((left, right) =>
    left.start.localeCompare(right.start),
  );
  document.permanentDeletions = values<PermanentDeletion>(document.permanentDeletions).sort(
    (left, right) => left.deletedAt.localeCompare(right.deletedAt),
  );
  document.captureReceipts = values<CaptureReceipt>(document.captureReceipts).sort((left, right) =>
    left.createdAt.localeCompare(right.createdAt),
  );
  document.settings.launchRules = values<LaunchRule>(document.settings.launchRules).sort(byOrder);
  for (const toDo of document.toDos)
    toDo.checklist = values<ChecklistItem>(toDo.checklist).sort(byOrder);
  for (const template of document.repeatingTemplates) {
    template.checklist = values<ChecklistItem>(template.checklist).sort(byOrder);
    if (template.projectContents) {
      template.projectContents.headings = values<RepeatingProjectHeadingBlueprint>(
        template.projectContents.headings,
      ).sort(byOrder);
      template.projectContents.toDos = values<RepeatingProjectToDoBlueprint>(
        template.projectContents.toDos,
      ).sort(byOrder);
    }
  }
...
function diffValue(
  ...
  operations.push({
    path,
    before:
      before === undefined ? { present: false } : { present: true, value: cloneJsonValue(before) },
    after:
      after === undefined ? { present: false } : { present: true, value: cloneJsonValue(after) },
  });
}

function orderedKeys<T extends { order: number }>(items: T[], key: (item: T) => string): string[] {
  return [...items]
    .sort((left, right) => left.order - right.order || key(left).localeCompare(key(right)))
    .map(key);
}
...
function workspaceOrderings(document: WorkspaceDocument): WorkspaceOrderingChange[] {
  ...
  add(
    ["spaces"],
    orderedKeys(document.spaces, (item) => item.id),
  );
  add(
    ["areas"],
    orderedKeys(document.areas, (item) => item.id),
  );
  add(
    ["projects"],
    orderedKeys(document.projects, (item) => item.id),
  );
  add(
    ["headings"],
    orderedKeys(document.headings, (item) => item.id),
  );
  add(
    ["tags"],
    orderedKeys(document.tags, (item) => item.id),
  );
  add(
    ["toDos"],
    orderedKeys(document.toDos, (item) => item.id),
  );
  add(
    ["settings", "launchRules"],
    orderedKeys(document.settings.launchRules, (item) => item.id),
  );
  for (const toDo of document.toDos)
    add(
      ["toDos", toDo.id, "checklist"],
      orderedKeys(toDo.checklist, (item) => item.id),
    );
  for (const template of document.repeatingTemplates) {
    add(
      ["repeatingTemplates", template.id, "checklist"],
      orderedKeys(template.checklist, (item) => item.id),
    );
    if (template.projectContents) {
      add(
        ["repeatingTemplates", template.id, "projectContents", "headings"],
        orderedKeys(template.projectContents.headings, (item) => item.key),
      );
      add(
        ["repeatingTemplates", template.id, "projectContents", "toDos"],
        orderedKeys(template.projectContents.toDos, (item) => item.key),
      );
    }
  }
  return orderings;
...
export function createWorkspaceDelta(
  ...
  const beforeOrderings = new Map(
    workspaceOrderings(before).map((ordering) => [ordering.path.join("/"), ordering.after]),
  );
  const orderings = workspaceOrderings(after)
    .map((ordering) => ({
      ...ordering,
      before: beforeOrderings.get(ordering.path.join("/")) ?? [],
    }))
...
function setAt(
  root: JsonObject,
  path: string[],
  next: { present: boolean; value?: JsonValue },
): void {
...
function permanentDeletionFor(
  ...
  return (
    document.permanentDeletions.find(
      (marker) => marker.entityKind === entityKind && marker.entityId === path[1],
    ) ?? null
  );
}
...
function keyedItemWasRemoved(tree: JsonObject, path: string[]): boolean {
  const keyedCollections = [
    ...
    {
      prefix: ["toDos", path[1], "checklist"],
      keyIndex: 3,
      applies: path[0] === "toDos" && path[2] === "checklist",
    },
    {
      prefix: ["repeatingTemplates", path[1], "checklist"],
      keyIndex: 3,
      applies: path[0] === "repeatingTemplates" && path[2] === "checklist",
    },
    {
      prefix: ["repeatingTemplates", path[1], "projectContents", path[3]],
      keyIndex: 4,
      applies:
        path[0] === "repeatingTemplates" &&
        path[2] === "projectContents" &&
        ["headings", "toDos"].includes(path[3]),
    },
    {
      prefix: ["settings", "launchRules"],
      keyIndex: 2,
      applies: path[0] === "settings" && path[1] === "launchRules",
    },
  ];
  for (const collection of keyedCollections) {
    ...
    if (!Object.prototype.hasOwnProperty.call(record(parent), path[collection.keyIndex]))
      return true;
  }
  return false;
}
  ...
  return Object.entries(container)
    .sort(
      ([leftKey, left], [rightKey, right]) =>
        Number(record(left).order ?? 0) - Number(record(right).order ?? 0) ||
        leftKey.localeCompare(rightKey),
    )
    .map(([key]) => key);
}

function applyOrderingChanges(
  tree: JsonObject,
  orderings: WorkspaceOrderingChange[],
  conflicts: WorkspaceSyncConflict[],
): void {
  for (const ordering of orderings) {
    ...
    const localOrder = ordering.after.filter((key) =>
    ...
    mergedOrder.forEach((key, index) => {
      record(container[key]).order = index;
    });
  }
}
...
function removePermanentlyDeletedEntities(document: WorkspaceDocument): void {
  for (const [collection, entityKind] of ENTITY_COLLECTIONS) {
    const blocked = new Set(
      document.permanentDeletions
        .filter((marker) => marker.entityKind === entityKind)
        .map((marker) => marker.entityId),
    );
    const items = document[collection as keyof WorkspaceDocument];
    if (Array.isArray(items))
      (document as unknown as Record<string, unknown>)[collection] = items.filter(
        (item) => !blocked.has((item as { id: string }).id),
      );
  }
}
...
function removeProjectOccurrence(document: WorkspaceDocument, projectId: string): void {
  ...
  document.toDos = document.toDos.filter(
    (toDo) =>
      !(toDo.location.kind === "project" && toDo.location.projectId === projectId) &&
  ...
  document.projectClosures = document.projectClosures.filter(
    (closure) => closure.projectId !== projectId,
  );
}

function deduplicateRemoteWork(
  document: WorkspaceDocument,
  remote: WorkspaceDocument,
  conflicts: WorkspaceSyncConflict[],
): void {
  const remoteToDoOccurrences = new Map(
    remote.toDos
      .filter((item) => item.occurrence)
      .map((item) => [`${item.occurrence!.templateId}:${item.occurrence!.scheduledDate}`, item.id]),
  );
  ...
  const remoteProjectOccurrences = new Map(
    remote.projects
      .filter((item) => item.occurrence)
      .map((item) => [`${item.occurrence!.templateId}:${item.occurrence!.scheduledDate}`, item.id]),
  );
  ...
  for (const project of [...document.projects]) {
    ...
    const preferred =
      remoteProjectOccurrences.get(key) ?? seenProjectOccurrences.get(key) ?? project.id;
    if (preferred !== project.id) {
      removeProjectOccurrence(document, project.id);
      conflicts.push({
        path: `projects/${project.id}/occurrence`,
        resolution: "remote-removal-kept",
      });
    } else seenProjectOccurrences.set(key, project.id);
  }

  const remoteReceipts = new Map(
    remote.captureReceipts.map((receipt) => [receipt.submissionId, receipt]),
  );
  for (const receipt of [...document.captureReceipts]) {
    ...
    document.captureReceipts = document.captureReceipts.filter(
      (item) => item.submissionId !== receipt.submissionId,
    );
    document.captureReceipts.push(cloneJsonValue(preferred));
    conflicts.push({
      path: `captureReceipts/${receipt.submissionId}`,
      resolution: "remote-removal-kept",
    });
  }

  const remoteCalendarSources = new Map(
    remote.calendarEvents
      .filter((event) => event.sourceUid)
      .map((event) => [event.sourceUid!, event.id]),
  );
  for (const event of [...document.calendarEvents]) {
    ...
    conflicts.push({
      path: `calendarEvents/${event.id}/sourceUid`,
      resolution: "remote-removal-kept",
    });
  }
}
...
export function applyWorkspaceDelta(
  ...
  if (!delta || !Array.isArray(delta.operations) || delta.operations.length > 100_000) {
    return {
      document: cloneJsonValue(remote),
      conflicts: [],
      errors: ["Workspace changes are malformed or too large."],
    };
  }
  const orderings = delta.orderings ?? [];
  if (
    !Array.isArray(orderings) ||
    orderings.length > 100_000 ||
    orderings.some(
      (ordering) =>
        !ordering ||
        !Array.isArray(ordering.path) ||
        !ordering.path.length ||
        ordering.path.length > 20 ||
        ordering.path.some(
          (segment) =>
        ...
        !Array.isArray(ordering.before) ||
        !Array.isArray(ordering.after) ||
        [...ordering.before, ...ordering.after].some(
          (key) => typeof key !== "string" || !key || key.length > 500,
        ) ||
        new Set(ordering.before).size !== ordering.before.length ||
        new Set(ordering.after).size !== ordering.after.length,
    )
  ) {
    return {
      document: cloneJsonValue(remote),
      conflicts: [],
      errors: ["Workspace ordering changes are malformed or too large."],
    };
  }
  let tree: JsonObject;
  try {
    ...
    return {
      document: cloneJsonValue(remote),
      conflicts: [],
      errors: ["The saved Workspace is malformed and needs recovery."],
    };
  }
  ...
  for (const operation of delta.operations) {
    if (
      ...
      !operation.before ||
      typeof operation.before.present !== "boolean" ||
      !operation.after ||
      typeof operation.after.present !== "boolean" ||
      (operation.before.present &&
        !Object.prototype.hasOwnProperty.call(operation.before, "value")) ||
      (operation.after.present && !Object.prototype.hasOwnProperty.call(operation.after, "value"))
    ) {
      return {
        document: cloneJsonValue(remote),
        conflicts: [],
        errors: ["A Workspace change has a malformed path or value."],
      };
    }
    ...
    if (!sameStoredValue(current, operation.before))
      conflicts.push({ path: conflictPath(path), resolution: "local-change-kept" });
  ...
  try {
    ...
    return {
      document: cloneJsonValue(remote),
      conflicts: [],
      errors: ["Workspace changes could not be reconstructed safely."],
    };
  }
}
...
export function validateSyncDocument(value: unknown): string[] {
  if (!value || typeof value !== "object" || Array.isArray(value))
    return ["Workspace data must be one object."];
  const document = value as Partial<WorkspaceDocument>;
  const errors: string[] = [];
  if (document.format !== "objects-workspace" || document.version !== 1)
    errors.push("Workspace format or version is not supported.");
  ...
  for (const name of collections)
    if (!Array.isArray(document[name])) errors.push(`Workspace ${name} must be a list.`);
  if (!document.settings || typeof document.settings !== "object")
    errors.push("Workspace settings are missing.");
  if (!document.sync || typeof document.sync !== "object")
    errors.push("Workspace sync metadata is missing.");
  return errors;
}
...
function validateWorkspaceRules(document: WorkspaceDocument, now: IsoDateTime): string[] {
  try {
    return createWorkspace(document, {
      now: () => now,
      createId: (kind) => `sync-validation-${kind}`,
    }).validate();
  } catch {
...
function mergeDeletionMarkers(
  ...
  for (const marker of current?.permanentDeletions ?? [])
    markers.set(`${marker.entityKind}:${marker.entityId}`, marker);
  for (const marker of next.permanentDeletions)
    markers.set(`${marker.entityKind}:${marker.entityId}`, marker);
...
export function resolveSyncCommand(
  ...
  if (!command.mutationId || command.mutationId.length > 200) {
    return {
      result: { status: "rejected", errors: ["A valid mutation identity is required."] },
      next: null,
    };
  }
  const submittedErrors = validateSyncDocument(command.document);
  if (submittedErrors.length)
    return { result: { status: "rejected", errors: submittedErrors }, next: null };
  ...
  if (command.expectedRevision !== currentRevision && !command.changes) {
    return current
      ...
      : {
          result: {
            status: "conflict",
            snapshot: { revision: 0, document: cloneJsonValue(command.document) },
          },
          next: null,
        };
  }
  ...
  if (current && command.changes) {
    ...
    if (applied.errors.length)
      return { result: { status: "rejected", errors: applied.errors }, next: null };
    nextDocument = applied.document;
    conflicts = applied.conflicts.slice(0, 40);
  }
  nextDocument.captureReceipts = Array.isArray(nextDocument.captureReceipts)
    ? nextDocument.captureReceipts
    : [];
  nextDocument.permanentDeletions = mergeDeletionMarkers(current?.document ?? null, nextDocument);
  const recreated = nextDocument.permanentDeletions.filter((marker) =>
    entityExists(nextDocument, marker),
  );
  if (recreated.length && !command.changes) {
    return {
      result: {
        status: "rejected",
        errors: recreated.map(
          (marker) =>
            `${marker.entityKind} “${marker.entityId}” was permanently deleted and cannot be recreated.`,
        ),
      },
      next: null,
    };
      ...
      return {
        ...
        async save(command) {
          ...
          if (resolved.next && resolved.result.status === "acknowledged") {
            ...
            const ownerReceipts =
              receipts.get(ownerId) ?? new Map<string, WorkspaceSyncAcknowledgement>();
