...
export type IcsParseResult =
  | { status: "parsed"; events: ParsedIcsEvent[] }
...
function timeZoneParts(date: Date, timeZone: string): number[] {
  ...
  return ["year", "month", "day", "hour", "minute", "second"].map((name) =>
    Number(parts.get(name)),
  );
}
...
function parseIcsDateTime(item: IcsProperty): { iso: string; allDay: boolean } | null {
  ...
  if (allDay) {
    ...
    return {
      iso: new Date(Date.UTC(parts[0], parts[1] - 1, parts[2])).toISOString(),
      allDay: true,
    };
  }
...
export function parseIcsCalendar(source: string): IcsParseResult {
  ...
  if (!lines.some((line) => line.trim().toUpperCase() === "BEGIN:VCALENDAR"))
    return { status: "rejected", errors: ["This file is not an ICS calendar."] };
  const calendarName =
    lines.map(property).find((item) => item?.name === "X-WR-CALNAME")?.value || "Imported";
  ...
  if (!blocks.length)
    return { status: "rejected", errors: ["The ICS calendar does not contain any events."] };
  ...
  blocks.forEach((block, index) => {
    ...
    const title = unescapeIcsText(
      block.find((item) => item.name === "SUMMARY")?.value ?? "",
    ).trim();
    ...
    const parsedEnd = end
      ? parseIcsDateTime(end)
      : parsedStart
        ? {
            iso: parsedStart.allDay ? addOneDay(parsedStart.iso) : parsedStart.iso,
    ...
    const invalidRange =
      parsedStart &&
      parsedEnd &&
      (Date.parse(parsedEnd.iso) < Date.parse(parsedStart.iso) ||
    ...
    if (seenEvents.has(identity)) {
      errors.push(`${label} duplicates another event in this file.`);
      return;
    }
    ...
    events.push({
      ...
      calendar: unescapeIcsText(calendarName),
      allDay: parsedStart.allDay,
    });
  });
...
function eventMatchesView(
  ...
  const timedEventContinuesOnEndDate =
    endDate > view.date || (endDate === view.date && localEnd.time > "00:00");
...
export function agendaForView(
    ...
    for (const event of document.calendarEvents) {
      ...
      if (match.matches)
        agenda.push({
          ...
          title: event.title,
          date: match.date,
          sortTime: event.allDay ? "00:00" : localStart.time,
          actionable: false,
          item: event,
        });
    }
  ...
  for (const project of document.projects) {
    if (
      ...
      project.trashedAt ||
      project.logbookAt ||
      project.schedule.kind !== "scheduled"
    )
      continue;
    if (activeSpaceId && spaceIdForProject(document, project) !== activeSpaceId) continue;
    if (activeTagIds.some((tagId) => !effectiveTagIdsForProject(document, project).includes(tagId)))
      continue;
    const matches =
      view.kind === "today"
        ? project.schedule.date <= view.date
        : view.kind === "tomorrow"
          ? project.schedule.date === view.date
          : project.schedule.date > view.date;
    if (matches)
  ...
  for (const toDo of document.toDos) {
    ...
    if (activeTagIds.some((tagId) => !effectiveTagIdsForToDo(document, toDo).includes(tagId)))
      continue;
    const matches =
      view.kind === "today"
