...
export type InterfaceChangeSet = {
  ...
  entities?: Partial<
    Record<
      "spaces" | "areas" | "projects" | "headings" | "calendarEvents" | "tasks",
      Array<{ id: string; patch: JsonRecord }>
    >
  >;
  deletes?: Partial<
    Record<"spaces" | "areas" | "projects" | "headings" | "calendarEvents" | "tasks", string[]>
  >;
};
...
function blueprintReminderForInterface(
  ...
  if (reminder.kind === "offset")
    return `${addDaysToDate(templateDate, reminder.days)}T${reminder.time}`;
  const reminderDate =
...
function blueprintDeadlineForInterface(
...
function blueprintScheduleFromInterface(
  item: InterfaceToDo,
  templateDate: string,
): RepeatingProjectToDoBlueprint["schedule"] {
  ...
  return schedule.kind === "scheduled"
    ? {
        kind: "scheduled",
        offsetDays: daysBetweenDates(templateDate, schedule.date),
        evening: schedule.evening,
      }
    : schedule;
}
...
function statusFor(
  outcome: Outcome,
  trashedAt: string | null,
): { status: string; previousStatus: string | null } {
...
function directLocationParts(
  ...
  if (location.kind === "unfiled")
    return { headingId: null, projectId: null, areaId: null, spaceId: location.spaceId };
  ...
  if (location.kind === "project") {
    ...
    const spaceId =
      project?.location.kind === "space" ? project.location.spaceId : (area?.spaceId ?? null);
  ...
  if (!heading)
    return { headingId: location.headingId, projectId: null, areaId: null, spaceId: null };
  const headingLocation = heading.location;
  if (headingLocation.kind === "area") {
    const area = document.areas.find((item) => item.id === headingLocation.areaId);
    return {
      headingId: heading.id,
      projectId: null,
      areaId: headingLocation.areaId,
      spaceId: area?.spaceId ?? null,
    };
  }
  ...
  const spaceId =
    project?.location.kind === "space" ? project.location.spaceId : (area?.spaceId ?? null);
  return { headingId: heading.id, projectId: headingLocation.projectId, areaId, spaceId };
}
...
function templateLocationParts(template: RepeatingTemplate, document: WorkspaceDocument) {
  ...
  if (templateLocation.kind === "space") return { areaId: null, spaceId: templateLocation.spaceId };
  return {
    areaId: templateLocation.areaId,
    spaceId: document.areas.find((item) => item.id === templateLocation.areaId)?.spaceId ?? null,
  };
}
...
function templateSources(
  ...
  for (const template of document.repeatingTemplates) {
    const base = {
      ...
      deadline:
        template.deadlineOffsetDays === null
          ? null
          : addDaysToDate(template.nextDate, template.deadlineOffsetDays),
    ...
    for (const blueprint of template.projectContents.toDos) {
      ...
      const reminderAt = blueprintReminderForInterface(
        blueprint.reminder,
        template.nextDate,
        blueprint.schedule,
      );
      ...
      tasks.push({
        ...
        checklist: blueprint.checklist.map((item, index) => ({
          id: `${template.id}:${blueprint.key}:check:${index}`,
          title: item.title,
          done: false,
        })),
        repeat: null,
...
export function workspaceDocumentToInterfaceState(
  ...
  const projects = document.projects.map((project) => {
    ...
    const location =
      projectLocation.kind === "space"
        ? { areaId: null, spaceId: projectLocation.spaceId }
        : {
            areaId: projectLocation.areaId,
            spaceId:
  ...
  return {
    ...
    areas: document.areas.map((area) => ({
      ...clone(area),
      tags: tagTitles(area.tags, tagById),
    })) as InterfaceEntity[],
    ...
    tasks: [
      ...document.toDos.map((toDo) => interfaceToDo(toDo, document, tagById, today)),
      ...templates.tasks,
    ],
  };
}
...
function applyChangeSet(state: InterfaceState, changes: InterfaceChangeSet): InterfaceState {
  ...
  for (const name of COLLECTIONS) {
    ...
    const items = new Map(
      next[name].filter((item) => !deleted.has(item.id)).map((item) => [item.id, item]),
    );
...
function tagsFromState(
  ...
  for (const title of state.settings.tags)
  ...
  for (const collection of [state.areas, state.projects, state.tasks]) {
    for (const item of collection)
      for (const title of Array.isArray(item.tags) ? item.tags : [])
        if (typeof title === "string" && title.trim()) titles.add(title.trim());
  }
  ...
  const tags = [...titles].map((title, order) => {
    ...
    return current
      ? { ...current, title, order }
      : { id: dependencies.createId("tag"), title, order };
  });
  ...
  return {
    ...
    idsFor(value) {
      return Array.isArray(value)
        ? value
            .map((title) =>
              typeof title === "string" ? idByTitle.get(title.toLocaleLowerCase()) : undefined,
            )
...
function toDoLocation(item: JsonRecord, state: InterfaceState): ToDoLocation | null {
  ...
  if (
    headingId &&
    state.headings.some((heading) => heading.id === headingId && !heading.workspaceTemplateId)
  )
    return { kind: "heading", headingId };
  const projectId = optionalString(item.projectId);
  if (
    projectId &&
    state.projects.some((project) => project.id === projectId && !project.workspaceTemplate)
  )
  ...
  if (spaceId && state.spaces.some((space) => space.id === spaceId))
...
function templateFromSource(
  ...
  const base = {
    ...
    firstDate:
      existing?.firstDate ?? optionalString(item.scheduledFor) ?? dependencies.now().slice(0, 10),
    nextDate:
      optionalString(repeat.nextDate) ??
      optionalString(item.scheduledFor) ??
  ...
  if (itemKind === "toDo")
    return { ...base, itemKind, location: location as ToDoLocation, projectContents: null };
  const headings = state.headings.filter(
    (heading) => heading.projectId === item.id && heading.workspaceTemplateId === item.id,
  );
  const headingKeyById = new Map(
    headings.map((heading) => [heading.id, stringValue(heading.workspaceBlueprintKey, heading.id)]),
  );
  const contents = {
    ...
    toDos: state.tasks
      .filter((toDo) => toDo.projectId === item.id && toDo.workspaceTemplateId === item.id)
      .map((toDo, order) => ({
        ...
        schedule: blueprintScheduleFromInterface(
          toDo,
          optionalString(item.scheduledFor) ?? dependencies.now().slice(0, 10),
        ),
...
function documentFromInterfaceState(
  ...
  const spaces: Space[] = state.spaces.flatMap((item, order) =>
    item.title.trim()
      ? [
          {
            id: item.id,
  ...
  const projects: Project[] = state.projects
    .filter((item) => !item.workspaceTemplate && !item.repeat)
    .flatMap((item, order) => {
      ...
      return [
        {
          ...
          trashedAt:
            item.status === "trashed"
              ? (durableDateTime(item.trashedAt) ?? dependencies.now())
              : null,
          logbookAt: durableDateTime(item.loggedAt),
          tags: tagData.idsFor(item.tags),
          occurrence: optionalString(item.repeatTemplateId)
            ? {
                templateId: String(item.repeatTemplateId),
                scheduledDate: optionalString(item.scheduledFor) ?? dependencies.now().slice(0, 10),
              }
            : null,
          completedAt:
            outcome === "completed"
              ? (durableDateTime(item.completedAt) ?? dependencies.now())
              : null,
          order: numberValue(item.order, order),
        },
      ];
  ...
  const headings: Heading[] = state.headings
    .filter((item) => !item.workspaceTemplateId)
    .flatMap((item, order) => {
      const projectId = optionalString(item.projectId);
      const areaId = optionalString(item.areaId);
      const location =
  ...
  const toDos: ToDo[] = state.tasks
    .filter((item) => !item.workspaceTemplate && !item.workspaceTemplateId && !item.repeat)
    .flatMap((item, order) => {
      ...
      return [
        {
          ...
          schedule: interfaceSchedule(item),
          reminder: reminderAt
            ? { at: reminderAt, sentAt: durableDateTime(item.reminderSentAt) }
            : null,
          deadline: optionalString(item.deadline),
          outcome,
          trashedAt:
  ...
  const templateSources = [
    ...state.tasks
      .filter((item) => item.workspaceTemplate || item.repeat)
      .map((item) => ({ item, kind: "toDo" as const })),
    ...state.projects
      .filter((item) => item.workspaceTemplate || item.repeat)
  ...
  for (const toDo of toDos)
    if (toDo.occurrence && !templateIds.has(toDo.occurrence.templateId)) toDo.occurrence = null;
  for (const project of projects)
    if (project.occurrence && !templateIds.has(project.occurrence.templateId))
      project.occurrence = null;
  ...
  for (const [kind, before, after] of [
    ...
    for (const item of before) {
      const wasPermanentlyRemoved =
        !afterIds.has(item.id) &&
        (kind === "repeatingTemplate" || ("trashedAt" in item && Boolean(item.trashedAt)));
      if (
        wasPermanentlyRemoved &&
        !permanentDeletions.some(
          (marker) => marker.entityKind === kind && marker.entityId === item.id,
        )
      ) {
  ...
  return {
    ...
    version: 1,
    settings: {
      theme: ["system", "light", "dark"].includes(stringValue(state.settings.theme))
        ? (state.settings.theme as "system" | "light" | "dark")
        : "system",
      ...
      logCompletedItems: ["immediately", "daily", "manually"].includes(
        stringValue(state.settings.logCompletedItems),
      )
        ? (state.settings.logCompletedItems as "immediately" | "daily" | "manually")
        : "daily",
      defaultSpaceId:
        defaultSpaceId && spaceIds.has(defaultSpaceId) ? defaultSpaceId : spaces[0].id,
      launchRules: Array.isArray(launchRulesSource)
        ? launchRulesSource.flatMap((rawRule, order) => {
        ...
        : [],
      quickDraft:
        record(state.settings.quickDraft).value !== undefined
          ? (clone(state.settings.quickDraft) as WorkspaceDocument["settings"]["quickDraft"])
          : null,
    },
    spaces,
    areas,
    ...
    toDos,
    repeatingTemplates,
    projectClosures: previous.projectClosures.filter(
      (closure) =>
        projectIds.has(closure.projectId) && closure.changedToDoIds.every((id) => toDoIds.has(id)),
    ),
    calendarEvents,
    permanentDeletions,
...
function explicitLifecycleTargets(changes: WorkspaceChange[]): {
  toDoIds: Set<string>;
...
function withoutInferredLifecycleChanges(changeSet: InterfaceChangeSet): InterfaceChangeSet {
  ...
  if (next.deletes?.tasks)
    next.deletes.tasks = next.deletes.tasks.filter((id) => !explicit.toDoIds.has(id));
  if (next.deletes?.projects)
    next.deletes.projects = next.deletes.projects.filter((id) => !explicit.projectIds.has(id));
  return next;
}
...
function queueExplicitInterfaceIds(changeSet: InterfaceChangeSet, queues: IdQueues): void {
  for (const change of changeSet.workspaceChanges ?? []) {
    ...
    const collection =
      change.type === "makeToDoRepeating"
        ? changeSet.entities?.tasks
        : changeSet.entities?.projects;
...
function applyProjectLifecycleChanges(
  ...
  for (const project of before.projects) {
    ...
    const item = desired.projects.find(
      (candidate) => candidate.id === project.id && !candidate.workspaceTemplate,
    );
    if (!item) continue;
    ...
    if (project.trashedAt && item.status !== "trashed") {
      errors.push(
        ...applyWorkspaceChange(workspace, { type: "restoreProjectFromTrash", id: project.id }),
      );
    }
    ...
    if (current.outcome === "open" && desiredOutcome !== "open") {
      const toDoOutcomes = workspace.read().toDos.flatMap((toDo) => {
        if (toDo.outcome !== "open" || workspace.locationOfToDo(toDo.id)?.projectId !== project.id)
          return [];
...
function applyToDoLifecycleChanges(
  ...
  const headingsInDeletedProjects = new Set(
    before.headings.flatMap((heading) =>
      heading.location.kind === "project" && deletedProjectIds.has(heading.location.projectId)
        ? [heading.id]
        : [],
    ),
  );
  for (const original of before.toDos) {
    const removedWithProject =
      (original.location.kind === "project" &&
        deletedProjectIds.has(original.location.projectId)) ||
      (original.location.kind === "heading" &&
        headingsInDeletedProjects.has(original.location.headingId));
    ...
    const item = desired.tasks.find(
      (candidate) => candidate.id === original.id && !candidate.workspaceTemplate,
    );
    if (!item) continue;
...
function queueCreatedIds(
  queues: IdQueues,
  values: Partial<Record<WorkspaceEntityKind | "projectClosure" | "undo", string[]>>,
): void {
...
function syncSettings(
  ...
  const applicationChanges = {
    ...
    ...(current.groupToday !== candidate.settings.groupToday
      ? { groupToday: candidate.settings.groupToday }
      : {}),
    ...(current.notifications !== candidate.settings.notifications
      ? { notifications: candidate.settings.notifications }
      : {}),
    ...(current.weekStartsOn !== candidate.settings.weekStartsOn
      ? { weekStartsOn: candidate.settings.weekStartsOn }
      : {}),
    ...(current.showCalendar !== candidate.settings.showCalendar
      ? { showCalendar: candidate.settings.showCalendar }
      : {}),
  };
  if (Object.keys(applicationChanges).length)
    errors.push(
      ...applyDirectWorkspaceChange(workspace, queues, {
        type: "updateSettings",
  ...
  if (workspace.read().settings.logCompletedItems !== candidate.settings.logCompletedItems) {
    errors.push(
      ...applyDirectWorkspaceChange(workspace, queues, {
        type: "setLogbookPolicy",
        policy: candidate.settings.logCompletedItems,
      }),
    );
  }
  ...
  if (!sameValue(quickDraft, candidate.settings.quickDraft)) {
    if (candidate.settings.quickDraft) {
      ...
      // The legacy InterfaceState stores the draft view loosely; the runtime
      // consumer reads only `kind` and `id`, so the WorkspaceView `date`
      ...
      errors.push(
        ...applyDirectWorkspaceChange(workspace, queues, {
          type: "saveQuickDraft",
          value: candidate.settings.quickDraft.value,
          view,
        }),
      );
    } else {
...
function createTagsAndOrganization(
  workspace: Workspace,
  candidate: WorkspaceDocument,
  queues: IdQueues,
): string[] {
  ...
  for (const tag of candidate.tags) {
    if (!current.tags.some((item) => item.id === tag.id)) {
      errors.push(
        ...applyDirectWorkspaceChange(
          workspace,
          queues,
          { type: "createTag", title: tag.title },
          { tag: [tag.id] },
        ),
      );
    ...
    if (!current.spaces.some((item) => item.id === space.id)) {
      errors.push(
        ...applyDirectWorkspaceChange(
          workspace,
          queues,
          { type: "createSpace", title: space.title, color: space.color, pinned: space.pinned },
          { space: [space.id] },
        ),
      );
      current = workspace.read();
...
function generatedOccurrenceIds(
  candidate: WorkspaceDocument,
  ...
  const projectIds = new Set(
    candidate.projects
      .filter((item) => item.occurrence && !currentProjectIds.has(item.id))
      .map((item) => item.id),
  );
  const headingIds = new Set(
    candidate.headings
      .filter(
        (heading) =>
          heading.location.kind === "project" && projectIds.has(heading.location.projectId),
      )
...
function createProjectsHeadingsAndToDos(
  ...
  for (const project of candidate.projects) {
    if (
      generated.projectIds.has(project.id) ||
  ...
  for (const heading of candidate.headings) {
    if (
      generated.headingIds.has(heading.id) ||
      current.headings.some((item) => item.id === heading.id)
    )
      continue;
    errors.push(
      ...applyDirectWorkspaceChange(
        workspace,
        queues,
        { type: "createHeading", title: heading.title, location: heading.location },
        { heading: [heading.id] },
      ),
    );
    if (heading.archivedAt)
      errors.push(
        ...applyDirectWorkspaceChange(workspace, queues, {
          type: "archiveHeading",
          id: heading.id,
  ...
  for (const toDo of candidate.toDos) {
    if (generated.toDoIds.has(toDo.id) || current.toDos.some((item) => item.id === toDo.id))
      continue;
    errors.push(
      ...applyDirectWorkspaceChange(
        workspace,
        queues,
        {
          ...
          reminderAt: toDo.reminder?.at ?? null,
          deadline: toDo.deadline,
          tags: toDo.tags,
          checklist: toDo.checklist.map((item) => item.title),
        },
...
function repeatingTemplateChanges(current: RepeatingTemplate, desired: RepeatingTemplate) {
  return {
    ...
    ...(current.reminderTime !== desired.reminderTime
      ? { reminderTime: desired.reminderTime }
      : {}),
    ...(current.deadlineOffsetDays !== desired.deadlineOffsetDays
      ? { deadlineOffsetDays: desired.deadlineOffsetDays }
      : {}),
    ...(!sameValue(current.location, desired.location) ? { location: desired.location } : {}),
    ...(!sameValue(
      current.checklist.map((item) => item.title),
      desired.checklist.map((item) => item.title),
    )
      ? { checklist: desired.checklist.map((item) => item.title) }
      : {}),
    ...(desired.itemKind === "project" &&
    current.itemKind === "project" &&
    !sameValue(current.projectContents, desired.projectContents)
      ? { projectContents: desired.projectContents }
      : {}),
  };
}

function createAndUpdateRepeatingTemplates(
  workspace: Workspace,
  candidate: WorkspaceDocument,
  queues: IdQueues,
): string[] {
  ...
  for (const desired of candidate.repeatingTemplates) {
    ...
    if (!current) {
      const linkedToDo = candidate.toDos.find(
        (item) =>
          item.occurrence?.templateId === desired.id &&
          currentDocument.toDos.some((before) => before.id === item.id && !before.occurrence),
      );
      const linkedProject = candidate.projects.find(
        (item) =>
          item.occurrence?.templateId === desired.id &&
          currentDocument.projects.some((before) => before.id === item.id && !before.occurrence),
      );
      if (linkedToDo && desired.itemKind === "toDo") {
        errors.push(
          ...applyDirectWorkspaceChange(
            ...
            queues,
            {
              ...
              id: linkedToDo.id,
              nextDate: desired.firstDate ?? desired.nextDate,
        ...
        errors.push(
          ...applyDirectWorkspaceChange(
            ...
            queues,
            {
              ...
              id: linkedProject.id,
              firstDate: desired.firstDate ?? desired.nextDate,
              pattern: desired.pattern,
              mode: desired.mode,
            },
            { repeatingTemplate: [desired.id] },
        ...
        const template =
          desired.itemKind === "toDo"
            ? {
                itemKind: "toDo" as const,
                title: desired.title,
                notes: desired.notes,
                tags: desired.tags,
                checklist: desired.checklist.map((item) => item.title),
        ...
        errors.push(
          ...applyDirectWorkspaceChange(
            workspace,
            queues,
            { type: "createRepeatingTemplate", template },
            {
              repeatingTemplate: [desired.id],
              checklistItem: desired.checklist.map((item) => item.id),
            },
          ),
        );
      }
      ...
      if (Object.keys(changes).length) {
        ...
        errors.push(
          ...applyDirectWorkspaceChange(
            workspace,
            queues,
            { type: "updateRepeatingTemplate", id: desired.id, changes },
            { checklistItem: checklistIds },
          ),
        );
      }
      current = workspace.read().repeatingTemplates.find((item) => item.id === desired.id)!;
      if (desired.state === "paused" && current.state === "active")
        errors.push(
          ...applyDirectWorkspaceChange(workspace, queues, {
            type: "pauseRepeatingTemplate",
            id: desired.id,
          }),
        );
      if (desired.state === "active" && current.state === "paused")
        errors.push(
          ...applyDirectWorkspaceChange(workspace, queues, {
            type: "resumeRepeatingTemplate",
            id: desired.id,
          }),
        );
      if (desired.state === "stopped" && current.state !== "stopped")
        errors.push(
          ...applyDirectWorkspaceChange(workspace, queues, {
            type: "stopRepeatingTemplate",
            id: desired.id,
          }),
        );
    }
...
function generateNewOccurrences(
  ...
  for (const template of workspace.read().repeatingTemplates) {
    if (template.itemKind === "toDo") {
      const occurrences = candidate.toDos
        .filter(
          (item) => generated.toDoIds.has(item.id) && item.occurrence?.templateId === template.id,
        )
        .sort((left, right) =>
          left.occurrence!.scheduledDate.localeCompare(right.occurrence!.scheduledDate),
        );
      for (const occurrence of occurrences) {
        ...
        if (occurrence.occurrence!.scheduledDate > throughDate)
          throughDate = occurrence.occurrence!.scheduledDate;
      }
      continue;
    }
    const occurrences = candidate.projects
      .filter(
        (item) => generated.projectIds.has(item.id) && item.occurrence?.templateId === template.id,
      )
      .sort((left, right) =>
        left.occurrence!.scheduledDate.localeCompare(right.occurrence!.scheduledDate),
      );
    for (const occurrence of occurrences) {
      ...
      const headings = candidate.headings
        .filter(
          (item) => item.location.kind === "project" && item.location.projectId === occurrence.id,
        )
        .sort((left, right) => left.order - right.order);
      const headingSet = new Set(headings.map((item) => item.id));
      const children = candidate.toDos
        .filter(
          (item) =>
            (item.location.kind === "project" && item.location.projectId === occurrence.id) ||
            (item.location.kind === "heading" && headingSet.has(item.location.headingId)),
        )
        .sort((left, right) => left.order - right.order);
      ...
      for (const child of children)
        checklistItemIds.push(
          ...child.checklist.sort((left, right) => left.order - right.order).map((item) => item.id),
        );
      if (occurrence.occurrence!.scheduledDate > throughDate)
        throughDate = occurrence.occurrence!.scheduledDate;
    }
  ...
  return applyDirectWorkspaceChange(
    workspace,
    queues,
    { type: "generateRepeatingOccurrences", throughDate },
    {
      project: projectIds,
      heading: headingIds,
      toDo: toDoIds,
      checklistItem: checklistItemIds,
    },
  );
}
...
function updateEntities(
  ...
  for (const desired of candidate.headings) {
    ...
    if (desired.archivedAt && !latest?.archivedAt)
      errors.push(
        ...applyDirectWorkspaceChange(workspace, queues, {
          type: "archiveHeading",
          id: desired.id,
  ...
  for (const desired of candidate.toDos) {
      ...
      if (!latest.checklist.some((candidateItem) => candidateItem.id === item.id)) {
        errors.push(
          ...applyDirectWorkspaceChange(
            workspace,
            queues,
            { type: "addChecklistItem", toDoId: desired.id, title: item.title },
      ...
      if (actual && (actual.title !== item.title || actual.completed !== item.completed)) {
        errors.push(
          ...applyDirectWorkspaceChange(workspace, queues, {
            ...
            toDoId: desired.id,
            itemId: item.id,
            changes: { title: item.title, completed: item.completed },
          }),
        );
      }
    }
    ...
    if (
      !sameValue(orderedIds(latest.checklist), desiredChecklistOrder) &&
      desiredChecklistOrder.length
    ) {
      errors.push(
        ...applyDirectWorkspaceChange(workspace, queues, {
          type: "reorderChecklistItems",
          toDoId: desired.id,
          orderedIds: desiredChecklistOrder,
        }),
      );
    }
  }
...
function applyOrders(
  ...
  for (let index = 0; index < desiredSpaces.length; index += 1) {
    if (workspace.read().spaces[index]?.id !== desiredSpaces[index])
      errors.push(
        ...applyDirectWorkspaceChange(workspace, queues, {
          type: "reorderSpace",
          id: desiredSpaces[index],
          toIndex: index,
        }),
      );
  }
  ...
  for (const [name, type, ids] of collections) {
    if (ids.length && !sameValue(orderedIds(workspace.read()[name]), ids))
      errors.push(...applyDirectWorkspaceChange(workspace, queues, { type, orderedIds: ids }));
  }
...
function removeMissingEntities(
  ...
  for (const item of workspace.read().calendarEvents)
    if (!desiredCalendar.has(item.id))
      errors.push(
        ...applyDirectWorkspaceChange(workspace, queues, {
  ...
  for (const item of workspace.read().headings)
    if (!desiredHeadings.has(item.id))
      errors.push(
        ...applyDirectWorkspaceChange(workspace, queues, {
          type: "deleteHeading",
          id: item.id,
          confirmation: DELETE_HEADING_CONFIRMATION,
        }),
      );
  const desiredAreas = new Set(candidate.areas.map((item) => item.id));
  for (const item of workspace.read().areas)
    if (!desiredAreas.has(item.id))
      errors.push(
        ...applyDirectWorkspaceChange(workspace, queues, {
          type: "removeArea",
          id: item.id,
          confirmation: REMOVE_AREA_CONFIRMATION,
        }),
      );
  ...
  for (const item of workspace.read().spaces) {
    ...
    errors.push(
      ...applyDirectWorkspaceChange(workspace, queues, {
        type: "deleteSpace",
        id: item.id,
        moveToSpaceId: candidate.spaces[0].id,
        confirmation: DELETE_SPACE_CONFIRMATION,
      }),
    );
  }
  const desiredTags = new Set(candidate.tags.map((item) => item.id));
  for (const item of workspace.read().tags)
    if (!desiredTags.has(item.id))
      errors.push(
        ...applyDirectWorkspaceChange(workspace, queues, {
          type: "deleteTag",
          id: item.id,
          confirmation: DELETE_TAG_CONFIRMATION,
        }),
      );
  return errors;
}

function finishSettingsAndLogbook(
  workspace: Workspace,
  candidate: WorkspaceDocument,
  queues: IdQueues,
): string[] {
  const errors: string[] = [];
  if (
    candidate.settings.defaultSpaceId &&
    workspace.read().settings.defaultSpaceId !== candidate.settings.defaultSpaceId
  ) {
    errors.push(
      ...applyDirectWorkspaceChange(workspace, queues, {
        type: "setDefaultSpace",
        spaceId: candidate.settings.defaultSpaceId,
      }),
    );
  }
  if (!sameValue(workspace.read().settings.launchRules, candidate.settings.launchRules)) {
    errors.push(
      ...applyDirectWorkspaceChange(workspace, queues, {
        ...
        rules: candidate.settings.launchRules,
      }),
    );
  }
  for (const desired of candidate.toDos) {
    const current = workspace.read().toDos.find((item) => item.id === desired.id);
    if (desired.logbookAt && current && current.outcome !== "open" && !current.logbookAt)
      errors.push(
        ...applyDirectWorkspaceChange(workspace, queues, { type: "logToDo", id: desired.id }),
      );
  }
  for (const desired of candidate.projects) {
    const current = workspace.read().projects.find((item) => item.id === desired.id);
    if (desired.logbookAt && current && current.outcome !== "open" && !current.logbookAt)
      errors.push(
        ...applyDirectWorkspaceChange(workspace, queues, { type: "logProject", id: desired.id }),
      );
  }
  return errors;
}
...
function syncCandidateThroughWorkspace(
  ...
  const unusedIds = [...queues.entries()]
    .filter(([, ids]) => ids.length)
    .flatMap(([kind, ids]) => ids.map((id) => `${kind}:${id}`));
  if (unusedIds.length)
    errors.push(`Workspace identity assignment was incomplete for ${unusedIds.join(", ")}.`);
  return errors;
}
...
export function applyInterfaceChangeSetToWorkspace(
  ...
  if (!changeSet.mutationId || changeSet.mutationId.length > 200)
    return { ok: false, errors: ["A valid interface mutation identity is required."] };
  ...
  const desiredState = applyChangeSet(
    workspaceDocumentToInterfaceState(lifecycleStart, today),
    compatibilityChangeSet,
  );
  const behaviorErrors = [
    ...applyProjectLifecycleChanges(
      workspace,
      lifecycleStart,
      desiredState,
      compatibilityChangeSet,
  ...
  const state = applyChangeSet(
    workspaceDocumentToInterfaceState(behaviorDocument, today),
    compatibilityChangeSet,
  );
  ...
  if (!candidate)
    return {
      ok: false,
      errors: ["The interface change would leave the Workspace without a Space."],
    };
  ...
  const synchronizationErrors = syncCandidateThroughWorkspace(
    workspace,
    behaviorDocument,
    candidate,
    idQueues,
...
export function interfaceLocationForWorkspaceUrl(
  ...
  if (target.kind === "view") {
    ...
    params.set(
      "id",
      heading.location.kind === "project" ? heading.location.projectId : heading.location.areaId,
    );
    const headingLocation = heading.location;
    if (headingLocation.kind === "project") {
      ...
      activeSpaceId =
        projectLocation?.kind === "space"
          ...
          : projectLocation?.kind === "area"
            ? (document.areas.find((item) => item.id === projectLocation.areaId)?.spaceId ?? null)
            : null;
    } else {
      activeSpaceId =
        document.areas.find((item) => item.id === headingLocation.areaId)?.spaceId ?? null;
    }
