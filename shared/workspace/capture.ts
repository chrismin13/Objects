...
function list(value: unknown, limit: number): string[] {
  ...
  for (const item of values) {
    ...
    if (
      clean &&
      !unique.some((existing) => existing.toLocaleLowerCase() === clean.toLocaleLowerCase())
    )
      unique.push(clean);
...
function locationFromRecord(
  ...
  if (rawLocation && typeof rawLocation === "object" && !Array.isArray(rawLocation)) {
    ...
    input = {
      ...
      ...(location.kind === "unfiled" || location.kind === "space"
  ...
  if (headingId) {
    if (!document.headings.some((heading) => heading.id === headingId))
      errors.push("The selected Heading is not available in this Workspace.");
    return { kind: "heading", headingId };
  }
  const projectId = text(input.projectId ?? input.project);
  if (projectId) {
    if (!document.projects.some((project) => project.id === projectId))
      errors.push("The selected Project is not available in this Workspace.");
    return { kind: "project", projectId };
  }
  const areaId = text(input.areaId ?? input.area);
  if (areaId) {
    if (!document.areas.some((area) => area.id === areaId))
      errors.push("The selected Area is not available in this Workspace.");
    return { kind: "area", areaId };
  }
  const spaceId = text(input.spaceId ?? input.space);
  if (spaceId) {
    if (!document.spaces.some((space) => space.id === spaceId))
      errors.push("The selected Space is not available in this Workspace.");
...
function scheduleFromRecord(
  ...
  if (rawSchedule && typeof rawSchedule === "object" && !Array.isArray(rawSchedule)) {
    ...
    input = {
      ...
      ...(schedule.kind === "scheduled"
        ? { scheduledFor: schedule.date, evening: schedule.evening }
        : {}),
    };
  }
  ...
  const evening =
    input.evening === true ||
    text(input.evening).toLocaleLowerCase() === "true" ||
    when === "this evening" ||
    when === "evening";
  if (scheduledFor) {
  ...
  if (when === "today" || when === "this evening" || when === "evening")
    return { kind: "scheduled", date: today, evening };
  ...
  errors.push(
    "The Schedule must be Inbox, Anytime, Someday, Today, Tomorrow, This Evening, or YYYY-MM-DD.",
  );
  return { kind: "inbox" };
}
...
export function captureInputFromRecord(
  ...
  if (!submissionId || submissionId.length > 200)
    errors.push("Capture needs a submission identity of 200 characters or fewer.");

  const sharedText = text(input.text);
  const sharedUrl = text(input.url);
  const title = text(input.title) || sharedText.split(/\r?\n/, 1)[0] || sharedUrl;
  if (!title || title.length > 500)
    errors.push("Capture needs a title of 500 characters or fewer.");

  const noteParts = [text(input.notes)];
  if (sharedText && sharedText !== title) noteParts.push(sharedText);
  if (sharedUrl && sharedUrl !== title && !noteParts.some((part) => part.includes(sharedUrl)))
  ...
  if (reminderAt && !isIsoDateTime(reminderAt))
    errors.push("The Reminder must be an ISO date and time.");
...
export function captureInputFromUrl(
  document: WorkspaceDocument,
  search: string,
  today: string,
): CaptureParseResult {
