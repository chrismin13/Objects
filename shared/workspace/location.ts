...
export function spaceIdForProject(
...
function locationIdsForToDoLocation(
  document: WorkspaceDocument,
  ...
  if (location.kind === "unfiled")
    return { spaceId: location.spaceId, areaId: null, projectId: null };
  ...
  const heading =
    location.kind === "heading"
      ? document.headings.find((item) => item.id === location.headingId)
      : null;
  const projectId =
    location.kind === "project"
      ? location.projectId
      : heading?.location.kind === "project"
        ? heading.location.projectId
        : null;
  if (projectId) {
...
export function locationIdsForToDo(
  document: WorkspaceDocument,
  toDo: ToDo,
): {
  spaceId: EntityId | null;
...
export function effectiveTagIdsForProject(
  document: WorkspaceDocument,
  project: Project,
): EntityId[] {
...
export function spaceIdForRepeatingTemplate(
  document: WorkspaceDocument,
  template: RepeatingTemplate,
): EntityId | null {
  if (template.itemKind === "toDo")
    return locationIdsForToDoLocation(document, template.location).spaceId;
  if (template.location.kind === "space") return template.location.spaceId;
  const areaId = template.location.areaId;
  return document.areas.find((area) => area.id === areaId)?.spaceId ?? null;
}

export function effectiveTagIdsForRepeatingTemplate(
  document: WorkspaceDocument,
  template: RepeatingTemplate,
): EntityId[] {
